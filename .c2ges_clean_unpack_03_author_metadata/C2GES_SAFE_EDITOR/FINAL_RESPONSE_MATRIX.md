...
| Email, funder name/role, final CRediT/COI, AI provenance, rights and repository release were unresolved at Round 3 | The responsible user has since supplied the correspondence email and exact funder/grant wording and has confirmed the existing CRediT allocation and no-conflict declaration by accepting the defaults. The manuscript now names OpenAI Codex, describes its uses in Materials and Methods, and records that the exact backend model identifier/version was not retained. Rights and repository release remain excluded from machine PASS semantics. | `SUBMISSION_HOLDS.md`; manuscript declarations. **CLOSED FOR AUTHOR METADATA AND AI DISCLOSURE; RIGHTS/REPOSITORY HOLDS PRESERVED.** |
...
file-level rights decisions, repository release/archive receipt, and the
editorial decision on retaining the aspirational exact title.
