"""Formal runner for protocol v1.1. Requires a hash-bound human approval."""
...
def main() -> None:
    ...
    audit = json.loads((ROOT / "INDEPENDENT_TECHNICAL_FREEZE_AUDIT_v1_1.json").read_text(encoding="utf-8"))
    ...
    if audit.get("protocol_id") != freeze["protocol_id"] or audit.get("freeze_sha256") != freeze_hash:
        raise SystemExit("Independent audit does not bind this exact v1.1 freeze")
    if not (
        approval.get("approved_by_human")
        ...
        raise SystemExit("Human launch approval is absent or does not bind this exact v1.1 freeze")
    ...
    if calls_completed != 2500:
        raise RuntimeError(f"Expected 2500 calls, got {calls_completed}")
    manifest = {
