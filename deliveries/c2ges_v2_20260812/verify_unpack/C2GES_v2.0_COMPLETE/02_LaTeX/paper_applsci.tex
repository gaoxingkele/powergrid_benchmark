...
\abstract{Long power-system reports often distribute related evidence across distant sections, which makes a relevance-only extract difficult to use as a coherent engineering reading aid. This study presents \cges{}, a deterministic extractive framework that combines centroid relevance, lexical text roles, a typed sentence graph, node-deletion path participation, and source-page traceability. The method was evaluated on a selected public NERC technical-report corpus comprising 12 development reports and 15 test reports. Seven conditions were compared at five- and ten-sentence budgets. Full \cges{} obtained the highest observed ROUGE-L among the evaluated non-ablation systems at both budgets, reaching 0.1060 and 0.1276, respectively. Its extracts were 54--63\% longer than those of Semantic-MMR and TextRank, so these differences are not length-controlled performance advantages. The strict no-path-deletion variant exceeded Full \cges{} by approximately 0.0033 ROUGE-L at both budgets, with report-composition intervals crossing zero. The path-deletion term was nevertheless active: it changed 9774 of 19,008 sentence scores and 28 of 30 report--budget selections. Development-only calibration selected zero path-deletion weight in all 12 leave-one-report-out folds. \cges{} therefore provides a structure-aware, source-linked framework for technical-report navigation, while the current path-deletion integration requires redesign and evaluation under matched output lengths and expert-reviewed data.}
...
\featuredapplication{\cges{} is designed to support source-linked navigation of long power-system technical reports by organizing selected passages around condition, event, impact, and mitigation roles. It is intended as analyst support: extracted passages remain linked to their original pages, and operational use requires validation on maintenance records and expert review.}
...
Extractive summarization is attractive in this setting because it preserves the wording and location of selected passages. Position, centroid relevance, graph centrality, and semantic diversity offer useful ranking signals \cite{zhong2020extractive,bi2021aredsum,liu2021unsupervised,mihalcea2004textrank,reimers2019sentencebert}. These signals, however, do not necessarily distinguish the roles that passages play in an engineering account. A highly central sentence may repeat background information, while a less central sentence may contain the only mitigation or consequence needed to interpret an event.

Graph-based summarizers address relationships among sentences through learned graphs, hypergraphs, or structural filtering \cite{siragusa2022sentencegraph,onan2024mches,hark2025mis}. Power-domain language studies likewise use entity--relation extraction and knowledge graphs to organize fault, alarm, and inspection evidence \cite{liu2022powerfault,ma2025alarm,zgg2026bridge}. \cges{} connects these lines of work through an engineering-oriented formulation: sentences receive transparent textual roles, compatible roles form typed paths, and path participation is combined with relevance, graph salience, position, and redundancy control. The resulting extract remains linked to the original report pages.

The study uses complete public NERC reliability, disturbance, event-analysis, recommendation, and assessment reports \cite{nercEventAnalysis,nerc2015qualityreport}. These reports contain event narratives and corrective lessons but are not utility work orders, inspection records, or field-maintenance narratives. They are therefore treated as a maintenance-oriented technical-report proxy. Twelve reports support development and 15 reports form the retained test set. All candidates are drawn from report pages outside the official Executive Summary, which is used only as the reference for automatic evaluation.

Three questions organize the evaluation. \textit{RQ1} asks how Full \cges{} compares with representative extractive baselines on the selected NERC corpus at fixed sentence budgets and how output length affects that comparison. \textit{RQ2} asks whether the typed path-deletion term improves ROUGE-L or redundancy relative to the otherwise fixed strict no-path-deletion variant. \textit{Exploratory RQ3} asks how consistently the term changes sentence scores and selected sequences across reports and budgets, and whether development-only coefficient calibration supports a nonzero path-deletion weight.

This work makes three contributions. First, it presents a deterministic, role-conditioned typed-path framework for selecting structurally complementary and source-linked evidence from long power-system reports. Second, it evaluates seven conditions at two sentence budgets using complete-PDF candidates, report-level pairing, and explicit output-length diagnostics. Third, it separates system-level performance from component behavior: Full \cges{} produced the highest observed ROUGE-L among the evaluated non-ablation systems, whereas the strict no-path-deletion variant was slightly higher and development calibration did not support a nonzero path weight. This negative component result provides a concrete mechanism diagnosis rather than being used to qualify away the overall framework. The empirical conclusions remain limited to the selected technical-report proxy and do not establish performance on operational maintenance records.

\section{Related Work}

\subsection{Extractive Technical-Document Summarization}

Extractive methods rank source sentences using position, document-centroid similarity, graph centrality, learned salience, or redundancy-aware relevance \cite{zhong2020extractive,bi2021aredsum,liu2021unsupervised}. Lead and centroid methods remain useful controls, while TextRank provides a widely used graph-ranking reference \cite{mihalcea2004textrank}. Dense sentence representations extend lexical matching and support semantic relevance--diversity selection \cite{reimers2019sentencebert}. Holding candidates, references, preprocessing, and sentence budgets constant allows these ranking principles to be compared within one report corpus, although representation and tuning opportunities still differ.
...
ROUGE-1, ROUGE-2, and ROUGE-L F1 quantify lexical overlap between the extract and that Executive Summary \cite{lin2004rouge}. ROUGE-L is the predefined primary endpoint; redundancy is a descriptive secondary outcome. These measures characterize reference overlap and lexical repetition, respectively, rather than expert-assessed completeness or operational usefulness.
...
The unit of analysis is the report. Sentences within a report share topic, vocabulary, layout, and reference-summary content and are not treated as independent observations. Candidate-level quantities drive selection, whereas paired differences and composition intervals use the 15 report-level observations.
...
For each role, the implementation counts registered cue classes and divides all non-negative scores for that role by the largest score in the report. The role channel $R_i$ is the maximum of these five unit-scaled values. A dominant role is assigned only when one positive role score is uniquely maximal. An all-zero vector or equal positive maximum yields abstention. An abstaining sentence remains eligible for selection through the non-role channels but contributes no typed path edge.
...
Hyperparameter opportunity is not balanced across systems (Table~\ref{tab:tuning-opportunity}). Full \cges{} inherited a 144-configuration development search, whereas the Semantic-MMR relevance/diversity coefficient was fixed at 0.5 and TextRank used its implementation defaults. This asymmetry is relevant to system-level ranks. The strict variant is the closer comparison for RQ2 because it shares the selected absolute weights, graph, candidates, and selector.
...
Sentence-count budgets allow every method to return five or ten complete source units. They do not guarantee equal information because extraction units vary in length. Output length was therefore analyzed after selection as a diagnostic; no outputs were truncated or reselected.
...
An exact paired sign-flip sensitivity enumerates all $2^{15}=32{,}768$ sign assignments for each contrast and applies Holm adjustment across the six values \cite{holm1979simple}. This post-result analysis assumes report-level independence and sign exchangeability and is reported in the Supplementary Materials. The interpretation in the main text relies primarily on effect sizes, paired distributions, and composition intervals.
...
Three findings organize the results. First, Full \cges{} achieved the highest observed ROUGE-L among the evaluated non-ablation systems at both sentence budgets, whereas the strict no-path-deletion variant produced the highest overall mean. Second, Full extracts were substantially longer than those of Semantic-MMR and TextRank, which limits the system-level comparison. Third, the path-deletion term changed scores and selected sequences but did not improve the predefined ROUGE-L endpoint; development-only calibration also favored zero weight. Detailed corpus and execution checks are reported in Supplementary Table~S3.

\subsection{Main Comparison (RQ1)}

Table~\ref{tab:all-results} contains all seven conditions and four recorded metrics at both budgets. Full \cges{} exceeded Semantic-MMR and TextRank on mean ROUGE-L at each equal-sentence budget but was below the strict no-path-deletion variant. Because extraction-unit lengths differ materially, these comparisons are not word- or token-budget controlled. Figure~\ref{fig:aggregate} visualizes the same ROUGE-L means.
...
5 & Strict variant & 0.2497 & 0.0648 & \textbf{0.1094} & 0.0798\\
...
10 & Strict variant & 0.3471 & 0.0930 & \textbf{0.1310} & 0.0746\\
...
The strict variant had the largest observed ROUGE-1, ROUGE-2, and ROUGE-L mean at both budgets. Semantic-MMR had the lowest redundancy, consistent with its explicit diversity objective and shorter outputs. Full \cges{} reduced redundancy relative to the strict variant at both budgets while also reducing all three ROUGE means. The configuration therefore expresses a measurable overlap--redundancy trade-off rather than a uniformly better operating point.
...
Figure~\ref{fig:paired} exposes every report-level difference. Full and the strict variant selected different sentence sequences in 28 of 30 report--budget cells, and their base scores differed in all 30, confirming an active coefficient rule but not a benefit.
...
The signs clarify heterogeneity hidden by macro-means. Against the strict variant, Full was positive and negative on equally many non-tied reports at $K=5$ and negative on more reports at $K=10$. Against Semantic-MMR and TextRank, positive signs predominated, although the length mismatch remains.

The report-level signs reveal heterogeneity hidden by the means. Against the strict variant, Full was positive and negative on equally many non-tied reports at $K=5$ and negative on more reports at $K=10$. Against Semantic-MMR and TextRank, positive differences predominated, although the length mismatch remains. The exact paired sign-flip analysis led to the same qualitative directions under a sign-exchangeability assumption; full values and Holm adjustments are reported in Supplementary Table~S4.

\subsection{Component Behavior and Coefficient Support (Exploratory RQ3)}

Across 19,008 Full/strict-variant sentence-score comparisons, 9774 were nonzero, and selected sequences changed in 28 of 30 report--budget cells. The term was therefore active rather than a silent no-op. Its report-level effect was heterogeneous, and the endpoint comparison in Section~4.4 did not show an average ROUGE-L gain.

The post-result development-only calibration evaluated 147 semantically deduplicated configurations. A zero path-deletion weight won all 12 leave-one-report-out folds. The best nonzero candidate used weight 0.025, won no fold, and remained below its strict-zero comparison at both budgets. This exploratory analysis cannot replace the retained test results, but it makes a simple coefficient adjustment an unpromising explanation for the negative ablation. A revised component will likely require different path semantics, role evidence, or score integration.
...
All reported selections and aggregate values were reproduced from the archived run artifacts. Candidate boundaries, condition--budget completeness, source identifiers, path-work limits, and aggregate recalculation passed the packaged checks. These tests support the integrity of the reported experiment; the complete matrix, file identities, and corrective history are provided in the Supplementary Materials.

\section{Discussion}

\subsection{Main Findings and System-Level Comparison}

Full \cges{} recovered more Executive-Summary wording than Semantic-MMR and TextRank on the selected test set at equal sentence counts. The result is consistent with the intended benefit of combining relevance with explicit role coverage and graph structure. It is also partly entangled with extraction-unit length: Full outputs contained 54--63\% more words than the two main baselines. Sentence budgets standardize the number of source units, but they do not standardize the opportunity to match reference wording. The system comparison should therefore be read as evidence that the integrated selector is competitive on this corpus, followed by a clear requirement for a matched-length study.
...
Lexical roles create a second limitation. Negation, hypothetical language, quotations, and cross-sentence coreference are not resolved, and an edge is permitted by role compatibility and local distance rather than by expert-validated discourse relations. A sentence can therefore participate in a strong typed path without adding useful Executive-Summary wording. Conversely, a semantically important passage can receive low path participation when its related evidence lies beyond the 12-position horizon. These explanations are consistent with the active but heterogeneous score changes; they require targeted evaluation rather than being treated as established causal mechanisms.
...
The practical value of \cges{} lies in providing a structured, source-linked entry point into long reports. A reader can begin with passages distributed across condition, event, impact, and mitigation roles and then return to their original pages. Deterministic selection also makes changes attributable to explicit feature channels rather than generation variability. This combination is useful for method development and analyst-facing prototypes even though automatic reference overlap is not a substitute for engineering judgment.
...
The test set contains 15 selected public reports from one organization, and the reports were inspected during corrective reconstruction. Revised editions, recurring institutional language, and near-duplicate passages were not modeled as cross-report dependence. The estimates therefore describe the retained reports; report-level resampling measures sensitivity to their composition rather than population-wide uncertainty.

Equal sentence counts produced unequal word counts, and some extraction units retained table, heading, footnote, or line-break material. Semantic-MMR used a fixed coefficient of 0.5, whereas \cges{} inherited a 144-configuration development search, so tuning opportunities were also asymmetric. A fairer system comparison requires layout-aware segmentation, equal word or token budgets, and comparable development resources.
...
\cges{} is a deterministic, structure-aware framework for extracting source-linked passages from long power-system technical reports. On the selected 15-report NERC test set, Full \cges{} achieved ROUGE-L F1 of 0.1060 and 0.1276 at five and ten sentences and produced the highest observed values among the evaluated non-ablation systems. Its extracts were longer than those of Semantic-MMR and TextRank, so the comparison establishes an equal-sentence observation rather than a length-controlled advantage.

The path-deletion term changed sentence scores and selected sequences but did not improve the predefined ROUGE-L endpoint. The strict variant was higher by approximately 0.0033 at both budgets, and development-only calibration selected zero path weight in all 12 folds. This negative result identifies the next design problem: path semantics and score integration must add information beyond explicit role reservation without relying on longer extraction units. The overall framework provides a reproducible basis for structured report navigation; the next evaluation should use layout-aware units, matched lengths, unseen maintenance-oriented data, and independent assessment by qualified power-grid personnel.

\supplementary{The supplementary PDF contains Table~S1 (the non-verbatim 40-report sampling frame), Table~S2 (output-length records), Table~S3 (corpus and implementation checks), and Table~S4 (exact paired sign-flip sensitivity), together with the exploratory development-only calibration summary. Machine-readable aggregate metrics, paired differences, report-level length records, and selected-ID-to-page locators are included in the accompanying reproducibility files. Source PDFs and verbatim derived text are excluded because third-party redistribution permission has not been established.}
