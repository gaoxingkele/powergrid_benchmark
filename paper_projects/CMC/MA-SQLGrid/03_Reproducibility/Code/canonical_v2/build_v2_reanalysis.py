...
def main() -> int:
    ...
    lines=["# MA-SQLGrid canonical v2 independent reanalysis","",f"**Decision: {'PASS' if result['passed'] else 'FAIL'}.** No model was run; the manuscript and prior canonical release were not modified.","",
        ...
        "## Inference","","Twenty-thousand cluster bootstrap draws preserve all rows within each of 70 normalized-gold-SQL template clusters. Inferential p-values use 100,000 Monte Carlo sign flips at that same cluster unit. Holm adjustment is applied separately to exactly three documented eight-test families. Question-level McNemar discordances and p-values are retained only as descriptive sensitivity fields.","",
