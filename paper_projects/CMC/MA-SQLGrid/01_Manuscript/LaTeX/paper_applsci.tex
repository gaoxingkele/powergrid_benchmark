...
\abstract{Power-grid databases encode asset, maintenance, temporal, and topological semantics, so executable SQL can still answer the wrong question. We present \masql{}, a traceable five-role architecture for intent analysis, schema mapping, candidate normalization, read-only execution validation, and constructed-state testing. A deterministic controller records these steps through a shared blackboard and fails closed when required evidence is incomplete. We evaluated generation and component behavior on GridDB, portability on BIRD Mini-Dev, and selection over a frozen pool of eight candidates for each of 180 GridDB questions. Context and value effects varied across the two frozen model snapshots. Under one shape-and-denotation evaluator, the original fixed-order policy matched 76 questions, while validation-aware and complete-witness selection matched 99 and 100. The strongest fixed candidate source matched 129. Both evidence-aware selectors tied at the highest score on 130 questions and varied with candidate order. Execution and shape evidence could reject some inadmissible candidates but often could not distinguish semantically competing queries. \masql{} therefore provides auditable evaluation and failure localization. The experiments do not establish an end-to-end accuracy advantage from five-role coordination. Broader efficacy requires unseen, expert-reviewed grid queries and budget-matched generation.}
...
\masql{} addresses this systems problem with five bounded role interfaces coordinated through a shared blackboard. The Query Analyst records intent, the Schema Cartographer maps schema and values, the SQL Synthesizer normalizes candidate statements, the Validation Engine records read-only execution evidence, and the Metamorphic-State Critic tests named constructed states. A deterministic controller orders these handoffs, requires specified evidence, and either selects an eligible candidate or abstains. It is not a sixth reasoning agent. This structure localizes errors to intent analysis, schema mapping, candidate normalization, execution, state testing, or adjudication instead of hiding them behind the final SQL.

The empirical design examines this workflow from complementary angles. GridDB experiments vary schema context, structural hints, and presented values on a controlled power-grid schema. A constructed-state study examines whether observed results persist across specified database transformations. BIRD Mini-Dev tests the portability of frozen prompting procedures across 11 public, non-grid databases. Finally, a no-generation study applies fixed-order and evidence-aware selectors to the same historical pool of eight candidates per GridDB question. The studies are not combined into a single accuracy benchmark. Each instead identifies where context, validation evidence, or deterministic selection affects observable behavior.
...
The paper makes three contributions. First, it defines a traceable five-role workflow with explicit boundaries between model-backed and deterministic components. Second, it separates generation behavior, cross-database portability, validation, and candidate selection across GridDB, BIRD, constructed states, and a fixed historical pool. Third, it reconciles the historical results under one shape-and-denotation evaluator and reports all eight fixed candidate sources. Validation-aware and complete-witness selection changed the original fixed-order result from 76 matches to 99 and 100, but the strongest fixed source reached 129. High tie rates and candidate-order sensitivity identify the remaining semantic-ranking bottleneck.

Throughout the paper, \emph{multi-agent} refers to five specialized role interfaces and their traceable coordination. Some interfaces are deterministic, and the controller is not a sixth agent. \emph{Robustness under tested conditions} is limited to mutation denial, bounded read-only execution, complete-evidence gating, deterministic failure handling, and candidate-order perturbation. The experiments do not estimate a causal five-role advantage over a call-matched single agent, and the historical-pool analysis is descriptive.
...
The context experiments do not support a single best prompting strategy. Structural hints sharply changed projected-column adherence, yet none of the nine primary GridDB execution effects survived Holm correction. Presented values improved Qwen first-candidate matches on the paired component subset but produced no change for Granite. BIRD produced another model-specific ordering: schema selection was best for Qwen, whereas bounded repair had the highest Granite point estimate. These patterns are consistent with schema-linking research in which context selection trades recall against distraction \cite{bozdemir2026schema,liu2025solidsql,hao2025genlink}. More context is not uniformly better; its effect depends on the model snapshot, serialization, and type of evidence.

This model dependence motivates modular Query Analyst and Schema Cartographer interfaces because later studies can vary intent extraction, schema retrieval, or value presentation without changing the executor or controller. The present results do not show that this separation improves accuracy. Projected-column conformity is also not a semantic endpoint: a query can adopt the requested result shape while retaining the wrong table, predicate, value, or join.
...
\masql{} makes power-grid Text-to-SQL decisions traceable across intent analysis, schema mapping, candidate normalization, read-only execution validation, and constructed-state testing. A deterministic controller records these handoffs through a shared blackboard, selects only eligible candidates, and can abstain when required evidence is incomplete. The resulting interface exposes errors that a final-SQL-only pipeline would hide.

The historical-pool study shows both the value and the limit of execution evidence. Under the unified evaluator, the original fixed-order policy produced 76/180 reference matches, validation-aware ranking produced 99/180, and complete-witness selection produced 100/180. These descriptive gains characterize selection within the existing pool, not an architectural treatment effect, and neither selector exceeded the strongest fixed source at 129/180. Top-score ties on 130/180 questions and the exact 95--128 range across global candidate orders expose an unresolved semantic-ranking problem.

The central conclusion is therefore not that five-role coordination improves end-to-end accuracy. Execution and shape evidence can filter inadmissible or inconsistent candidates, but they do not yet rank semantically plausible alternatives reliably. MA-SQLGrid provides an auditable research interface for studying that gap. Claims about broader efficacy or deployment require a call- and budget-matched evaluation on unseen, independently expert-reviewed grid queries, with prospectively frozen semantic evidence, tie handling, abstention, and risk--coverage analysis.
