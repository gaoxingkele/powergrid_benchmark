...
- [x] 作者顺序确认为：Bijing Liu → Yong Yang。
- [x] 出版姓名按 0823 原始稿的 given-name/family-name 顺序填写。
- [x] 两位作者均标注单位 `1/2`。
- [x] ORCID：Bijing Liu `NONE`；Yong Yang `NONE`。LaTeX 按 MDPI 模板要求省略 ORCID 命令，不在排印页显示字面值 `NONE`。
- [x] 通讯作者确认为 Yong Yang。
- [x] 通讯邮箱按 0823 原始稿和变更日志恢复为：`yangyong1@sgepri.sgcc.com.cn`。
...
- [x] Institutional Review / Informed Consent 为 `Not applicable`，与仅使用已发布技术报告的计算研究设计一致。
...
- [x] 自动清单确认公开包不含 NERC 原始 PDF 或逐字派生文本。
- [x] 代码许可证状态：`All rights reserved`（仓库未提供显式开源许可证；公开可检查不等于授予复用许可）。
- [x] Data Availability 仅指向 rights-safe 冻结包并说明第三方限制。
- [x] 任何受限材料仅在适用许可允许时向编辑/审稿人提供。

## D. 科学边界与终稿

- [x] 当前范围保持为技术报告代理和事后诊断；不主张专家结构有效性、未见系列泛化或工程任务效用。
- [x] 路径删除项未改善预定义 ROUGE-L 终点，且自动文本图不解释为物理因果图。
...
本地 `submission_package_ready` 可在重新编译和发布校验通过后设为 `true`；投稿门户中的无一稿多投、全体作者批准及最终 PDF 确认仍须由通讯作者人工完成。
