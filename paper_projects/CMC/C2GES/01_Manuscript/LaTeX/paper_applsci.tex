% C2GES protocol-ready prospective upgrade, 5 September 2026.
% Scientific evidence remains the independently audited v0.3.1 post-audit corrective run only;
% E1/E2/E3 outcomes are not inserted until their frozen protocols have been executed.
...
\abstract{Evidence about a power-system event is often dispersed across long technical reports, so relevance-only extracts may not form a coherent engineering account. We present \cges{}, a deterministic framework combining centroid lexical relevance, transparent text roles, a typed sentence graph, node-deletion path participation, and source-page traceability. In a historical retained-test evaluation, seven conditions were compared at two extraction-unit budgets on 12 development and 15 test reports from the North American Electric Reliability Corporation. Full \cges{} reached ROUGE-L F1 values of 0.1060 and 0.1276 and exceeded Semantic-MMR and TextRank at equal unit counts, but its extracts were 54--63\% longer. Under development-derived word caps, every Full-minus-baseline series-cluster interval crossed zero, and no exact comparison survived Holm adjustment. The path term changed most selected sequences without improving the predefined endpoint, while development-only calibration selected zero path weight in all 12 folds. This revision treats those results as historical evidence and predefines unseen-series matched-word evaluation, a controlled component factorial, and independent human validation. Those experiments remain unexecuted. Current evidence establishes an auditable, source-linked implementation and a negative result for the path component; it does not establish a length-controlled advantage, validated structural relations, or operational reading benefit.}
\keyword{extractive summarization; typed textual proxy graph; power-grid reports; length-controlled evaluation; human validation; external-series validation}
...
The prospective upgrade is organized around three confirmatory questions. \textit{RQ1} asks whether recommended \cges{} outperforms principal baselines on unseen report series when every method uses layout-aware candidates, matched 110- and 260-word budgets, and equal development opportunity. \textit{RQ2} asks for the independent and interacting contributions of role scoring, role-group reservation, typed graph salience, normalized path deletion, and redundancy control. \textit{RQ3} asks how strongly independent human judgments support the extraction units, lexical roles, typed edges, typed paths, source faithfulness, and omission behavior. The existing 15-report analysis is retained below as historical evidence and is not reused as a new confirmatory test.

The paper makes three current contributions and defines the evidence needed to sustain stronger claims. First, it introduces a deterministic, role-conditioned typed-path framework that preserves source pages. Second, it reports a fully qualified historical evaluation on complete NERC reports, including output-length, series-cluster, layout, embedding, and scale-preserving path diagnostics. Third, it separates observed system behavior from component benefit: the current path integration is a diagnosed limitation rather than a validated performance mechanism. The prospective protocol adds a layout-aware external-series comparison, a clean component factorial, and direct human construct validation. These additions are design commitments, not empirical contributions, until their frozen runs and blinded annotations are complete.
...
PacSum augments thresholded sentence centrality with asymmetric weights for preceding and following positions \cite{zheng2019pacsum}. It is selected as the prospective modern unsupervised comparator because it combines semantic similarity, graph centrality, and position without supervised summary labels. The controlled implementation is explicitly named PacSum-MiniLM: it independently implements the published centrality equations while using the same frozen MiniLM representation and long-unit policy as prospective Semantic-MMR. It is therefore not presented as a byte-equivalent reproduction of the authors' task-fine-tuned BERT implementation.
...
\subsection{Prospective External-Series Corpus and Layout-Aware Units}

The next confirmatory evaluation is governed by the companion machine-readable external protocol. At the time of this protocol-ready revision, that record remains a draft: external-test access is recorded as false and execution is disabled. Before any external test report is opened or scored, the authors must supply a rights-cleared inventory, source-file hashes, series-disjoint development and test assignments, immutable method revisions, parameter spaces, random seeds, and a signed tuning decision. At least eight independent external series are required and 12 are recommended; report series, rather than individual reports, are the allocation and inferential unit.

The audited PyMuPDF block rule will be promoted to the prospective candidate builder only after a blinded boundary audit. The builder distinguishes body paragraphs, headings, list items, table units, captions, and footnotes; removes recurrent headers, footers, and page numbers; prevents table rows from being fused with body sentences; repairs cross-line and cross-page units; and stores unit type, page, coordinates, source order, word count, and tokenizer length. Before external outcomes are accessed, overall candidate validity must reach 90\% and table--body fusion error must not exceed 5\% under the study-specific audit rule. Failure closes the external-outcome gate and triggers builder revision without access to reference scores.

The confirmatory budgets are 110 and 260 words. Budget enforcement occurs during selection over the complete ranking: a unit that cannot fit the remaining budget is skipped, later units remain eligible, and no unit is truncated. All systems use the same candidates and budget function, and mean realized length must be within 5\% of the nominal budget for an advantage claim. Fixed $K\in\{5,10\}$ results are retained only for historical comparability.
...
\subsection{Human Validation of Roles, Edges, Paths, and Source Faithfulness}

The prospective human study requires two independent annotators, including at least one person with documented power-system reliability, event-analysis, or closely related technical-report experience. The target sample comprises 200 extraction units for boundary assessment, 200 role judgments that may reuse those sampled units as a separate task, 150 typed edges, 80 complete typed paths, and 30 system-summary sets, stratified by report series, split, layout, role confidence, path availability, direction agreement, and Full/no-path selection agreement. During independent annotation, annotators are blinded to system scores, condition names, automated role labels, and one another's labels. The administrator-only sampling manifest is separated from the label forms, and adjudication begins only after the schema, manifest, and both completed annotation files are bound by SHA-256 in the pre-adjudication agreement record.

The tasks separately assess standalone unit validity; one of five roles, none/other, or ambiguous/multiple roles; edge support and direction; path coherence and information added beyond role reservation; and source faithfulness, contextual distortion, critical omission, and page-locator correctness. The analysis reports raw agreement and an appropriate kappa or alpha coefficient, role macro precision/recall/F1, supported-edge precision, direction accuracy, path-validity proportions, faithfulness, omission, and layout/confidence strata. Single-label role metrics exclude and separately report `cannot judge' and ambiguous/multiple judgments. Rates are aggregated with equal report-series weight and series-cluster bootstrap intervals. The adjudication table must cover exactly the frozen disagreements; any changed input, missing disagreement, or added agreement makes the analysis fail closed. Study-specific claim gates are role macro-F1 of at least 0.70, role/edge agreement of at least 0.60, supported-edge precision of at least 0.70, coherent-or-partially-coherent paths of at least 0.70, page-locator accuracy of 100\%, and source faithfulness of at least 0.90. A missed gate downgrades the corresponding construct to a heuristic proxy. Before recruitment, the authors must document the applicable institutional ethics review or exemption determination; this manuscript does not presume that human annotation is automatically exempt.
...
For the prospective external evaluation, the primary estimand is the equal-series mean paired ROUGE-L F1 difference. Report outcomes are first averaged within \texttt{report\_series\_id}; series then receive equal weight. The confirmatory system family comprises recommended \cges{} versus Semantic-MMR, TextRank, and the frozen modern unsupervised baseline at 110 and 260 words. A separate mechanistic family contains the two AB-5/AB-6 budget contrasts; the reservation--path factorial and typed--untyped graph comparison form their own predeclared families. Every key contrast reports the paired effect, a 95\% series-cluster bootstrap interval, an exact series sign-flip value when feasible, Holm adjustment within its family, and leave-one-series-out stability. A length-controlled advantage is stated only when the mean is positive, the interval lower bound exceeds zero, the adjusted exact value is below 0.05, realized length is within tolerance, and no single series reverses the direction.
...
The results in this section are historical retained-test findings; they do not answer the prospective RQ1--RQ3 defined in the Introduction. The unrenormalized coefficient-removal comparison produced the highest overall ROUGE-L mean. Full \cges{} exceeded Semantic-MMR and TextRank at both extraction-unit budgets, but its extracts were substantially longer, which prevents a length-controlled system conclusion. The path-deletion term changed scores and selected sequences but did not improve the predefined ROUGE-L endpoint, and development-only calibration also favored zero weight. E1--E3 results will be inserted only after the external protocol is frozen and executed and the independent annotations are complete.
...
The path-deletion term changed candidate scores and selected sequences without improving the predefined ROUGE-L endpoint. The unrenormalized coefficient-removal comparison was higher by approximately 0.0033 at both budgets, the scale-preserving diagnostic had the same point-estimate direction, and development-only calibration selected zero path weight in all 12 folds. The present empirical contribution is therefore an auditable implementation with a portable public verification subset and a negative component diagnosis. This revision additionally freezes the design vocabulary and claim gates for layout-aware matched-length external evaluation, clean factorial ablation, and independent human validation; it does not report their outcomes. Abstract, component claims, and the final recommended configuration must be updated from measured E1--E3 results before submission.

\supplementary{The supplementary PDF contains Table~S1 (the non-verbatim 40-report sampling frame), Table~S2 (output-length records), Table~S3 (corpus and implementation checks), and Table~S4 (exact paired sign-flip sensitivity), together with the exploratory development-only calibration summary. Machine-readable historical results and selected-ID-to-page locators are included in the accompanying reproducibility files. The protocol-ready package additionally contains the draft external-series freeze record, AB/RP/G condition registry, modern-baseline identity record, administrator-only sampling schema, blinded blank annotation form, pre-adjudication locking and analysis code, qualification record, and ethics-or-exemption gate. These protocol files contain no prospective outcomes or human labels. Source PDFs and verbatim derived text are excluded because third-party redistribution permission has not been established.}
...
\institutionalreview{The historical computational study reported here uses published technical reports and did not involve human participants or animals. The prospective human-annotation experiment has not been executed; the authors must obtain and report the applicable institutional review or exemption determination before recruitment.}
\informedconsent{Not applicable.}
\dataavailability{The manuscript-bound, rights-safe technical package can be inspected in the public \nolinkurl{powergrid_benchmark} repository under the frozen protocol-ready tag \nolinkurl{c2ges-2026-09-06-protocol-ready-v1} via the \href{https://github.com/gaoxingkele/powergrid_benchmark/tree/c2ges-2026-09-06-protocol-ready-v1/paper_projects/CMC/C2GES}{frozen C2GES package}. It includes metadata, hashes, configurations, the non-verbatim selected-ID-to-page locator, descriptive diagnostic records, code, tests, and audit records. Owing to third-party licensing restrictions, source PDFs and verbatim derived text are not publicly redistributed. Access to restricted materials is subject to the applicable third-party permissions and licenses. The historical \texttt{c2ges} repository and the earlier \nolinkurl{cmc-2026-08-24-v3} tag are not the manuscript-bound release for this version. A later submission-final tag is required after E1--E3 backfill.}
