...
- R1 已在无 `.git` 的独立 ZIP 解压目录重新验证：当前 250 个清单文件均为零缺失、零不匹配、零未登记；公共验证全部通过，验证过程未改写 checksum 或 manifest。
...
- E1/E3 已具备一次性、哈希绑定、失败关闭的正式运行器和授权生成器；新增 46 条已见材料排除登记（历史 40 份、预冻结暴露 6 条），当前未执行确认性外部实验，也未生成任何新科学结果。
