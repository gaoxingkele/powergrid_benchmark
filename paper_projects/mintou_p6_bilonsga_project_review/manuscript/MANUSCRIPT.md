...
Utilities select grid-investment portfolios under hard annual budgets, yet generic evolutionary operators do not express changes in review terms. BiLo-NSGA embeds affordable insertion, atomic delete--insert substitution, a heuristic group-label bonus, and feasibility recovery within non-dominated sorting. We compare a metered stage-local BiLo-NSGA implementation with a specified NSGA-II and Pareto Local Search on 120 public-proxy candidates, eight scenarios, and 30 common seeds. Under an exact 3200-unit budget that charges every population-objective evaluation or evaluated local proposal, mean feasible-front hypervolume is 0.16277 for BiLo-NSGA, 0.17213 for NSGA-II, and 0.11626 for local search. BiLo-NSGA loses four of eight Holm-corrected NSGA-II contrasts and wins none, while exceeding local search in all eight. At equal 0.20-s search deadlines, the NSGA-II comparison has three significant wins, two losses, and three unresolved contrasts. Every population is budget-feasible. Bound, clipping, reference-point, and local-parameter checks preserve the NSGA-II--BiLo--local-search pooled ordering but change gap magnitudes; the joint proposal-cap setting $(D_+,D_-)=(2,2)$ is descriptively 4.29% above the registered $(8,4)$ setting. Thus, matched evidence supports a feasible project-vocabulary search framework and a bounded local-search advantage, not superiority over NSGA-II. Event summaries are not lineage or replay records.
...
2. **A shared public benchmark used to isolate the local-search question.** The 120-candidate generator and public source corpora are shared with the TRACE-MOEA companion study; this paper's hard-budget formulation, BiLo-NSGA implementation, scenarios, run records, selected portfolios, and comparisons are reported as paper-specific (Sections 2.4 and 3.4).
3. **Matched-evaluation and matched-time comparisons with explicit multiplicity.** A new immutable archive contains 1440 runs of the metered stage-local BiLo-NSGA, NSGA-II, and Pareto Local Search implementations: 720 at exactly 3200 combined population-objective/local-proposal units and 720 at a common 0.20-s search deadline. The primary 16-contrast family shows four significant BiLo-NSGA losses to NSGA-II and eight wins over the bounded local search; the separately corrected time family is mixed (Section 6.8).
4. **Bound, clipping, reference-point, and local-rule sensitivity with bounded attribution.** Six hypervolume schemes and a seven-cell one-factor local scan retain adverse and null results. The pooled method ordering is stable across the schemes, but scale and gap magnitude are not; the joint $(2,2)$ proposal-cap setting is descriptively stronger than the registered $(8,4)$ setting, while penalty changes are null because evaluated proposals remain feasible. Earlier atomic-substitution contrasts and the public-record checks remain unresolved or descriptive (Sections 6.3--6.5 and 6.8).
...
Portfolio studies respond with tailored search. Doerner et al. [6] apply Pareto ant colony optimization, while Carazo et al. [7] model dependencies, schedules, and resource limits with explicit feasibility handling. Energy applications include coupled risk–benefit portfolios [8] and hybrid exact–heuristic investment planning [9]. These sources establish adjacent portfolio and investment-planning problem classes. They are not used here as evidence that all such methods enforce budgets in one particular way, nor as evidence of atomic delete--insert substitution.

### 2.2. Power Grid Investment Planning and Project Review Decision Methods

Grid investment research includes expansion optimization and multi-criteria review. Optimization studies coordinate transmission and storage [10], value reliability through interruption cost [11], assess grid-side storage returns [12], and balance battery investment against operating benefit [13]. These citations support those asset- and planning-specific descriptions; they are not evidence about performance on this paper's 120-item heterogeneous proxy portfolio.

Review methods include AHP [14] and TOPSIS [15]. Recent work combines objective weighting with an explainable multi-criteria application [16], while a site-selection review covers multi-objective optimization methods [17]. Those method families expose ranking or optimization structure, but the cited sources do not validate BiLo-NSGA's group heuristic, accepted-event summaries, or practical review usefulness. Simulation-based assessment of substations and commissioning delays [18] adds network context, and a survey maps investment-efficiency-oriented grid-infrastructure planning [19]. The gap examined here is limited to the intersection of portfolio search and project-vocabulary move reporting.
...
Power-system studies apply improved NSGA-II variants to distribution-network co-optimization [26], offshore-wind storage configuration [27], and resilient backbone-grid planning [28]. Enhanced swarm methods address storage planning [29] and transmission expansion [30]. These sources establish relevant power-system optimization comparators. The available citation records do not establish that their neighborhoods implement an atomic project substitution, so they are not cited as evidence for that stronger negative claim.

Portfolio-scheduling studies also combine NSGA-II with Pareto local search [31]. More recent methods tailor local refinement to cardinality and pre-assignment constraints [32], while Panadero et al. integrate variable neighborhood search with Monte Carlo simulation for project portfolio selection under uncertain cash flows and discount rates [34]. For this paper, an atomic substitution means that one deletion and one insertion are proposed and accepted or rolled back together. It is not equivalent to a neighborhood that evaluates or commits a deletion and an insertion as two separate moves. References [31], [32], and [34] establish nearby hybrid, constrained-portfolio, and variable-neighborhood methods but are not treated as proof that no earlier atomic operator exists. BiLo-NSGA records accepted move types and pool-local positions during a run, then releases only aggregate count and set-overlap fields; it does not reconstruct a final-portfolio lineage.
...
The resulting positioning is specific. The verified sources cover multi-objective knapsack search, project portfolios, grid planning, multi-criteria ranking, and memetic optimization, but the present audit does not establish an exhaustive absence claim for atomic project substitution across those literatures. BiLo-NSGA combines non-dominated sorting with forward insertion, atomic replacement, feasibility recovery, and accepted-move/repair counters. This is a method-description claim about the implementation, not a cybersecurity, provenance, audit-trail, or universal-superiority claim.
...
$$
\min_{x \in \{0,1\}^n} \; F(x) = \Big( \textstyle\sum_i c_i x_i, \;\; -\sum_i r_i x_i, \;\; -\sum_i g_i x_i, \;\; f_4(x) \Big),
$$

where the fourth objective follows the implemented empty-portfolio convention:

$$
f_4(x)=
\begin{cases}
\dfrac{\sum_i \rho_i x_i}{\sum_i x_i}, & \sum_i x_i>0,\\[4pt]
1, & \sum_i x_i=0.
\end{cases}
$$
...
The four minimization objectives are thus total cost, negated reliability benefit, negated renewable benefit, and mean per-project risk with an explicit empty-portfolio value of 1. The budget is not a soft preference: portfolios exceeding $B$ are not fundable, which is why the benchmark evaluates only the feasible non-dominated front (Section 5.3) and why the method design treats budget slack and violation as first-class quantities that shape the search neighborhood (Section 4).
...
**Formal definitions.** The equations below specify the legacy implementation used for the broad archive; Section 5.4 discloses the metered stage-local differences used in the matched experiment. Let \(x\in\{0,1\}^{n}\) and define remaining budget and the same dimensionless violation used by evaluation and constraint dominance as
...
**Prospective action-task contract.** These equations and Algorithm 1 describe the retained legacy archive; in particular, its “backward” pass is atomic delete--insert substitution. The next-stage four-arm study does not silently reuse that label. Its `backward_only` arm is standalone deletion, while atomic substitution is a separately gated control. The prospective contract also uses one normalized budget violation in scoring, constraint handling, and reporting; strict scalar improvement with equality rejected; ascending stable action IDs for move and repair ties; one cache entry per canonical binary phenotype within a task instance; separate request, cache-hit, and unique-evaluation counts; and both unique-evaluation and request caps. Full equations, pseudocode, and executable invariants are in `manuscript/reconstruction_v2/METHOD_TASK_IMPLEMENTATION_CONTRACT.md`.

That study requires two independently generated action-aligned families rather than relabeling the mixed 120-candidate archive: identified RTS-GMLC branch-rating actions and identified SimBench line-rating actions. Each row must retain its source hash, element ID, preregistered increment, synthetic-cost formula ID/input/coefficients, and provenance. No such formal run has occurred. Missing registries and cost-model provenance keep the gate at NO-GO; the design supplies no calibrated monetary, electrical, cybersecurity, or deployment evidence. Its records are diagnostics, not an audit mechanism.
...
### 5.4. Immutable Matched-Effort Protocol
...
The primary protocol gives every method exactly **3200 search-evaluation units**, so $N_{\mathrm{population}}+N_{\mathrm{local}}=3200$ for every primary run. A unit is either one initialization/offspring candidate scored on the four objectives or one evaluated local proposal. A local proposal is not double-counted as a population evaluation. Budget repair and archive ranking use already visible candidate attributes and do not consume an objective-evaluation unit. Final-front extraction and hypervolume are readouts after search and do not feed selection. This accounting equalizes scored candidates, not primitive CPU operations, generations, or neighborhood opportunities; the separate time protocol supplies the complementary computational estimand. BiLo-NSGA retains the disclosed low-density initialization, uniform crossover, $1/n$ bit mutation, deterministic repair, first-half local refinement, atomic substitution, and constraint non-dominated selection. Pareto Local Search uses 40 repaired starts and the disclosed add/delete/swap cycle. Its 40 starts plus 3160 proposals consume the same total as the other methods.

The metered BiLo-NSGA runner is not semantics-identical to the legacy broad-archive implementation. The legacy code freezes bounds on parents plus pre-repair offspring and uses a $10^{-9}$ denominator floor; the metered runner freezes bounds on parents plus the repaired child and uses $10^{-12}$. The matched results therefore estimate this stage-local metered implementation, not an exact replay of the legacy implementation. The matched runner records evaluation, accepted-move, and repair counts but does not evaluate the legacy accepted-event log or its position-co-occurrence summary.

The recorded legacy optimizer could not be executed in the matched environment. The study therefore implements and labels a fully specified constrained NSGA-II: binary tournament by constraint rank and crowding, 0.9-probability two-point crossover, $1/n$ bit mutation, no repair, and constraint non-dominated environmental selection. The matched NSGA-II result applies to this disclosed implementation; equivalence to the earlier baseline is not claimed. Its final-feasibility rate of 1.0 is an empirical result across the retained runs, not a guarantee supplied by its variation operators. BiLo-NSGA and Pareto Local Search instead construct or repair candidates before retention.

The secondary protocol runs the same cells to a common **0.20-s search deadline**, checked before every candidate or proposal evaluation. Environmental ranking can produce a small recorded overshoot; the observed range is 0.200004--0.201374 s. Method order rotates with seed index. Search runtime, total invocation runtime, population-objective evaluations, local-proposal evaluations, final-population feasibility, feasible-front size, and feasible-front hypervolume are retained per run.

The primary multiplicity family comprises 16 paired contrasts: BiLo-NSGA versus two comparators in eight scenarios. Tests are exact two-sided paired sign tests over the 30 common seed indices, followed by one Holm correction. The 16 matched-time contrasts form a separate secondary family. Hypervolume and local-parameter sensitivity are descriptive and emit no p-values. The six hypervolume schemes cross the reported empirical bounds with clipping on/off, 25%-expanded empirical bounds, conservative analytic bounds, and reference points 1.05, 1.1, and 1.2. The local scan changes one of joint proposal-cap depth $\{2,8,16\}$, violation penalty $\{5,10,20\}$, or group bonus $\{1.00,1.06,1.12\}$ at a time, on all eight scenarios with ten common seeds per cell. Depth values 2, 8, and 16 set $(D_+,D_-)$ to $(2,2)$, $(8,4)$, and $(16,8)$, respectively; the scan does not estimate single-cap effects or interactions among depth, penalty, and bonus.

---

## 6. Results

### 6.1. Legacy Fixed-Generation Comparison
...
**Table 4.** Pooled leaderboard across eight experiments. Stochastic methods have 240 runs; deterministic-rule summaries derive from eight unique outputs. Final-front/event position co-occurrence is descriptive only.
...
**Table 8.** Mean feasible-front hypervolume under exactly 3200 combined evaluation units (30 common seeds per cell). A dagger marks a Holm-significant BiLo-NSGA contrast in the declared 16-test primary family; all four daggers against NSGA-II are losses and all eight against PLS are wins.

| Scenario | BiLo-NSGA | NSGA-II | Pareto Local Search |
|---|---:|---:|---:|
| budget_constrained_selection | 0.14899 | 0.16987$^{\dagger}$ | 0.10517$^{\dagger}$ |
| reliability_prioritized_review | 0.16634 | 0.17006 | 0.11574$^{\dagger}$ |
| renewable_accommodation_review | 0.16745 | 0.17569$^{\dagger}$ | 0.12537$^{\dagger}$ |
| dependency_constrained_review | 0.16840 | 0.17024 | 0.11595$^{\dagger}$ |
| local_move_explainability | 0.16357 | 0.17419 | 0.12190$^{\dagger}$ |
| ranking_robustness | 0.16755 | 0.17389$^{\dagger}$ | 0.11507$^{\dagger}$ |
| budget_sensitivity | 0.14404 | 0.16587$^{\dagger}$ | 0.10016$^{\dagger}$ |
| project_pool_scalability | 0.17578 | 0.17723 | 0.13071$^{\dagger}$ |
| **Pooled descriptive mean** | **0.16277** | **0.17213** | **0.11626** |
...
#### 6.8.3. Hypervolume and Local-Rule Sensitivity
...
The seven-cell local scan is descriptive and retains every adverse or null cell. The depth cells jointly set $(D_+,D_-)$ to $(2,2)$, $(8,4)$, and $(16,8)$. Relative to the registered $(8,4)$ setting, $(2,2)$ raises the scenario-balanced mean by 0.00691 (4.29%) and $(16,8)$ by 0.00035 (0.22%). Changing the group-order bonus from 1.06 to 1.00 changes the mean by -0.00023 (-0.15%); 1.12 changes it by +0.00016 (+0.10%). Penalties 5, 10, and 20 produce identical rows because all locally evaluated proposals are feasible, so the violation term is zero. These results identify $(8,4)$ as an unoptimized fixed choice and provide no measured accuracy benefit for the 1.06 bonus. Because the scan changes one factor at a time, it does not estimate interactions among proposal caps, penalty, and bonus. The results are not used to replace or retune the immutable primary run.

---

## 7. Discussion

**Why the legacy operator effects are forward-side.** Under deterministic repair, populations remain near the budget boundary. Forward insertion can exploit residual slack directly. Atomic substitution must find a removal, an affordable replacement, and a joint scalar improvement in one step, while repair has already removed many weak projects. This makes substitution opportunities rarer, but the explanation remains an inference from the earlier event and ablation patterns rather than a proved mechanism. The matched-evaluation loss to NSGA-II prevents these component contrasts from being promoted to an overall algorithmic advantage.
...
**Positioning against TRACE-MOEA.** The TRACE-MOEA companion study [33] uses preference-adaptive elitism, deterministic repair, and run-level event co-occurrence summaries over the same public candidate pipeline. BiLo-NSGA instead studies project-vocabulary variation under a hard budget. Its central questions concern forward insertion, atomic substitution, and budget-indexed cross-scenario behavior. The methods share candidate generation and public-record infrastructure but not their core operators, configurations, executions, or analyses.

**Practical implications.** Equal evaluation budgets favor the disclosed NSGA-II in pooled hypervolume and produce four significant BiLo-NSGA losses, while equal time produces three BiLo-NSGA wins, two losses, and three unresolved contrasts. This is a quality--allocation trade-off: BiLo-NSGA's inexpensive local proposals permit more scored moves per second, but spending most of a fixed unit budget on those proposals is less effective than NSGA-II's population evaluations in this study. The joint $(2,2)$ proposal-cap setting is descriptively stronger than the registered $(8,4)$ setting, so the committed configuration should not be recommended without a separately registered tuning or validation study. Pareto Local Search remains weaker under both matched protocols, but only as the bounded implementation tested here.
...
33. Lin, Y.; Li, J.; Ruan, X.; Huang, X.; Yang, D. TRACE-MOEA: Constrained Power-Grid Portfolio Search with Preference-Guided Elitism and Run-Level Event Summaries. Unpublished manuscript, 2026; available to editors and reviewers on request.

34. Panadero, J.; Doering, J.; Kizys, R.; Juan, A.A.; Fito, A. A variable neighborhood search simheuristic for project portfolio selection under uncertainty. *Journal of Heuristics* 2020, 26(3), 353--375. https://doi.org/10.1007/s10732-018-9367-z
