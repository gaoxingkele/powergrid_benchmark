...
9. **The matched NSGA-II is stage-local.** The recorded legacy optimizer could not be executed in the matched environment, so the new study uses the fully specified constrained NSGA-II disclosed in Section 5.4. Its results are valid for that implementation; equivalence to the earlier baseline is not claimed. Equal-time results are additionally machine-specific.
