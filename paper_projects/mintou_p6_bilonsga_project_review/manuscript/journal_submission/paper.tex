...
Review practice commonly uses AHP {[}14{]} and TOPSIS {[}15{]}. Recent extensions add objective weighting and explainability {[}16{]}, while site-selection reviews show growing use of multi-objective optimization {[}17{]}. MCDM exposes scoring steps but scores projects largely independently, so it misses budget crowding-out and portfolio interaction. Simulation-based assessments of substations and commissioning delays {[}18{]} add network context but do not solve the portfolio search. The open gap examined here is therefore a portfolio-aware method with project-vocabulary local moves {[}19{]}.
...
dependency\_constrained\_review & 1.00x & group-size-at-least-two filter; no co-selection constraint \\
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Pooled leaderboard across eight experiments. Stochastic methods have 240 runs; deterministic-rule summaries derive from eight unique outputs retained as 240 repeated provenance rows. Position co-occurrence is descriptive only.}\label{tab:4}
