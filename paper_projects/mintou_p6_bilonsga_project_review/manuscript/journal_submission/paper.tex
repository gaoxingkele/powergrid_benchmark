...
\abstract{Utilities select grid-investment portfolios under hard annual budgets, yet generic evolutionary operators do not express changes in review terms. BiLo-NSGA embeds affordable insertion, atomic delete--insert substitution, a heuristic group-label bonus, and feasibility recovery within non-dominated sorting. We compare a metered stage-local BiLo-NSGA implementation with a specified NSGA-II and Pareto Local Search on 120 public-proxy candidates, eight scenarios, and 30 common seeds. Under an exact 3200-unit budget that charges every population-objective evaluation or evaluated local proposal, mean feasible-front hypervolume is 0.16277 for BiLo-NSGA, 0.17213 for NSGA-II, and 0.11626 for local search. BiLo-NSGA loses four of eight Holm-corrected NSGA-II contrasts and wins none, while exceeding the disclosed local-search configuration in all eight. At equal 0.20-s search deadlines, the NSGA-II comparison has three significant wins, two losses, and three unresolved contrasts. Every population is budget-feasible. Bound, clipping, reference-point, and local-parameter checks preserve the NSGA-II--BiLo--local-search pooled ordering but change gap magnitudes; the joint proposal-cap setting \((D_+,D_-)=(2,2)\) is descriptively 4.29\% above the registered \((8,4)\) setting. Thus, matched evidence supports a feasible project-vocabulary search framework and a bounded advantage over the disclosed local-search configuration, not superiority over NSGA-II. Run-level event summaries are not lineage or replay records.}
\keyword{power grid investment planning; project portfolio selection; budget constraint; multi-objective evolutionary optimization; NSGA-II; local search; accepted-move summaries}
...
\tightlist
\item
  \textbf{Budget-aware project-level local moves with run-level summaries.} BiLo-NSGA defines forward insertion, atomic delete--insert substitution, a heuristic group-label bonus, and feasibility recovery within constrained non-dominated sorting. Accepted insertions, substitutions, and repair drops are reduced to run-level count and pool-position co-occurrence fields, providing inspectable summaries without claiming lineage or replay.
\item
  \textbf{Matched-effort evaluation on a reproducible public proxy.} The study compares fully specified BiLo-NSGA, NSGA-II, and Pareto Local Search implementations under both an exact 3200-unit combined evaluation budget and a common 0.20-s search deadline across eight scenarios and 30 common seeds. This design separates search quality from the unmatched computation in the earlier fixed-generation archive.
\item
  \textbf{Component and sensitivity evidence that identifies the quality--effort boundary.} The exact-budget comparison favors NSGA-II over BiLo-NSGA and BiLo-NSGA over the disclosed local-search configuration, whereas equal-time behavior is scenario-dependent. Bound, clipping, reference-point, proposal-cap, group-bonus, and penalty checks show which conclusions are stable and which local-design choices remain unresolved.
\end{enumerate}
...
Selecting items under a budget is the multi-objective knapsack setting used by Zitzler and Thiele \cite{ref1} to compare Pareto-based schemes. Problem-aware search is important in this landscape. Jaszkiewicz \cite{ref2} shows that local search over scalarizing functions can improve on purely recombinative MOEAs, a result reviewed by Lust and Teghem \cite{ref3} and later knapsack taxonomies \cite{ref4}. Many-objective knapsack problems further challenge standard MOEAs \cite{ref5}.

Portfolio studies respond with tailored search. Doerner et al.~\cite{ref6} apply Pareto ant colony optimization, while Carazo et al.~\cite{ref7} model dependencies, schedules, and resource limits with explicit feasibility handling. Energy applications include coupled risk--benefit portfolios \cite{ref8} and hybrid exact--heuristic investment planning \cite{ref9}. These sources establish adjacent portfolio and investment-planning problem classes. They are not used here as evidence that all such methods enforce budgets in one particular way, nor as evidence of atomic delete--insert substitution.
...
Grid investment research includes expansion optimization and multi-criteria review. Optimization studies coordinate transmission and storage \cite{ref10}, value reliability through interruption cost \cite{ref11}, assess grid-side storage returns \cite{ref12}, and balance battery investment against operating benefit \cite{ref13}. These citations support those asset- and planning-specific descriptions; they are not evidence about performance on this paper's 120-item heterogeneous proxy portfolio.

Review methods include AHP \cite{ref14} and TOPSIS \cite{ref15}. Recent work combines objective weighting with an explainable multi-criteria application \cite{ref16}, while a site-selection review covers multi-objective optimization methods \cite{ref17}. Those method families expose ranking or optimization structure, but the cited sources do not validate BiLo-NSGA's group heuristic, accepted-event summaries, or practical review usefulness. Simulation-based assessment of substations and commissioning delays \cite{ref18} adds network context, and a survey maps investment-efficiency-oriented grid-infrastructure planning \cite{ref19}. The gap examined here is limited to the intersection of portfolio search and project-vocabulary move reporting.
...
Hybridizing evolutionary search with local refinement is a long-standing multi-objective strategy. Ishibuchi and Murata \cite{ref20} apply weighted-sum local moves after genetic variation, while Knowles and Corne \cite{ref21} show that a \((1+1)\) local search with a Pareto archive can approximate nondominated fronts competitively. NSGA-II \cite{ref22}, MOEA/D \cite{ref23}, and NSGA-III \cite{ref24} rely primarily on selection and recombination; Neri and Cotta \cite{ref25} survey the many memetic extensions built around them.

Power-system studies apply improved NSGA-II variants to distribution-network co-optimization \cite{ref26}, offshore-wind storage configuration \cite{ref27}, and resilient backbone-grid planning \cite{ref28}. Enhanced swarm methods address storage planning \cite{ref29} and transmission expansion \cite{ref30}. These sources establish relevant power-system optimization comparators. The available citation records do not establish that their neighborhoods implement an atomic project substitution, so they are not cited as evidence for that stronger negative claim.

Portfolio-scheduling studies also combine NSGA-II with Pareto local search \cite{ref31}. More recent methods tailor local refinement to cardinality and pre-assignment constraints \cite{ref32}, while Panadero et al. integrate variable neighborhood search with Monte Carlo simulation for project portfolio selection under uncertain cash flows and discount rates \cite{ref34}. For this paper, an atomic substitution means that one deletion and one insertion are proposed and accepted or rolled back together. It is not equivalent to a neighborhood that evaluates or commits a deletion and an insertion as two separate moves. References \cite{ref31,ref32,ref34} establish nearby hybrid, constrained-portfolio, and variable-neighborhood methods but are not treated as proof that no earlier atomic operator exists. BiLo-NSGA records accepted move types and pool-local positions during a run, then releases only aggregate count and set-overlap fields; it does not reconstruct a final-portfolio lineage.

\subsection{Relation to TRACE-MOEA and Gap Statement}\label{relation-to-trace-moea-and-gap-statement}

The TRACE-MOEA companion study \cite{ref33} addresses power-grid project review through preference-adaptive elitism, deterministic repair, and run-level event co-occurrence summaries. Its contribution acts at selection and ranking. BiLo-NSGA instead changes variation through forward insertion, atomic substitution, a heuristic group-label bonus, and feasibility recovery. The two studies share the candidate generator and the public NERC and MTEP16 source corpora. Their problem objectives, algorithm implementations, scenario definitions, run archives, selected portfolios, and reported comparisons are paper-specific.
...
\subsection{Accepted-Move and Repair Counters}\label{accepted-move-and-repair-counters}
...
\subsection{Immutable Matched-Effort Protocol}\label{immutable-matched-effort-protocol}
...
\subsection{Primary Matched-Effort Finding}\label{primary-matched-effort-finding}

Matched effort changes the interpretation of the earlier fixed-generation archive. Under exactly 3200 combined population-objective and evaluated-local-proposal units, BiLo-NSGA has lower mean feasible-front hypervolume than NSGA-II in all eight scenarios; four contrasts remain significant after correction. The same protocol favors BiLo-NSGA over the disclosed Pareto Local Search in all eight scenarios. At a common 0.20-s search deadline, the NSGA-II comparison becomes heterogeneous, with three significant BiLo-NSGA gains, two losses, and three unresolved contrasts. These matched results, reported in detail in Section~\ref{matched-evaluation-matched-time-and-sensitivity-results}, provide the primary basis for the paper's quality--effort conclusion.

\subsection{Secondary Legacy Fixed-Generation Context}\label{legacy-fixed-generation-comparison}

Table 4 reports the previously archived fixed-generation summaries over eight experiments. This archive remains necessary for the broad baseline and ablation record, but its methods do not have verified equal objective-plus-proposal budgets. It is therefore historical context; Section~\ref{matched-evaluation-matched-time-and-sensitivity-results} is the primary matched-effort comparison. Figure 2 shows the original distribution view, and Section~\ref{direct-local-search-and-substitution-controls} isolates the direct local-search control. Repeated deterministic-rule rows are summarized descriptively and are not treated as independent observations.

\begin{table*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Pooled leaderboard across eight experiments. Stochastic methods have 240 runs; deterministic-rule summaries derive from eight unique outputs. Final-front/event position co-occurrence is descriptive only.}\label{tab:4}
...
The two stress variants bound other claims. Reassigning every third candidate to an isolated group label changes the pooled mean by less than 0.3\%, so no group-density or dependency benefit is claimed. The LooseBudget variant's 6.35\% deficit was discussed in Section~\ref{budget-indexed-cross-scenario-behavior}.
...
In the legacy archive, Pareto Local Search supplies a direct non-evolutionary neighborhood-search baseline with add, delete, and swap moves under 1600 evaluated neighbors per run. Its pooled hypervolume is 0.11636, compared with 0.17190 for BiLo-NSGA, and the proposed method is significantly higher in all eight scenarios after Holm correction. The numerical ceilings do not capture BiLo-NSGA's local proposals, so this is not the exact combined-budget result. Section~\ref{matched-evaluation-matched-time-and-sensitivity-results} reproduces the eight favorable contrasts under both exact total units and equal time. The finding applies to the disclosed PLS configuration rather than to Pareto local search as a family.
...
\caption{Pooled hypervolume and scenario-level significance counts for BiLo-NSGA, Pareto Local Search, NoForwardSearch, NoBackwardSearch, and LegacyDeletion. The direct PLS control is separated in all eight scenarios. Forward insertion resolves only in dependency-constrained review, local-move explainability, and project-pool scalability; both substitution-control families remain unresolved in all eight scenarios.}\label{fig:9}
\end{figure*}

\subsection{Matched-Evaluation, Matched-Time, and Sensitivity Results}\label{matched-evaluation-matched-time-and-sensitivity-results}

\subsubsection{Exact Combined Evaluation Budget}\label{exact-combined-evaluation-budget}
...
\subsubsection{Equal Search Deadlines}\label{equal-search-deadlines}
...
\subsubsection{Hypervolume and Local-Rule Sensitivity}\label{hypervolume-and-local-rule-sensitivity}
...
\textbf{Matched effort defines the principal comparison.} Equal evaluation budgets favor the disclosed NSGA-II in pooled hypervolume and produce four significant adverse BiLo-NSGA contrasts, whereas equal time produces three favorable, two adverse, and three unresolved contrasts. The two protocols reveal an allocation effect: inexpensive local proposals permit more scored moves per second, but allocating most of a fixed unit budget to those proposals is less effective than allocating the same budget to NSGA-II population evaluations on this benchmark. BiLo-NSGA exceeds the disclosed Pareto Local Search under both protocols, but that result applies to the tested implementation rather than to local search as a family.

\textbf{The local-operator evidence is scenario-dependent.} Under deterministic repair, forward insertion can exploit residual budget slack directly. Atomic substitution must identify a removal, an affordable replacement, and a joint improvement in one accepted move, after repair has already removed many weak projects. This interpretation is consistent with the event and ablation patterns, but it is not a proved mechanism. The no-forward ablation has a higher pooled mean, the full method resolves only in three legacy scenarios, and no corresponding resolved gain exists for atomic substitution. The descriptively stronger \((2,2)\) proposal-cap setting further shows that the registered \((8,4)\) allocation was not optimized.

\textbf{External consistency remains a secondary evidence layer.} The NERC rule check shows concentration on documented risk patterns but retains construct overlap with pool generation. The MTEP16 backtest uses independent historical outcomes and 2016-vintage features, yet its project-level diagnostics ignore portfolio dependence and face a severe outcome base rate. Together, the checks test whether optimized portfolios immediately contradict two public-record views; they do not replace expert-labeled feasibility outcomes, calibrated costs, or electrical validation.

\textbf{The run-level summaries have a narrow informational role.} The reported 99.6\% statistic is the set overlap between pool-local positions appearing anywhere in returned fronts and anywhere in a run's event-position set. It does not measure per-portfolio coverage, explanation sufficiency, lineage, or replay. The substitution rule is therefore retained as a project-vocabulary move rather than as a validated reviewer aid.

\textbf{Positioning against TRACE-MOEA.} The TRACE-MOEA companion study \cite{ref33} uses preference-adaptive elitism, deterministic repair, and run-level event co-occurrence summaries over the same public candidate pipeline. BiLo-NSGA instead studies project-vocabulary variation and effort allocation under a hard budget. The methods share candidate generation and public-record infrastructure but not their core operators, configurations, executions, or analyses.
...
The ablation and sensitivity evidence motivates a method redesign. Future atomic substitution should test bounded look-ahead or dominance-based acceptance (Section~\ref{ablation-study-scenario-dependent-forward-effects-and-unresolved-substitution}), and any depth change should be registered before rerunning the primary family. The completed penalty scan is null because proposals are feasible; future acceptance-rule work should therefore vary objective aggregation rather than the inactive violation coefficient. The small bonus effects provide no basis for retaining 1.06 as an accuracy-tuned value.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Limitations}\label{limitations}

Six limitation groups bound the interpretation of the study.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{The proxy review task lacks expert and economic ground truth.} Candidate attributes and synthetic costs are derived from public grid statistics and reliability metadata rather than expert decisions or calibrated utility investments. NERC shares kind-level constructs with pool generation, while the MTEP16 check uses prognostic appendix status and an approved-plan pool with approximately 98\% strict-label build rate. These sources provide descriptive consistency, not validated review quality or engineering-economic evidence.
\item
  \textbf{Benchmark and electrical validity are restricted.} All eight scenarios use one 120-candidate pool derived from RTS-GMLC, SimBench, and NERC, and feasibility is budgetary rather than electrical. A second independently constructed pool and load-flow or OPF checks are needed to test whether the move asymmetry and budget-margin pattern transfer to other systems.
\item
  \textbf{The local-search advantage is comparator-specific.} BiLo-NSGA exceeds the disclosed add/delete/swap Pareto Local Search in all eight scenarios under both matched protocols. The comparison does not cover memetic, large-neighborhood, or exact knapsack methods and therefore supports a bounded result for the implemented configuration rather than family-wide superiority.
\item
  \textbf{The group label is a heuristic rather than a dependency model.} The 1.06 multiplier changes proposal order when a group label is already open but encodes neither precedence nor co-selection constraints. Sensitivity changes pooled mean hypervolume by only -0.15\% at 1.00 and +0.10\% at 1.12. These descriptive differences do not establish dependency synergy or identify 1.06 as an optimal value.
\item
  \textbf{Matched evidence is stage-local and not an exact legacy replay.} The legacy optimizer could not be executed in the matched environment, so the new study uses the fully specified constrained NSGA-II in Section 5.4. The matched BiLo-NSGA runner also differs from the legacy code in normalization-bound timing and denominator floor, and equal-time results are machine-specific. The conclusions therefore apply to the disclosed matched implementations; neither archive serializes the population states or event payloads required for exact event replay.
\item
  \textbf{The local scan is not an interaction study.} Depth changes the forward and backward proposal caps jointly, and depth, penalty, and bonus are varied one factor at a time. Single-cap effects and interactions are therefore unidentified.
\end{enumerate}
...
BiLo-NSGA expresses hard-budget portfolio changes as project-level insertion and atomic substitution moves, with deterministic feasibility recovery and run-level accepted-move summaries. The primary effort-controlled evidence does not support superiority over NSGA-II. At exactly 3200 combined population-objective and evaluated-local-proposal units, pooled feasible-front hypervolume is 0.16277 for BiLo-NSGA and 0.17213 for NSGA-II. All eight scenario means are lower for BiLo-NSGA, four contrasts remain significant after correction, and none significantly favors BiLo-NSGA. The earlier fixed-generation margin is therefore retained only as an unmatched quality--compute trade-off.

At a common 0.20-s search deadline, the comparison is scenario-dependent: three contrasts favor BiLo-NSGA, two favor NSGA-II, and three remain unresolved. BiLo-NSGA exceeds the disclosed Pareto Local Search in all eight scenarios under both matched protocols, but no family-wide local-search conclusion follows. Bound and reference checks preserve the pooled NSGA-II--BiLo--PLS ordering while changing gap magnitude, and the descriptively stronger \((2,2)\) proposal cap identifies the registered \((8,4)\) allocation as a redesign target.

The study therefore supports BiLo-NSGA as a feasible project-vocabulary search framework and identifies how its effectiveness depends on effort allocation. It does not establish BiLo-NSGA superiority over NSGA-II, universal forward benefit, substitution benefit, dependency synergy, lineage, replay, or a preferred operational configuration. Expert labels, calibrated costs, a second benchmark family, broader local-search comparisons, and load-flow verification remain necessary for practical review claims.
...
\bibitem{ref33} Lin, Y.; Li, J.; Ruan, X.; Huang, X.; Yang, D. TRACE-MOEA: Constrained Power-Grid Portfolio Search with Preference-Guided Elitism and Run-Level Event Summaries. Unpublished manuscript, 2026; available to editors and reviewers on request.
\bibitem{ref34} Panadero, J.; Doering, J.; Kizys, R.; Juan, A.A.; Fito, A. A variable neighborhood search simheuristic for project portfolio selection under uncertainty. \emph{Journal of Heuristics} 2020, 26(3), 353--375. \url{https://doi.org/10.1007/s10732-018-9367-z}
\end{thebibliography}
