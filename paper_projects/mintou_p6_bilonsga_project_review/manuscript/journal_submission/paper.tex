...
Portfolio-scheduling studies also combine NSGA-II with Pareto local search \cite{ref31}. More recent methods tailor local refinement to cardinality and pre-assignment constraints \cite{ref32}. For this paper, an atomic substitution means that one deletion and one insertion are proposed and accepted or rolled back together. It is not equivalent to a neighborhood that evaluates or commits a deletion and an insertion as two separate moves. References \cite{ref31,ref32} establish nearby hybrid and constrained-portfolio methods but are not treated as proof that no earlier atomic operator exists. BiLo-NSGA records accepted move types and pool-local positions during a run, then releases only aggregate count and set-overlap fields; it does not reconstruct a final-portfolio lineage.
