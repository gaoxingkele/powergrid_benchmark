...
\dateaccepted{}
\datepublished{}
\Title{BiLo-NSGA: Budget-Aware Project-Level Local Moves with Accepted-Move Logging for Power-Grid Portfolio Optimization}
...
\keyword{power grid investment planning; project portfolio selection; budget constraint; multi-objective evolutionary optimization; NSGA-II; local search; accepted-move logging}
...
\subsection{Legacy Fixed-Generation Comparison}\label{legacy-fixed-generation-comparison}
...
\item
  \textbf{The matched implementations are stage-local.} The recorded legacy optimizer could not be executed in the matched environment, so the new study uses the fully specified constrained NSGA-II disclosed in Section 5.4. The matched BiLo-NSGA runner also differs from the legacy implementation in normalization-bound timing and denominator floor. Results are valid for these disclosed implementations; equivalence to the earlier implementations is not claimed. Equal-time results are additionally machine-specific.
\item
