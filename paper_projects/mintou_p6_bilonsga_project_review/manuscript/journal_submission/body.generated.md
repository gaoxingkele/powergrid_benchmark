...
# Problem Formulation and Public Benchmark

## Budget-Constrained Portfolio Review as Multi-Objective Selection
...
## Benchmark Construction from Public Data
...
## Shared Public Benchmark Statement
...
\caption{BiLo-NSGA architecture. The feedback arrow denotes the next generation. “Dependency-aware” in the diagram denotes only the 1.06 group-label proposal bonus, not a dependency objective or constraint. The lower strip identifies sensitivity and external-consistency analyses rather than optimization inputs.}\label{fig:1}
\end{figure*}
...
## Atomic Backward--Forward Substitution
...
## Heuristic Dependency-Group Bonus
...
## Accepted-Move and Repair Counters
...
## Immutable Matched-Effort Protocol

The fixed-generation archive above does not record function-evaluation counts and therefore cannot establish equal search effort. We consequently preregistered and executed a stage-local comparison of BiLo-NSGA, NSGA-II, and Pareto Local Search on the same eight scenario pools, budgets, objectives, and 30 seed indices. The complete design, implementation, input hashes, per-run rows, and validation record are under `experiments/p6_s3_matched_effort/runs/primary_v1/`; the runner refuses to overwrite an existing output directory.

The primary protocol gives every method exactly **3200 search-evaluation units**. A unit is either one initialization/offspring candidate scored on the four objectives or one evaluated local proposal. A local proposal is not double-counted as a population evaluation. Budget repair and archive ranking use already visible candidate attributes and do not consume an objective-evaluation unit. Final-front extraction and hypervolume are readouts after search and do not feed selection. BiLo-NSGA preserves the disclosed low-density initialization, uniform crossover, $1/n$ bit mutation, deterministic repair, first-half local refinement, atomic substitution, and constraint non-dominated selection. Pareto Local Search uses 40 repaired starts and the disclosed add/delete/swap cycle. Its 40 starts plus 3160 proposals consume the same total as the other methods.

The recorded `pymoo==0.6.2` optimizer runtime is unavailable on this host, while the installed `pymoo==0.4.1` does not expose the committed import paths. Rather than silently substitute a library version, the stage implements and labels a fully specified constrained NSGA-II: binary tournament by constraint rank and crowding, 0.9-probability two-point crossover, $1/n$ bit mutation, no repair, and constraint non-dominated environmental selection. Pymoo 0.4.1 is used only for its pure-Python hypervolume indicator. The matched NSGA-II result therefore applies to this disclosed implementation, not byte-for-byte reproduction of the earlier pymoo baseline.
...
\end{tabularx}
\end{table*}
In this legacy archive, BiLo-NSGA attains pooled mean hypervolume 0.17190 (standard deviation 0.00861), 1.12% above the recorded pymoo NSGA-II (0.17000), 5.87% above NSGA-III, and 23.9% above AHP-TOPSIS. Across five stochastic baselines and eight scenarios, it has 37 positive mean differences in 40 comparisons; 36 are Holm-significant wins and none is a significant loss. These values describe the fixed-generation implementations and must not be read as an effort-controlled advantage. The new 3200-unit comparison reverses the pooled NSGA-II ordering (Section 6.8), so the manuscript's headline conclusion follows the matched result.
...
\end{tabularx}
\end{table*}
In the legacy archive, the 0.75x and renewable-filtered comparisons are unresolved nominal losses and the 0.88x mean gain also fails multiplicity correction. Runtime is 0.219 s versus 0.080 s for NSGA-II, while Pareto Local Search uses 1.416 s. Because neither objective-call nor proposal counts were retained, this table cannot identify whether the 1.12% pooled difference comes from search logic or unequal work. That unresolved fairness issue motivates Section 6.8.

## Budget-Indexed Cross-Scenario Behavior
...
## Ablation Study: Scenario-Dependent Forward Effects and Unresolved Substitution
...
\caption{Local-search diagnostics pooled over eight experiments and 30 seeds (240 runs per configuration). (a) Accepted-local-move and repair-event counts per run; (b) seed-level mean-front hypervolume. Boxes are descriptive aggregates. Formal component decisions use per-experiment Holm-adjusted tests.}\label{fig:5}
\end{figure*}
...
## External Consistency: NERC Rule-Based Backtest
...
## Historical Outcome Consistency: MISO MTEP16 Backtest
...
### Outcome-Definition and Dependence Limits
...
## Search-Effort and Outcome-Definition Diagnostics

For the legacy fixed-generation archive, computation and event-summary production are shown beside hypervolume. BiLo-NSGA averages 0.17190 hypervolume, 0.2188 s, 3668 accepted-move/repair events counted in memory, and 0.996 final-front/event position co-occurrence. NSGA-II averages 0.17000 at 0.0798 s and has no instrumented event list. The historical 1.12% margin and 2.74-fold runtime factor are not effort-controlled; Section 6.8 supplies the matched estimands. The released count and overlap fields are additional outputs, not evidence that a final recommendation can be reconstructed.

\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_search_audit_efficiency.png}
\caption{Pooled search-and-event diagnostics over eight experiments and 30 seeds. The figure's historical "trace" label denotes run-level count and position co-occurrence only. Each panel retains its native unit; no composite score is formed from optimization quality and event volume.}\label{fig:7}
\end{figure*}
...
## Direct Local-Search and Substitution Controls
...
## Matched-Evaluation, Matched-Time, and Sensitivity Results

### Exact Combined Evaluation Budget
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Mean feasible-front hypervolume under exactly 3200 combined evaluation units (30 common seeds per cell). A dagger marks a Holm-significant BiLo-NSGA contrast in the declared 16-test primary family; all four daggers against NSGA-II are losses and all eight against PLS are wins.}\label{tab:8}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
### Hypervolume and Local-Rule Sensitivity
...
8. **The legacy generated configuration is not a complete replay manifest.** Its JSON omits the local penalty, generation-local normalization, group bonus, positional offspring-selection rule, effective MOEA/D population, tie behavior, library versions, and pymoo operator defaults. The new matched-effort package freezes those stage-local choices, source/input hashes, and per-run accounting, but neither archive serializes population states or event payloads. Exact event replay is not supported.

9. **The matched NSGA-II is stage-local.** The host cannot execute the recorded pymoo 0.6.2 optimizer and its installed pymoo 0.4.1 lacks the committed import paths. The new study therefore uses a fully specified stage-local constrained NSGA-II rather than silently changing a library version. Its results are valid for that disclosed implementation; equivalence to the earlier pymoo baseline is not claimed. Equal-time results are additionally machine-specific.

---

# Conclusions

BiLo-NSGA defines budget-aware local moves in project-review terms: insertion under slack, atomic delete--insert substitution, a heuristic group-label bonus, deterministic feasibility recovery, and run-level counting of accepted moves and repair drops. The new effort-controlled evidence does not support superiority over NSGA-II. At exactly 3200 combined population-objective/local-proposal units, pooled feasible-front hypervolume is 0.16277 for BiLo-NSGA and 0.17213 for the fully specified stage-local NSGA-II. All eight mean differences are negative; four are Holm-significant losses and none is a significant win. The earlier 1.12% fixed-generation advantage is therefore retained only as unmatched historical context.
...
All data used in this study are public. Candidate projects are derived from RTS-GMLC (https://github.com/GridMod/RTS-GMLC), SimBench (https://simbench.de), and metadata from public NERC reliability reports (https://www.nerc.com). MISO MTEP16 records are available through the MISO transmission-planning archive (https://www.misoenergy.org/planning/transmission-planning). The supplementary review package contains the legacy 4320-run archive and the immutable matched-effort package at `experiments/p6_s3_matched_effort/runs/primary_v1/`: frozen configuration and environment, input hashes, 1440 per-run comparison rows, 4320 per-run hypervolume-sensitivity rows, 560 local-sensitivity rows, corrected inference, summaries, and validation. The legacy JSON is not a complete replay manifest; the new package records its stage-local parameters and accounting but still does not serialize event payloads or population states. Pre-atomic-substitution backtests remain explicitly suffixed and excluded. A persistent public archive can be supplied before publication, subject to third-party redistribution terms. The companion project `mintou_p5_trace_moea_feasibility_review` (TRACE-MOEA) shares the versioned candidate generator, source corpora, common benchmark/evaluation utilities, and public-record backtest infrastructure. Its evidence files were hash-checked before and after the new run and were unchanged. Problem objectives, method configurations, executions, run outputs, selected portfolios, and reported comparisons are paper-specific.
...
\bibitem{ref33} Lin, Y.; Li, J.; Ruan, X.; Huang, X.; Yang, D. TRACE-MOEA: Constrained Power-Grid Portfolio Search with Adaptive Preference Elitism, Budget Repair, and Run-Level Event Co-Occurrence Summaries. Unpublished manuscript, 2026; available to editors and reviewers on request.
\end{thebibliography}