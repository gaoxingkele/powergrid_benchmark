...
This document records the p4 equation-to-code trace inspected for stage `p4_s2_equation_implementation`. It is a narrative/reproducibility contract, not a new experiment or a substitute for the run archives. No configuration, source implementation, evidence CSV, table, figure, p-value, or numerical result was changed in this stage.
...
| Survivability | $S(x,s)=\min(1,0.42(1-\sigma)+\sum g_ix_i/24)$. | Depends on outage severity, not load or DER output. `NoResilienceObj` removes $-S$ only from environmental selection; screening still uses all five objectives. |
...
- The isolated manuscript worktree does not contain the source/configuration/evidence tree used above.
