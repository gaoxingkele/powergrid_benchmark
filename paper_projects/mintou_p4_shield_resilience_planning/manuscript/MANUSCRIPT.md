...
# SHIELD-MOEA: Scenario Screening with Disjoint Evaluation for Proxy-Based Distribution-Network Resilience Planning
...
2. **A selective worst-$K$ screening mechanism with quantified optimization-loop economy.** At the default configuration, static accounting of optimization-loop objective-array rows falls by 65% (51,200 to 17,920); this denominator excludes common reference-bound construction and held-out evaluation. No front-quality difference is detected against no screening, and periodic re-screening remains unresolved against a fixed generation-1 subset; screening is therefore a selective-exposure mechanism with lower analytic loop-row arithmetic rather than a demonstrated quality or wall-clock mechanism (Section 6.3).
...
Screening concentrates search-phase effort on scenarios difficult under the implemented load/outage proxy. At the default setting, static optimization-loop accounting is 65% lower without a detected change in mean front quality; reference-bound construction and held-out evaluation are outside that denominator. The screening-off ablation is 0.48% lower, and periodic re-screening does not outperform a generation-1 fixed worst-$K$ set. Because the objectives are closed-form and no runtime counter records the analytic call total, this arithmetic does not establish a full-pipeline or wall-clock saving. Screening therefore supplies selective exposure and a computed loop-row reduction, while repair carries the resolved component gain.
...
Finally, screening workload is reported independently of wall-clock time. Including both active-set union evaluation and full-set ranking rounds, the static optimization-loop code-path reduction relative to full-scenario union evaluation is

$$
\eta_{\mathrm{screen}}=1-\frac{GN_uK+N_rN_p|\mathcal S|}{GN_u|\mathcal S|}=1-\frac{17{,}920}{51{,}200}=0.65.
$$

This count and held-out front quality answer different estimands. The 65% reduction is analytic optimization-loop plan--scenario row arithmetic rather than an instrumented objective-call counter. Reference-bound construction and held-out evaluation are excluded from both loop counts; because final-front size varies and no call counter exists, the manuscript does not infer a full-pipeline reduction. Against no screening, the mean-HV contrast is unresolved in all eight labels, equivalence was not tested, and archived runtime is higher rather than lower. Every efficiency reading therefore retains this quality--workload asymmetry.
...
Figure 3 orders the single-switch evidence by what is resolved: repair first, followed by the screening quality--workload comparison, then the two narrower objective/exposure switches. Pooled bars describe the eight archived blocks; the inferential decisions remain within-label.

![Figure 3. Repair and screening mechanism evidence, including proxy quality and static optimization-loop objective-row workload.](figures/fig_repair_screening.png)
...
![Figure 7. Scenario screening quality--workload asymmetry: held-out proxy quality and static optimization-loop plan--scenario objective rows.](figures/fig_screening_quality_calls.png)
...
Component attribution is narrower. SHIELD's initialization-and-offspring repair contributes most within its own ablation. Screening lowers static optimization-loop objective-row arithmetic from 51,200 to 17,920, excluding reference-bound construction and held-out evaluation; no mean-hypervolume difference is detected, equivalence is not established, and archived wall-clock time is not lower. Dynamic re-screening does not beat fixed worst-$K$; hybrid variation does not beat DE-only, including in the boundary matrix; and survivability in environmental selection is a decision-support feature rather than a detected performance feature. The boundary audit further shows that clipping and a very wide cost bound alter gap magnitude even when comparison signs remain stable. Under the illustrative fixed composition mapping, removing outage-aware search is associated with six-network AC feasibility changing from 0.685 to 0.574 and mean maximum loading changing from 52.7% to 67.0%; this is not causal attribution.

Post-event recovery, element-level $N-k$ modeling, monetary calibration, a second independently optimized benchmark system, and nodal AC differentiation remain necessary extensions. Within the tested setting, the main contribution is a scenario-selective planning workflow with disjoint final evaluation, a resolved repair effect, and explicit evidence that screening reduces optimization-loop objective-row workload without a resolved quality, full-pipeline, or wall-clock gain, while hybridization also remains unresolved.
