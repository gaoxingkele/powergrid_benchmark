...
4. **Sampled-envelope, boundary, sensitivity, and cross-family AC diagnostics.** The method retains +5.36% sampled worst-envelope HV and +6.04% when held-out load and outage ranges move beyond the search ranges; the simultaneously changed DER range is inactive in the p4 proxy. A 1050-run predeclared boundary matrix varies budget tightness, scenario count, and the implemented survivability action coefficient, while auditing clipping and reference point 1.2. A 1296-case pandapower check across six networks yields aggregate feasibility 0.685 versus 0.389 without planning, while post-hoc-repaired NSGA-II reaches 0.694. The parameter sweep also reports the setting at which separation from NSGA-II is lost ($N_p=60$, $p=0.104$) rather than presenting only favorable points (Sections 6.2, 6.4--6.6).
...
Practice-oriented studies consider resilient storage siting [11], zone-wise planning under decision-dependent uncertainty [12], and genetic-algorithm-driven feeder hardening [13]. Related work addresses resilient renewable microgrids [14] and islanding with fault recovery [15]. A parallel strand grounds planning claims in full AC physics. Mahmoudi and Alizadeh quantify errors caused by linearized expansion-planning models [16], while Trivic and Savic combine NSGA-II storage planning with power-flow analysis [17]. This evidence motivates the AC validation layer in Section 6.6.
...
|---|---:|---:|---:|---:|---:|
| reference ($B=1.00B_0$, $|\mathcal S|=16$, $\gamma_g=1.00$) | 0.27108 | 0.25725 | 0.01383 [0.00963, 0.01809] | 5.38% | 13 |
...
### 6.6. AC Power-Flow Validation on Six Distribution Networks
...
**Figure 6.** Original SimBench-family AC slice. (a) AC-feasible rate over 72 network-by-scenario cases and over stress scenarios only; the dashed line is the no-plan reference. (b) Mean maximum line loading. The cross-family extension is reported in Table 7 and Figure 9.
...
### 6.7. Quality--Cost and Resilience-Composition Diagnostics
...
**Table 8.** Selected pooled quality--cost readouts from the archived run records.
...
### 6.8. Cross-Family AC Transportability
...
The full method has pooled mean hypervolume 5.09% above post-hoc-repaired NSGA-II and 5.56% above plain NSGA-II over eight archived labels and 30 seed indices per stochastic method. It wins all 32 Holm-corrected comparisons with stochastic baselines; sixteen additional deterministic-rule gaps are descriptive and favor SHIELD-MOEA. Because four labels share the reference proxy configuration, these counts do not represent eight distinct regimes. The method retains a 6.04% mean margin under held-out load/outage ranges and a 5.36% margin on sampled worst-envelope hypervolume. In 1050 predeclared supplementary reruns, its primary within-setting margin over NSGA-II+Repair remains positive across all seven budget, scenario-count, and survivability-coefficient settings (3.46--7.20%), and the direction persists without clipping and at reference point $1.2^5$. Across four SimBench networks, CIGRE MV, and IEEE 33-bus, its mapped compositions improve aggregate AC feasibility over no planning (0.685 versus 0.389) but trail post-hoc-repaired NSGA-II (0.694).

Component attribution is narrower. SHIELD's initialization-and-offspring repair contributes most within its own ablation. Screening has lower static objective-row arithmetic; no mean-hypervolume difference is detected, and equivalence is not established. Dynamic re-screening does not beat fixed worst-$K$; hybrid variation does not beat DE-only, including in the boundary matrix; and survivability in environmental selection is a decision-support feature rather than a detected performance feature. The boundary audit further shows that clipping and a very wide cost bound alter gap magnitude even when comparison signs remain stable. Under the fixed composition mapping, removing outage-aware search reduces six-network AC feasibility from 0.685 to 0.574 and raises mean maximum loading from 52.7% to 67.0%.
