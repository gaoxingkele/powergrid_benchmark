<!-- MDPI Energies submission manuscript.
     ...
     All numerical claims verified against:
       ...
       evidence/runs/p4_s3_boundary_runs_20260813.csv
       evidence/tables/p4_s3_boundary_gaps_20260813.csv
       evidence/tables/p4_s3_hv_bounds_20260813.csv
       evidence/manifests/p4_s4_results_artifact_manifest_20260813.json
...
2. **A selective worst-$K$ screening mechanism with quantified code-path economy.** At the default configuration, static accounting of evaluated objective-array rows falls by 65% (51,200 to 17,920). No front-quality difference is detected against no screening, and periodic re-screening remains unresolved against a fixed generation-1 subset; screening is therefore a selective-exposure mechanism with lower analytic row arithmetic rather than a demonstrated quality or wall-clock mechanism (Section 6.3).
...
4. **Sampled-envelope, boundary, sensitivity, and cross-family AC mapping diagnostics.** Relative to plain NSGA-II, the method retains +5.36% sampled worst-envelope HV and +6.04% when held-out load and outage ranges move beyond the search ranges; the simultaneously changed DER range is inactive in the p4 proxy. A 1050-run predeclared boundary matrix varies budget tightness, scenario count, and the implemented survivability action coefficient, while auditing clipping and reference point 1.2. An illustrative 1296-case pandapower fixed-mapping check across six networks yields aggregate feasibility 0.685 versus 0.389 without planning, while post-hoc-repaired NSGA-II reaches 0.694. The parameter sweep also reports the setting at which separation from NSGA-II is lost ($N_p=60$, $p=0.104$) rather than presenting only favorable points (Sections 6.2, 6.4--6.6).
...
![Figure 1. SHIELD-MOEA scenario screening, hybrid search, and evaluation flow.](figures/fig_architecture.png)
...
This count and held-out front quality answer different estimands. The 65% reduction is analytic plan--scenario row arithmetic rather than an instrumented objective-call counter; against no screening, the mean-HV contrast is unresolved in all eight labels, equivalence was not tested, and archived runtime is higher rather than lower. Every efficiency reading therefore retains this quality--call asymmetry.
...
After reconciling the implemented equations, we froze a compact one-at-a-time matrix before execution. The reference setting uses budget factor 1.0, 16 search and 16 evaluation scenarios, $K=4$, and the implemented survivability action term $\sum_i g_ix_i/24$. Two settings change the budget factor to 0.82 or 1.20; two change both disjoint draw sizes to 8 or 32 while preserving $K/|\mathcal S|=1/4$; and two multiply only the selected-action term in the survivability equation by 0.75 or 1.25. The fixed repair score, including its $g_i/2$ term, is not rescaled, so the last axis is a formulation sensitivity rather than a repair sensitivity. The inactive DER-output multiplier remains generated but cannot affect any p4 objective.

For each of the seven settings, 30 independent method-specific seed streams run SHIELD-MOEA, NSGA-II+Repair, GA-only SHIELD, DE-only SHIELD, and fixed-worst-$K$ SHIELD: $7\times5\times30=1050$ optimizer invocations. Search and evaluation use new disjoint scenario seeds 2026081301 and 2026081302. The NSGA-II comparator retains sparse binary sampling, two-point crossover, bit-flip mutation, the budget constraint, and final-population repair; the local executable uses the installed pymoo 0.4.1 implementation, so this boundary archive supplements rather than replaces the main comparison archive.

Each setting obtains one bounds vector before any method is run from the empty plan, all 72 singletons, and 2048 seeded budget-feasible random plans with the same 5% margin. A bounds digest is recorded on every run row, and verification requires one digest across all methods and seeds within the setting. Every raw mean-objective front is then scored three ways: the primary $[0,1]$-clipped hypervolume at $z^{\mathrm{ref}}=1.1^5$, an unclipped audit at $1.1^5$, and a predeclared alternative $[0,1]$-clipped score at $1.2^5$. The primary contrast is SHIELD-MOEA minus NSGA-II+Repair; mechanism contrasts subtract each within-SHIELD control. Pointwise 95% percentile bootstrap intervals use 5000 fixed-seed resamples of the two independent method samples and are not multiplicity adjusted. Because each formulation setting has its own method-independent bounds, absolute HV levels are not compared across settings; only within-setting gaps are interpreted.
...
Proxy quality is reported before mechanism attribution. Table 3 pools all 8 labels $\times$ 30 seed indices for SHIELD-MOEA and the external comparators. Figure 2 then separates four independent optimizer-seed blocks under the same reference ranges from four configurations that actively change a p4 range or objective. The pooled view weights the reference proxy configuration four times because `deterministic_vs_scenario`, `der_uncertainty`, `scenario_screening_efficiency`, and `pareto_quality` differ only in optimizer seed streams under the p4 equations. Statistical independence of those runs does not turn them into four independent uncertainty regimes; the pooled summary is descriptive over archived blocks.
...
| Experiment | SHIELD | NSGA-II+Repair | $\Delta HV$ (95% CI) | $r_{rb}$ | Holm p |
|---|---:|---:|---:|---:|---:|
...
| scenario_screening_efficiency | 0.26703 | 0.25495 | 0.01207 [0.00796, 0.01593] | 0.744 | 3.80e-6 |
...
The display order makes the coverage boundary explicit. The first four rows use independent optimizer-seed streams but one active reference proxy configuration. The other four rows change the load range, outage range, loss objective, or held-out load/outage ranges. The `der_uncertainty` margin cannot be attributed to $\delta$ because that coordinate is unused by p4 objectives. The unseen-stress label tests changed held-out load and outage ranges, not its simultaneous DER-output shift, and does not certify absence of scenario-distribution specialization.
...
![Figure 2. Proxy-quality comparison across identical-range replicate blocks and distinct active proxy configurations.](figures/fig_proxy_quality.png)

**Figure 2.** Held-out proxy hypervolume (mean +/- std; 30 independent optimizer seeds per method and label) for SHIELD-MOEA, NSGA-II with post-hoc final-population repair, and plain NSGA-II. The first four blocks are identical-range replicates under one active proxy configuration; their independent optimizer seeds provide repeated samples, not four uncertainty regimes. The remaining blocks change an active range or objective. The DER-label block remains a reference replicate because $\delta$ is absent from p4 objectives.
...
### 6.3. Repair Drives the Resolved Gain; Screening Reduces Analytic Calls

Figure 3 orders the single-switch evidence by what is resolved: repair first, followed by the screening quality--call comparison, then the two narrower objective/exposure switches. Pooled bars describe the eight archived blocks; the inferential decisions remain within-label.

![Figure 3. Repair and screening mechanism evidence, including proxy quality and static objective-row workload.](figures/fig_repair_screening.png)

**Figure 3.** Left: pooled mean proxy hypervolume of SHIELD-MOEA and four single-switch ablations over 8 archive-label blocks $\times$ 30 seed indices, with relative differences and the number of within-label Holm-significant contrasts. "No survivability in selection" removes that column from environmental selection only; screening retains the five-objective scalar. Right: screening evaluates 17,920 static plan--scenario objective rows under the declared schedule versus 51,200 without screening. This 65% arithmetic reduction is paired with the unresolved quality contrast, the absence of an equivalence test, and the lack of a wall-clock saving.
...
- **Scenario screening has lower static objective-row arithmetic without a resolved front-quality difference.** The screening-off ablation is nominally 0.48% lower and is unresolved in every label. The declared code path evaluates 17,920 plan--scenario rows with screening and 51,200 without it, a 65% reduction, but the implementation does not instrument an objective-call counter and equivalence was not tested. At this instance size, the full method is slower in the archived runtimes (0.0889 s versus 0.0792 s per run). The quality--call asymmetry is therefore the result: fewer analytic objective rows, no detected quality gain, and no wall-clock saving.
...
| Setting | SHIELD | NSGA-II+Repair | $\Delta HV$ (95% CI) | Relative gap | Fronts clipped / 150 |
|---|---:|---:|---:|---:|---:|
| reference ($B=1.00B_0$, $\lvert\mathcal{S}\rvert=16$, $\gamma_g=1.00$) | 0.27108 | 0.25725 | 0.01383 [0.00963, 0.01809] | 5.38% | 13 |
...
![Figure 6. Illustrative fixed-mapping AC composition check: AC-feasible rates and mean maximum line loading per method.](figures/fig_ac_mapping.png)
...
2. **The mapping associates outage-aware search with a different composition readout.** The NoOutage ablation--identical except that search never experiences outage severity--drops to an AC-feasible rate of 0.574 (0.533 stress-only), while mean maximum loading rises to 67.0%. This fixed-mapping composition difference is illustrative rather than a seeded causal estimate; it motivates a nodal rerun but does not establish an electrical mechanism effect.

3. **A cautionary baseline result.** Cost-first Deterministic Planning attains only a 0.417 AC-feasible rate and mean maximum loading 132.0%, because cheap actions add load-side assets without relieving binding corridors. Its loss value is also the largest in Table 7.

### 6.7. Screening Quality--Call and Resilience-Composition Diagnostics
...
![Figure 7. Scenario screening quality--call asymmetry: held-out proxy quality and static plan--scenario objective rows.](figures/fig_screening_quality_calls.png)

**Figure 7.** Screening quality--call asymmetry. (a) Pooled held-out proxy hypervolume is 0.27396 for SHIELD-MOEA and 0.27266 without screening; the within-label contrast is unresolved in 8/8 labels and equivalence was not tested. (b) The declared search code path evaluates 17,920 static plan--scenario objective rows with screening and 51,200 without it. The implementation lacks an instrumented objective-call counter, and archived wall-clock time is higher with screening, so neither a quality gain nor a runtime saving is claimed.

**Table 8.** Screening quality--call record. Quality values are descriptive pooled means over eight archived blocks; inference is within label. Static objective rows are analytic code-path counts, not an instrumented counter.

| Method | Mean proxy HV | Holm-significant labels vs. full | Static objective rows | Mean runtime (s) | Supported reading |
|---|---:|---:|---:|---:|---|
| SHIELD-MOEA | 0.27396 | reference | 17,920 | 0.0889 | selective exposure with disjoint final scoring |
| NoScenarioScreen | 0.27266 | 0/8 | 51,200 | 0.0792 | no quality difference detected; equivalence not tested |

With $K=4$ of 16 scenarios, active-set union evaluation is 75% smaller than full-scenario union evaluation. Eight full-set ranking rounds add 5120 objective rows, yielding a static total of 17,920 rows versus 51,200 (65% lower). Yet the quality contrast remains unresolved and archived runtime is 0.0889 s versus 0.0792 s. The efficiency result is therefore asymmetric: fewer analytic objective rows, no resolved quality gain, and no measured wall-clock saving. The call count cannot be promoted to energy or deployment savings without an instrumented expensive-simulator experiment.
...
| Do the mechanism conclusions change at the predeclared boundaries? | seven settings against GA-only, DE-only, and fixed worst-$K$ | DE-only mean is higher in 7/7 but all pointwise CIs include zero; fixed worst-$K$ is unresolved in 7/7; tight-budget GA-only has one adverse pointwise interval | no uniform hybrid or dynamic-screening benefit; no equivalence claim |
...
**What the evidence supports, component by component.** The framework-level result is a 5.56% pooled margin over plain NSGA-II, 32/32 significant stochastic-baseline comparisons across eight archived labels, and positive margins in the sampled worst-envelope and held-out-load/outage diagnostics. The pooled result is not an eight-regime DER/load/outage estimate because four independent optimizer-seed blocks share the reference p4 configuration. The new boundary archive retains a positive within-setting margin over post-hoc-repaired NSGA-II across budget, scenario-count, and survivability-coefficient settings and under the clipping/reference audits. It also shows that clipping is frequent and changes magnitude, while the current cost bound is dominated by infeasible singleton plans. Repair contributes most within SHIELD. Screening has 65% lower static objective-row arithmetic while no quality difference is detected against no screening and archived runtime is not lower; periodic re-screening does not beat a fixed generation-1 worst-$K$ set, and hybrid GA/DE does not beat DE-only. The illustrative AC composition check shows a lower feasibility rate when outage exposure is removed (0.574 versus 0.685), but this fixed-mapping association is not a seeded causal estimate or a nodal-plan validation. The method should therefore be read as an evaluated load/outage-proxy pipeline with repair and direct-reuse leakage control, not as evidence for DER-output robustness, calibrated cost tradeoffs, or the necessity of every named operator.

**Scenario selection as a design axis.** Screening reduces static plan--scenario objective-row arithmetic from 51,200 to 17,920, while no quality difference is detected against no screening; equivalence is not established, the implementation has no instrumented objective-call counter, and archived runtime is not lower. This quality--call asymmetry rules out a quality or wall-clock efficiency claim from the present evidence. Dynamic updates also do not outperform a fixed generation-1 worst-$K$ set, including at 8 and 32 scenarios with $K/|\mathcal S|=1/4$. The supported design principle is selective search evaluation with a disjoint scoring set, not periodic adaptation itself. Using one scenario sample for both search decisions and final grading would leave the reported score vulnerable to scenario overfitting.
...
**Practitioner reading.** The full pipeline produces 3.5--6.5% higher mean hypervolume than post-hoc-repaired NSGA-II across the archived labels, not eight distinct regimes. The new boundary reruns retain 3.46--7.20% primary within-setting margins, but these are normalized proxy scores whose magnitude depends on clipping and reference geometry. Ablations associate most of the framework difference with SHIELD's repair schedule rather than screening or hybrid variation. Screening lowers analytic objective-row exposure without a detected quality gain or wall-clock saving. Under the fixed AC mapping, selected compromise compositions have feasible rates within the range of the strongest alternatives and avoid the loading pattern observed for cost-first selection. The NoOutage difference is an illustrative association that motivates nodal validation rather than proving an electrical mechanism.
...
3. **Recovery and restoration are not modeled.** The scenario coordinate $\sigma$ scales a survivability proxy, and the planning stage does not model post-event restoration, network reconfiguration, or intentional islanding. Despite its name, `restoration_aware_evaluation` applies $L^e$ in both search and scoring. The $N-1$ contingency appears only in the illustrative AC mapping layer. An element-level $N-k$ sampling model with fragility-curve-driven failures and post-event restoration optimization--in the tradition of [10,11]--would deepen the resilience claim and simultaneously decorrelate survivability from reliability (Section 6.3).
...
5. **Single planning benchmark family.** All optimization labels instantiate one 72-candidate pool from one SimBench complete-mixed extraction. CIGRE MV and IEEE 33-bus extend only the illustrative AC mapping layer; they do not provide independently optimized candidate pools. The component attribution therefore remains specific to the SimBench-derived planning proxy.

6. **The AC layer checks mapped action-kind counts, not plans or nodal designs.** Only seed-index-0 compromise counts are transported to six networks, and fixed rules reconstruct locations. The resulting comparisons are illustrative and associational. A nodal siting/sizing formulation on each feeder would provide a stronger transfer test and may change the low IEEE 33-bus feasible rates.

7. **Statistical caveats are part of the record.** The advantage over NSGA-II loses significance at population size 60 in the sensitivity sweep ($p=0.104$), the scenario-screening ablation remains unresolved in all eight labels, and the NoResilienceObj ablation nominally exceeds the full method. Moreover, NoResilienceObj retains survivability in scenario screening, so it tests environmental selection only. Boundary-study intervals are pointwise and multiplicity-unadjusted; the tight-budget adverse hybrid-minus-GA-only interval must not be promoted into a confirmatory GA-only superiority claim.

8. **Proxy objectives and normalization choices bound the claims.** The planning-stage objectives are engineering proxies. Raw objectives define dominance; clipping occurs only inside hypervolume, while search scalarization and compromise choice are unclipped. In the boundary archive, 441/1050 fronts require low-side clipping and the primary gap magnitude changes relative to the unclipped audit. The reference sample also includes infeasible singletons, expanding the cost bounds to approximately -32,195 through 676,102 against budgets of 754.4--1104 and compressing feasible cost differences. Method independence prevents output-conditioned scaling but does not make that scale engineering-calibrated. All electrical statements rest on the pandapower composition layer and do not extend beyond its six networks, six operating cases, and fixed mapping.
...
10. **Hybrid and dynamic-screening necessity are not established.** The archived controls return null results: hybrid GA/DE is statistically inseparable from DE-only, and periodic re-screening is inseparable from a fixed generation-1 worst-$K$ set in all eight labels. The boundary reruns do not rescue those mechanisms: DE-only has a higher mean in all seven settings with intervals spanning zero, fixed worst-$K$ remains unresolved, and the tight-budget GA-only contrast is adverse. These mechanisms remain motivated defaults, not demonstrated sources of the headline gain.
...
12. **The boundary archive is supplementary and environment-specific.** Its local runner preserves the declared NSGA-II operator and repair contract under the installed pymoo 0.4.1 implementation, whereas the shared main runner targets a newer pymoo API. The new 1050 runs test the predeclared boundaries and HV definitions; they do not overwrite or pool with the historical main archive.
...
The full method has pooled mean hypervolume 5.09% above post-hoc-repaired NSGA-II and 5.56% above plain NSGA-II over eight archived labels and 30 seed indices per stochastic method. It wins all 32 Holm-corrected comparisons with stochastic baselines; sixteen additional deterministic-rule gaps are descriptive and favor SHIELD-MOEA. Because four independently seeded labels share the reference proxy configuration, these counts do not represent eight distinct regimes. The method retains a 6.04% mean margin over plain NSGA-II under held-out load/outage ranges and a 5.36% margin on sampled worst-envelope hypervolume. In 1050 predeclared supplementary reruns, its primary within-setting margin over NSGA-II+Repair remains positive across all seven budget, scenario-count, and survivability-coefficient settings (3.46--7.20%), and the direction persists without clipping and at reference point $1.2^5$. In the illustrative fixed-mapping AC layer across four SimBench networks, CIGRE MV, and IEEE 33-bus, its compositions have aggregate feasibility 0.685 versus 0.389 without planning and 0.694 for post-hoc-repaired NSGA-II; these are associational composition readouts rather than nodal-plan validation.

Component attribution is narrower. SHIELD's initialization-and-offspring repair contributes most within its own ablation. Screening lowers static objective-row arithmetic from 51,200 to 17,920; no mean-hypervolume difference is detected, equivalence is not established, and archived wall-clock time is not lower. Dynamic re-screening does not beat fixed worst-$K$; hybrid variation does not beat DE-only, including in the boundary matrix; and survivability in environmental selection is a decision-support feature rather than a detected performance feature. The boundary audit further shows that clipping and a very wide cost bound alter gap magnitude even when comparison signs remain stable. Under the illustrative fixed composition mapping, removing outage-aware search is associated with six-network AC feasibility changing from 0.685 to 0.574 and mean maximum loading changing from 52.7% to 67.0%; this is not causal attribution.

Post-event recovery, element-level $N-k$ modeling, monetary calibration, a second independently optimized benchmark system, and nodal AC differentiation remain necessary extensions. Within the tested setting, the main contribution is a scenario-selective planning workflow with disjoint final evaluation, a resolved repair effect, and explicit evidence that screening reduces analytic objective-row workload without a resolved quality or wall-clock gain, while hybridization also remains unresolved.
