...
Real resilience-investment records are utility-internal, so evaluation uses a SimBench-derived proxy benchmark {[}7{]}. Statistics from the 18 highest-ranked subnets generate 72 candidate actions. The archive has eight experiment labels but only five active proxy configurations: four labels share the reference objective configuration under different optimizer seed streams, including \texttt{der\_uncertainty}, because the configured DER-output multiplier is inactive in the p4 objective vector. The unseen-stress setting changes held-out load and outage ranges that do enter the proxy; its changed DER range does not. Shared infrastructure with CARS-MODE is declared in Section 2.4 and the Data Availability Statement.
...
\tightlist
\item
  \textbf{A disjoint-evaluation protocol for scenario-aware evolutionary planning.} SHIELD-MOEA scores final fronts on scenario draws not used during search, preventing direct reuse of the realizations that guided optimization. An unseen-range experiment provides an additional test of search-range specialization (Sections 4.3 and 4.6).
\item
  \textbf{A selective worst-\(K\) screening mechanism with quantified code-path economy.} At the default configuration, static accounting of evaluated objective-array rows falls by 65\% (51,200 to 17,920). No front-quality difference is detected against no screening, and periodic re-screening remains unresolved against a fixed generation-1 subset; screening is therefore a selective-exposure mechanism with lower analytic row arithmetic rather than a demonstrated quality or wall-clock mechanism (Section 6.3).
\item
...
\subsection{Resilience-Oriented Distribution Planning and Hardening Decisions}\label{resilience-oriented-distribution-planning-and-hardening-decisions}
...
\subsection{Scenario Generation, Reduction, and Screening in Stochastic Power System Optimization}\label{scenario-generation-reduction-and-screening-in-stochastic-power-system-optimization}
...
\subsection{Hybrid Multi-Objective Evolutionary Algorithms and Robustness-Aware Objectives}\label{hybrid-multi-objective-evolutionary-algorithms-and-robustness-aware-objectives}
...
\subsection{Relation to CARS-MODE and Gap Statement}\label{relation-to-cars-mode-and-gap-statement}
...
\subsection{Resilience-Oriented Planning as Scenario-Aware Multi-Objective Selection}\label{resilience-oriented-planning-as-scenario-aware-multi-objective-selection}
...
Here, for total selected-source load \(P_\Sigma\) and line length \(D_\Sigma\), the implementation sets \(L_0=0.12+0.015D_\Sigma/\max(1,P_\Sigma)\), \(U_0=0.18+0.010P_\Sigma/\max(1,D_\Sigma)\), and \(H_0=0.45P_\Sigma\). \(L\) is a loss index, \(U\) is a voltage-risk index, \(H\) is a hosting diagnostic, \(R\) is a scenario-invariant reliability proxy, and \(S\) is a survivability proxy driven only by outage severity and selected actions. The experiment labeled \texttt{restoration\_aware\_evaluation} replaces \(L\) by \(L^e\) during both search and evaluation:
...
\subsection{Scenario Model and the Eight Archive Labels}\label{scenario-model-and-the-eight-archive-labels}
...
\caption{The eight archive labels. Ranges are uniform sampling intervals for (load multiplier lambda, DER multiplier delta, outage severity sigma).}\label{tab:1}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
Experiment label & lambda range & delta range & sigma range & Implemented p4 effect \\
\midrule
deterministic\_vs\_scenario & 0.95-1.25 & 0.7-1.3 & 0.00-0.25 & reference proxy configuration \\
der\_uncertainty & 0.95-1.25 & 0.4-1.7 & 0.00-0.25 & inactive-factor replicate: $\delta$ is not consumed by p4 objectives \\
load\_uncertainty & 0.85-1.45 & 0.7-1.3 & 0.00-0.25 & widened load band \\
outage\_contingency & 0.95-1.25 & 0.7-1.3 & 0.10-0.55 & elevated outage severity \\
restoration\_aware\_evaluation & 0.95-1.25 & 0.7-1.3 & 0.10-0.55 & $L^e$ replaces $L$ in search and evaluation \\
scenario\_screening\_efficiency & 0.95-1.25 & 0.7-1.3 & 0.00-0.25 & reference proxy configuration under a separate optimizer seed stream \\
pareto\_quality & 0.95-1.25 & 0.7-1.3 & 0.00-0.25 & reference proxy configuration under a separate optimizer seed stream \\
unseen\_stress\_generalization & 0.95-1.25 (search) & 0.7-1.3 (search) & 0.00-0.25 (search) & evaluation uses $\lambda\in[1.3,1.6]$, $\delta\in[1.4,1.9]$, $\sigma\in[0.4,0.7]$; only the $\lambda$ and $\sigma$ shifts affect p4 scores \\
\bottomrule
\end{tabularx}
\end{table*}
...
\section{SHIELD-MOEA}\label{shield-moea}
...
\caption{SHIELD-MOEA architecture. Scenario screening is population dependent; the feedback arrow denotes generational updating. The two evaluation branches use held-out stress scenarios or an illustrative AC mapping check and do not feed into selection.}\label{fig:1}
\end{figure*}
...
\subsection{Non-Dominated Sorting Kernel (NSGA-II Framework)}\label{non-dominated-sorting-kernel-nsga-ii-framework}
...
\subsection{Hybrid GA/DE Variation}\label{hybrid-gade-variation}
...
\subsection{Resilience Objective and Outage Awareness in Search}\label{resilience-objective-and-outage-awareness-in-search}

The survivability objective \(-S(x,s)\) is part of environmental selection by default. \texttt{Ablation-NoResilienceObj} drops this column from parent--offspring environmental selection but, as implemented, the worst-\(K\) screening score \(\rho_t\) still uses all five objectives. Its result therefore tests removal from environmental selection, not complete removal of survivability information from search. \texttt{Ablation-NoOutage} instead sets \(\sigma=0\) throughout search, affecting both screening and environmental selection while leaving evaluation unchanged. The scope of each contrast is retained in Sections 6.3 and 6.6.

\subsection{Direct-Reuse-Leakage-Controlled Evaluation and Worst-Envelope Rationale}\label{direct-reuse-leakage-controlled-evaluation-and-worst-envelope-rationale}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Methods compared.}\label{tab:2}
...
\subsection{Predeclared Boundary and Hypervolume-Audit Protocol}\label{predeclared-boundary-and-hypervolume-audit-protocol}
...
\subsection{Proxy-Quality Comparison}\label{proxy-quality-comparison}

Proxy quality is reported before mechanism attribution. Table 3 pools all 8 labels \(\times\) 30 seed indices for SHIELD-MOEA and the external comparators. Figure 2 then separates four independent optimizer-seed blocks under the same reference ranges from four configurations that actively change a p4 range or objective. The pooled view weights the reference proxy configuration four times because \texttt{deterministic\_vs\_scenario}, \texttt{der\_uncertainty}, \texttt{scenario\_screening\_efficiency}, and \texttt{pareto\_quality} differ only in optimizer seed streams under the p4 equations. Statistical independence of those runs does not turn them into four independent uncertainty regimes; the pooled summary is descriptive over archived blocks.

\begin{table*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Pooled proxy-quality comparison across eight archive-label blocks. Stochastic methods have 240 runs; deterministic rules summarize eight unique outputs retained as 240 repeated provenance rows. The eight blocks contain five active proxy configurations because the reference configuration appears four times.}\label{tab:3}
...
\midrule
\textbf{SHIELD-MOEA} & \textbf{proposed} & \textbf{0.27396} & \textbf{0.02700} & \textbf{0.26911} & \textbf{38.9} & \textbf{0.089} \\
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{SHIELD-MOEA vs. the strongest external comparator, NSGA-II with post-hoc final-population repair. Intervals are pointwise, multiplicity-unadjusted 5000-resample bootstrap CIs for the mean HV difference; $r_{rb}$ is rank-biserial correlation. Holm correction covers eight stochastic opponents per label and determines the significance statements.}\label{tab:4}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
Experiment & SHIELD & NSGA-II+Repair & $\Delta HV$ (95\% CI) & $r_{rb}$ & Holm p \\
\midrule
deterministic\_vs\_scenario & 0.26959 & 0.25322 & 0.01636 [0.01257, 0.02018] & 0.893 & 1.46e-8 \\
der\_uncertainty & 0.27137 & 0.25502 & 0.01635 [0.01334, 0.01955] & 0.951 & 1.04e-9 \\
scenario\_screening\_efficiency & 0.26703 & 0.25495 & 0.01207 [0.00796, 0.01593] & 0.744 & 3.80e-6 \\
pareto\_quality & 0.26652 & 0.25349 & 0.01302 [0.00905, 0.01697] & 0.738 & 4.77e-6 \\
load\_uncertainty & 0.26211 & 0.24768 & 0.01443 [0.01046, 0.01841] & 0.816 & 2.40e-7 \\
outage\_contingency & 0.26982 & 0.25599 & 0.01382 [0.00995, 0.01747] & 0.798 & 4.63e-7 \\
restoration\_aware\_evaluation & 0.33878 & 0.32699 & 0.01179 [0.00797, 0.01556] & 0.718 & 7.44e-6 \\
unseen\_stress\_generalization & 0.24650 & 0.23824 & 0.00825 [0.00182, 0.01432] & 0.442 & 0.0134 \\
\bottomrule
\end{tabularx}
\end{table*}

The display order makes the coverage boundary explicit. The first four rows use independent optimizer-seed streams but one active reference proxy configuration. The other four rows change the load range, outage range, loss objective, or held-out load/outage ranges. The \texttt{der\_uncertainty} margin cannot be attributed to \(\delta\) because that coordinate is unused by p4 objectives. The unseen-stress label tests changed held-out load and outage ranges, not its simultaneous DER-output shift, and does not certify absence of scenario-distribution specialization.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_proxy_quality.png}
\caption{Held-out proxy hypervolume (mean +/- std; 30 independent optimizer seeds per method and label) for SHIELD-MOEA, NSGA-II with post-hoc final-population repair, and plain NSGA-II. The first four blocks are identical-range replicates under one active proxy configuration; their independent optimizer seeds provide repeated samples, not four uncertainty regimes. The remaining blocks change an active range or objective. The DER-label block remains a reference replicate because $\delta$ is absent from p4 objectives.}\label{fig:2}
\end{figure*}

\subsection{Sampled Worst-Envelope Readout}\label{sampled-worst-envelope-readout}
...
\subsection{Repair Drives the Resolved Gain; Screening Reduces Analytic Calls}\label{repair-drives-the-resolved-gain-screening-reduces-analytic-calls}
...
\begin{itemize}
\item
  \textbf{SHIELD's repair schedule is the main contributor.} Disabling it costs 8.10\% of pooled hypervolume (0.25176 vs.~0.27396) and is Holm-significant in all 8 labels. Without repair, infeasible plans can occupy slots when too few feasible solutions are available, thinning the feasible population evaluated at the end.
\item
  \textbf{Scenario screening has lower static objective-row arithmetic without a resolved front-quality difference.} The screening-off ablation is nominally 0.48\% lower and is unresolved in every label. The declared code path evaluates 17,920 plan--scenario rows with screening and 51,200 without it, a 65\% reduction, but the implementation does not instrument an objective-call counter and equivalence was not tested. At this instance size, the full method is slower in the archived runtimes (0.0889 s versus 0.0792 s per run). The quality--call asymmetry is therefore the result: fewer analytic objective rows, no detected quality gain, and no wall-clock saving.
\item
  \textbf{Removing survivability from environmental selection does not yield a resolved quality difference.} The ablation yields pooled mean 0.27467, nominally +0.26\% above the full method, and is not Holm-significant in any label (0/8; the label-level mean difference ranges from -0.0035 to +0.0039 in absolute HV). This is not a complete no-survivability-search ablation because the five-objective screening score remains active. The supported conclusion is limited to environmental selection: retaining the survivability column exposes a cost--survivability decision coordinate, but no search-quality gain is detected for that use.
\item
  \textbf{Outage exposure has label-dependent proxy effects and an associational AC composition difference.} Under the primary within-label opponent families, zeroing outage severity is significant in \texttt{outage\_contingency} (\(p=0.032\)) and \texttt{restoration\_aware\_evaluation} (\(p=0.009\)). Under the supplementary outage-ablation family across eight labels, only \texttt{restoration\_aware\_evaluation} remains significant (\(p=0.024\); \texttt{outage\_contingency} \(p=0.075\)). The fixed-mapping AC layer is an illustrative composition comparison (Section 6.6), not a seeded causal effect.
\end{itemize}

\textbf{Targeted variation and screening controls.} Three additional controls use the same eight labels, 30 seed indices, population, generations, search/evaluation split, normalization, and evaluation schedule (720 runs). Pooled mean HV is 0.27425 for DE-only, 0.27408 for a worst-\(K\) set fixed after generation 1, 0.27396 for SHIELD-MOEA, and 0.27014 for GA-only. After Holm correction within each label, the full hybrid significantly exceeds GA-only in 3/8 labels and is inseparable in the other five; it is inseparable from DE-only and fixed worst-\(K\) in all eight. Thus the data support neither a hybrid-variation gain nor a dynamic-re-screening gain. The narrower interpretation is that the full stack is strong relative to the external baselines, repair is load-bearing, and screening has a lower static objective-row count; DE-only and one-shot worst-\(K\) are credible engineering simplifications on this benchmark.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_mechanism_controls.png}
\caption{Seed-level mechanism controls across eight archive labels (240 runs per method). (a) Mean-front hypervolume; (b) sampled worst-envelope hypervolume. Boxes pool labels and seeds and are therefore descriptive. Within-label Holm tests show no separation between the full method and DE-only or fixed worst-$K$ in 8/8 labels; the full method exceeds GA-only in 3/8.}\label{fig:4}
\end{figure*}

\subsection{Parameter Sensitivity Analysis}\label{parameter-sensitivity-analysis}

We performed a one-at-a-time sensitivity analysis on the reference experiment (deterministic\_vs\_scenario). The sweep covers population size \(N_p\in\{20,40,60\}\) (default 40, with NSGA-II rerun at each matched size), screening width \(K\in\{2,4,8\}\) (default 4), and screening period \(T_s\in\{3,5,10\}\) (default 5). Each point uses 10 independent seeds. Figure 5 and Table 5 summarize the results.

\begin{table*}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Exploratory parameter sweep (10 seeds per point). Reported Mann--Whitney p-values are nominal and multiplicity-unadjusted.}\label{tab:5}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
Parameter & Value & SHIELD-MOEA HV (mean +/- std) & NSGA-II reference & p (MWU) \\
\midrule
population size N\_p & 20 & 0.2583 +/- 0.0155 & 0.2360 & 0.011 \\
population size N\_p & 40 (default) & 0.2674 +/- 0.0108 & 0.2527 & 0.007 \\
...
screening period T\_s & 5 (default) & 0.2687 +/- 0.0085 & 0.2527 & 0.002 \\
screening period T\_s & 10 & 0.2671 +/- 0.0121 & 0.2527 & 0.017 \\
\bottomrule
\end{tabularx}
\end{table*}

Note: the default configuration (population 40, K=4, T\_s=5) appears once per sweep axis, so the table contains three default rows; they show slightly different SHIELD-MOEA means (0.2674, 0.2658, 0.2687) because each axis is drawn from an independent 10-seed stream -- a normal consequence of sampling variance, not an inconsistency. For the same reason the NSGA-II reference is identical (0.2527) across the K and T\_s rows, which reuse the matched population-40 reference, while the population-size rows re-run NSGA-II at each matched size (0.2360, 0.2527, 0.2610).

\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_sensitivity.png}
\caption{Parameter sensitivity over population size, screening width $K$, and screening period $T_s$ (10 seeds per point; NSGA-II re-run at matched population sizes).}\label{fig:5}
\end{figure*}
...
\subsection{Predeclared Formulation Boundary and Hypervolume Audit}\label{predeclared-formulation-boundary-and-hypervolume-audit}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Predeclared boundary reruns. Hypervolume uses fixed bounds within each setting, clipping to $[0,1]$, and reference point $1.1^5$. Intervals are pointwise, multiplicity-unadjusted 5000-resample bootstrap intervals for the independent-sample mean difference. The final column counts all five methods and 30 seeds in that setting.}\label{tab:6}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\subsection{Illustrative Fixed-Mapping AC Check on Six Distribution Networks}\label{illustrative-fixed-mapping-ac-check-on-six-distribution-networks}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Illustrative fixed-mapping AC composition summary over six networks (108 cases per method). Only action-kind counts are transported; rows are descriptive mapped cases and carry no optimizer-level p-values.}\label{tab:7}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Screening quality--call record. Quality values are descriptive pooled means over eight archived blocks; inference is within label. Static objective rows are analytic code-path counts, not an instrumented counter.}\label{tab:8}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_resilience_tradeoff.png}
\caption{Pooled reliability and survivability of compromise portfolios derived from the frozen evaluation records; marker area represents worst-case hypervolume. The three readouts remain separately encoded rather than being collapsed into a synthetic index.}\label{fig:8}
\end{figure*}
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Mechanism-level decision record.}\label{tab:9}
...
\subsection{Cross-Family AC Mapping Illustration}\label{cross-family-ac-mapping-illustration}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_cross_family_ac.png}
\caption{Illustrative AC-feasible rates and mean minimum bus voltage by network family for the no-plan reference, SHIELD-MOEA, and NSGA-II with post-hoc final-population repair. The dashed voltage line marks the 0.95 pu criterion. Each family receives the same three seed-0 action-kind compositions through fixed placement rules and six operating scenarios. SHIELD-MOEA improves over no planning in this mapping but does not outperform the comparator; no causal or nodal-transfer claim follows.}\label{fig:9}
\end{figure*}
...
\bibitem{ref1} Panteli, M.; Mancarella, P. Influence of extreme weather and climate change on the resilience of power systems: Impacts and possible mitigation strategies. \emph{Electric Power Systems Research} \textbf{2015}, \emph{127}, 259-270. \url{https://doi.org/10.1016/j.epsr.2015.06.012}
\bibitem{ref2} Ton, D.T.; Wang, W.T.-P. A more resilient grid: The U.S. Department of Energy joins with stakeholders in an R\&D plan. \emph{IEEE Power and Energy Magazine} \textbf{2015}, \emph{13}(3), 26-34. \url{https://doi.org/10.1109/MPE.2015.2397337}
\bibitem{ref3} Yuan, W.; Wang, J.; Qiu, F.; Chen, C.; Kang, C.; Zeng, B. Robust optimization-based resilient distribution network planning against natural disasters. \emph{IEEE Transactions on Smart Grid} \textbf{2016}, \emph{7}(6), 2817-2826. \url{https://doi.org/10.1109/TSG.2015.2513048}
\bibitem{ref4} Mishra, D.K.; Eskandari, M.; Abbasi, M.H.; Sanjeevikumar, P.; Zhang, J.; Li, L. A detailed review of power system resilience enhancement pillars. \emph{Electric Power Systems Research} \textbf{2024}, \emph{230}, 110223. \url{https://doi.org/10.1016/j.epsr.2024.110223}
\bibitem{ref5} Ma, S.; Chen, B.; Wang, Z. Resilience enhancement strategy for distribution systems under extreme weather events. \emph{IEEE Transactions on Smart Grid} \textbf{2018}, \emph{9}(2), 1442-1451. \url{https://doi.org/10.1109/TSG.2016.2591885}
\bibitem{ref6} Lin, Y.; Bie, Z. Tri-level optimal hardening plan for a resilient distribution system considering reconfiguration and DG islanding. \emph{Applied Energy} \textbf{2018}, \emph{210}, 1266-1279. \url{https://doi.org/10.1016/j.apenergy.2017.06.059}
\bibitem{ref7} Meinecke, S.; Sarajlic, D.; Drauz, S.R.; Klettke, A.; Lauven, L.-P.; Rehtanz, C.; Moser, A.; Braun, M. SimBench -- A benchmark dataset of electric power systems to compare innovative solutions based on power flow analysis. \emph{Energies} \textbf{2020}, \emph{13}(12), 3290. \url{https://doi.org/10.3390/en13123290}
\bibitem{ref8} Panteli, M.; Mancarella, P. The grid: Stronger, bigger, smarter?: Presenting a conceptual framework of power system resilience. \emph{IEEE Power and Energy Magazine} \textbf{2015}, \emph{13}(3), 58-66. \url{https://doi.org/10.1109/MPE.2015.2397334}
\bibitem{ref9} Bhusal, N.; Abdelmalak, M.; Kamruzzaman, M.; Benidris, M. Power system resilience: Current practices, challenges, and future directions. \emph{IEEE Access} \textbf{2020}, \emph{8}, 18064-18086. \url{https://doi.org/10.1109/ACCESS.2020.2968586}
\bibitem{ref10} Panteli, M.; Pickering, C.; Wilkinson, S.; Dawson, R.; Mancarella, P. Power system resilience to extreme weather: Fragility modeling, probabilistic impact assessment, and adaptation measures. \emph{IEEE Transactions on Power Systems} \textbf{2017}, \emph{32}(5), 3747-3757. \url{https://doi.org/10.1109/TPWRS.2016.2641463}
\bibitem{ref11} Chen, Y.; Shi, Q.; Tang, B.; Zhang, Y.; Wang, H. A distributed energy storage-based planning method for enhancing distribution network resilience. \emph{Energies} \textbf{2026}, \emph{19}(2), 574. \url{https://doi.org/10.3390/en19020574}
\bibitem{ref12} Chen, X.; Liu, L.; Kang, X. A differential planning strategy for distribution network resilience enhancement considering decision dependence uncertainty. \emph{Energies} \textbf{2025}, \emph{18}(23), 6353. \url{https://doi.org/10.3390/en18236353}
\bibitem{ref13} Maris, T.; Christodoulou, C.; Mladenov, V. Enhancing distribution network resilience using genetic algorithms. \emph{Electronics} \textbf{2025}, \emph{14}(21), 4324. \url{https://doi.org/10.3390/electronics14214324}
\bibitem{ref14} Kiptoo, M.K.; Adewuyi, O.B.; Furukakoi, M.; Mandal, P.; Senjyu, T. Integrated multi-criteria planning for resilient renewable energy-based microgrid considering advanced demand response and uncertainty. \emph{Energies} \textbf{2023}, \emph{16}(19), 6838. \url{https://doi.org/10.3390/en16196838}
\bibitem{ref15} Yang, Z.; Han, J.; Li, L.; Deng, Y.; Yang, F.; Lei, Y. Research of islanding operation and fault recovery strategies of distribution network considering uncertainty of new energy. \emph{Electronics} \textbf{2023}, \emph{12}(20), 4230. \url{https://doi.org/10.3390/electronics12204230}
\bibitem{ref16} Mahmoudi, S.; Alizadeh, B. Simultaneous generation and network expansion planning in large-scale power systems under exact AC power flow equations. \emph{IET Generation, Transmission and Distribution} \textbf{2022}, \emph{16}(20), 4158-4175. \url{https://doi.org/10.1049/gtd2.12586}
\bibitem{ref17} Trivic, B.; Savic, A. Optimal allocation and sizing of BESS in a distribution network with high PV production using NSGA-II and LP optimization methods. \emph{Energies} \textbf{2025}, \emph{18}(5), 1076. \url{https://doi.org/10.3390/en18051076}
\bibitem{ref18} Dupacova, J.; Growe-Kuska, N.; Romisch, W. Scenario reduction in stochastic programming: An approach using probability metrics. \emph{Mathematical Programming} \textbf{2003}, \emph{95}(3), 493-511. \url{https://doi.org/10.1007/s10107-002-0331-0}
\bibitem{ref19} Heitsch, H.; Romisch, W. Scenario reduction algorithms in stochastic programming. \emph{Computational Optimization and Applications} \textbf{2003}, \emph{24}(2-3), 187-206. \url{https://doi.org/10.1023/A:1021805924152}
\bibitem{ref20} Morales, J.M.; Pineda, S.; Conejo, A.J.; Carrion, M. Scenario reduction for futures market trading in electricity markets. \emph{IEEE Transactions on Power Systems} \textbf{2009}, \emph{24}(2), 878-888. \url{https://doi.org/10.1109/TPWRS.2009.2016072}
\bibitem{ref21} Papavasiliou, A.; Oren, S.S. Multiarea stochastic unit commitment for high wind penetration in a transmission constrained network. \emph{Operations Research} \textbf{2013}, \emph{61}(3), 578-592. \url{https://doi.org/10.1287/opre.2013.1174}
\bibitem{ref22} Chen, Y.; Wang, Y.; Kirschen, D.; Zhang, B. Model-free renewable scenario generation using generative adversarial networks. \emph{IEEE Transactions on Power Systems} \textbf{2018}, \emph{33}(3), 3265-3275. \url{https://doi.org/10.1109/TPWRS.2018.2794541}
\bibitem{ref23} Babatunde, O.; Fasesin, K.; Dosa, A.; Ighravwe, D.; Ogbemhe, J.; Olanrewaju, O. Multi-stage stochastic MILP framework for renewable microgrid dispatch under high renewable penetration: Optimizing variability and uncertainty management. \emph{Applied Sciences} \textbf{2025}, \emph{15}(19), 10303. \url{https://doi.org/10.3390/app151910303}
\bibitem{ref24} Esmaeilnezhad, B.; Amini, H.; Noroozian, R.; Jalilzadeh, S. Flexible reconfiguration for optimal operation of distribution network under renewable generation and load uncertainty. \emph{Energies} \textbf{2025}, \emph{18}(2), 266. \url{https://doi.org/10.3390/en18020266}
\bibitem{ref25} Wang, G.; Li, H.; Yang, X.; Lu, H.; Song, X.; Li, Z. Multi-objective site selection and capacity determination of distribution network considering new energy uncertainties and shared energy storage of electric vehicles. \emph{Electronics} \textbf{2025}, \emph{14}(1), 151. \url{https://doi.org/10.3390/electronics14010151}
\bibitem{ref26} Li, P.; Shen, Y.; Shang, Y.; Alhazmi, M. Innovative distribution network design using GAN-based distributionally robust optimization for DG planning. \emph{IET Generation, Transmission and Distribution} \textbf{2025}, \emph{19}(1), e13350. \url{https://doi.org/10.1049/gtd2.13350}
\bibitem{ref27} Zhou, Y.; Li, X.; Han, H.; Wei, Z.; Zang, H.; Sun, G.; Chen, S. Resilience-oriented planning of integrated electricity and heat systems: A stochastic distributionally robust optimization approach. \emph{Applied Energy} \textbf{2024}, \emph{353}, 122053. \url{https://doi.org/10.1016/j.apenergy.2023.122053}
\bibitem{ref28} Yin, W.; Hou, Y. Models and applications of stochastic programming with decision-dependent uncertainty in power systems: A review. \emph{IET Renewable Power Generation} \textbf{2024}, \emph{18}(14), 2819-2834. \url{https://doi.org/10.1049/rpg2.13082}
\bibitem{ref29} Wu, X.; Hao, Y.; Zhou, J.; Zhang, X.; Zhang, Y. A scenario generation method for wind/PV power outputs and load sequences preserving extreme scenario characteristics. \emph{IET Renewable Power Generation} \textbf{2026}, \emph{20}(1), e70185. \url{https://doi.org/10.1049/rpg2.70185}
\bibitem{ref30} Deb, K.; Pratap, A.; Agarwal, S.; Meyarivan, T. A fast and elitist multiobjective genetic algorithm: NSGA-II. \emph{IEEE Transactions on Evolutionary Computation} \textbf{2002}, \emph{6}(2), 182-197. \url{https://doi.org/10.1109/4235.996017}
\bibitem{ref31} Zhang, Q.; Li, H. MOEA/D: A multiobjective evolutionary algorithm based on decomposition. \emph{IEEE Transactions on Evolutionary Computation} \textbf{2007}, \emph{11}(6), 712-731. \url{https://doi.org/10.1109/TEVC.2007.892759}
\bibitem{ref32} Zitzler, E.; Thiele, L. Multiobjective evolutionary algorithms: A comparative case study and the strength Pareto approach. \emph{IEEE Transactions on Evolutionary Computation} \textbf{1999}, \emph{3}(4), 257-271. \url{https://doi.org/10.1109/4235.797969}
\bibitem{ref33} Storn, R.; Price, K. Differential evolution -- A simple and efficient heuristic for global optimization over continuous spaces. \emph{Journal of Global Optimization} \textbf{1997}, \emph{11}(4), 341-359. \url{https://doi.org/10.1023/A:1008202821328}
\bibitem{ref34} Das, S.; Suganthan, P.N. Differential evolution: A survey of the state-of-the-art. \emph{IEEE Transactions on Evolutionary Computation} \textbf{2011}, \emph{15}(1), 4-31. \url{https://doi.org/10.1109/TEVC.2010.2059031}
\bibitem{ref35} Jin, Y.; Branke, J. Evolutionary optimization in uncertain environments -- A survey. \emph{IEEE Transactions on Evolutionary Computation} \textbf{2005}, \emph{9}(3), 303-317. \url{https://doi.org/10.1109/TEVC.2005.846356}
\bibitem{ref36} Deb, K.; Gupta, H. Introducing robustness in multi-objective optimization. \emph{Evolutionary Computation} \textbf{2006}, \emph{14}(4), 463-494. \url{https://doi.org/10.1162/evco.2006.14.4.463}
\bibitem{ref37} Zhao, C.; Zhou, Y.; Lai, X. An integrated framework with evolutionary algorithm for multi-scenario multi-objective optimization problems. \emph{Information Sciences} \textbf{2022}, \emph{600}, 342-361. \url{https://doi.org/10.1016/j.ins.2022.03.093}
\bibitem{ref38} Yadav, D.; Ramu, P.; Deb, K. Handling objective preference and variable uncertainty in evolutionary multi-objective optimization. \emph{Swarm and Evolutionary Computation} \textbf{2025}, \emph{94}, 101860. \url{https://doi.org/10.1016/j.swevo.2025.101860}
\bibitem{ref39} He, R.; Hao, J.; Zhou, H.; Chen, F. Multi-objective collaborative optimization of distribution networks with energy storage and electric vehicles using an improved NSGA-II algorithm. \emph{Energies} \textbf{2025}, \emph{18}(19), 5232. \url{https://doi.org/10.3390/en18195232}
\bibitem{ref40} Ding, J.; Liao, Q.; Tang, F.; Li, B.; Yu, Y.; Zhou, T. Bi-objective resilient backbone-grid planning via a three-stage TER-NSGA-II approach considering pumped-storage hub effects. \emph{Energies} \textbf{2026}, \emph{19}(12), 2798. \url{https://doi.org/10.3390/en19122798}
\bibitem{ref41} Qi, H.; Zhao, C.; Yan, X.; Zhang, W.; Guo, F.; Zhang, L.; Yang, B.; Lu, H. Vulnerability-driven multi-objective energy storage planning using enhanced beluga whale optimization for resilient distribution networks. \emph{Energies} \textbf{2026}, \emph{19}(1), 210. \url{https://doi.org/10.3390/en19010210}
\bibitem{ref42} Zhang, L.; Zheng, J.; Zhang, Z.; Ni, S.; Wu, G. CARS-MODE: Constraint-Aware Repair and Strategy-Pool Multi-Objective Differential Evolution for Distribution Network Expansion Planning. Unpublished manuscript, 2026; available to editors and reviewers on request.
\bibitem{ref43} Blank, J.; Deb, K. pymoo: Multi-objective optimization in Python. \emph{IEEE Access} \textbf{2020}, \emph{8}, 89497-89509. \url{https://doi.org/10.1109/ACCESS.2020.2990567}
\bibitem{ref44} Holm, S. A simple sequentially rejective multiple test procedure. \emph{Scandinavian Journal of Statistics} \textbf{1979}, \emph{6}(2), 65-70.
\bibitem{ref45} Thurner, L.; Scheidler, A.; Schafer, F.; Menke, J.-H.; Dollichon, J.; Meier, F.; Meinecke, S.; Braun, M. pandapower -- An open-source Python tool for convenient modeling, analysis, and optimization of electric power systems. \emph{IEEE Transactions on Power Systems} \textbf{2018}, \emph{33}(6), 6510-6521. \url{https://doi.org/10.1109/TPWRS.2018.2829021}
\end{thebibliography}
