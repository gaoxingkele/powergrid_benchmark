...
\item
  \textbf{A selective worst-\(K\) screening mechanism with quantified code-path economy.} At the default configuration, static accounting of evaluated objective-array rows falls by 65\% (51,200 to 17,920). No front-quality difference is detected against no screening, and periodic re-screening remains unresolved against a fixed generation-1 subset; screening is therefore an exposure-and-call-count mechanism rather than a demonstrated quality or wall-clock mechanism (Section 6.3).
\item
...
\caption{SHIELD-MOEA architecture. Scenario screening is population dependent; the feedback arrow denotes generational updating. The two validation branches use held-out stress scenarios or AC power flow and do not feed into selection.}\label{fig:1}
\end{figure*}
...
Table 3 pools all 8 labels \(\times\) 30 seed indices; Figure 2 shows label-level distributions for the six main methods and the pooled mean-versus-worst-envelope panel. This pooled view weights the reference proxy configuration four times because \texttt{deterministic\_vs\_scenario}, \texttt{der\_uncertainty}, \texttt{scenario\_screening\_efficiency}, and \texttt{pareto\_quality} differ only in optimizer seed streams under the p4 equations. It is therefore descriptive over archived blocks, not an equally weighted average over eight distinct uncertainty regimes.

\begin{table*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Pooled leaderboard across eight archive labels. Stochastic methods have 240 runs; deterministic rules summarize eight unique outputs retained as 240 repeated provenance rows.}\label{tab:3}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{SHIELD-MOEA vs. NSGA-II. Intervals are pointwise, multiplicity-unadjusted 5000-resample bootstrap CIs for the mean HV difference; $r_{rb}$ is rank-biserial correlation. Holm correction covers eight stochastic opponents per experiment and determines the significance statements.}\label{tab:4}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
Experiment & SHIELD & NSGA-II & $\Delta HV$ (95\% CI) & $r_{rb}$ & Holm p \\
\midrule
deterministic\_vs\_scenario & 0.26959 & 0.25444 & 0.01515 [0.01096, 0.01945] & 0.791 & 5.89e-7 \\
der\_uncertainty & 0.27137 & 0.25471 & 0.01665 [0.01340, 0.01988] & 0.971 & 6.56e-10 \\
load\_uncertainty & 0.26211 & 0.24772 & 0.01439 [0.01080, 0.01802] & 0.842 & 1.10e-7 \\
outage\_contingency & 0.26982 & 0.25224 & 0.01757 [0.01335, 0.02176] & 0.862 & 5.05e-8 \\
restoration\_aware\_evaluation & 0.33878 & 0.32708 & 0.01170 [0.00793, 0.01577] & 0.736 & 5.14e-6 \\
scenario\_screening\_efficiency & 0.26703 & 0.25481 & 0.01221 [0.00738, 0.01675] & 0.700 & 1.33e-5 \\
pareto\_quality & 0.26652 & 0.25276 & 0.01375 [0.00882, 0.01868] & 0.676 & 2.89e-5 \\
unseen\_stress\_generalization & 0.24650 & 0.23247 & 0.01403 [0.00761, 0.02033] & 0.571 & 7.47e-4 \\
\bottomrule
\end{tabularx}
\end{table*}

The margin is largest in \texttt{outage\_contingency} (+6.97\%) and in the inactive-factor block labeled \texttt{der\_uncertainty} (+6.54\%), and remains positive in \texttt{unseen\_stress\_generalization} (+6.04\%). The \texttt{der\_uncertainty} margin cannot be attributed to \(\delta\) because that coordinate is unused by p4 objectives; it reflects another optimizer seed stream on the reference proxy. The unseen-stress label tests changed held-out load and outage ranges, not DER-output shift, and does not certify absence of scenario-distribution specialization.
...
\caption{Hypervolume distributions across eight archive labels (30 seeds per box for stochastic methods); deterministic rules are point estimates. The panel text “DER uncertainty” is the preserved archive identifier, not an active-factor claim: $\delta$ is absent from p4 objectives. The bottom panel pairs each method's pooled mean HV (filled) with its sampled worst-envelope HV (open).}\label{fig:2}
\end{figure*}
...
\subsection{Ablation Study: Where the Gain Actually Comes From}\label{ablation-study-where-the-gain-actually-comes-from}
...
\caption{Mean hypervolume (+/- std over 8 archive labels $\times$ 30 seed indices) of SHIELD-MOEA and the four ablations. The plotted legacy label “No resilience objective” means removal of survivability from environmental selection only; screening retains the five-objective scalar. Callouts give the relative difference to the full method and the number of labels (out of 8) in which the difference is Holm-significant.}\label{fig:3}
\end{figure*}
...
\midrule
reference ($B=1.00B_0$, $ & \textbackslash\{\}mathcal S & =16$, $\textbackslash\{\}gamma\_g=1.00$) & 0.27108 & 0.25725 & 0.01383 [0.00963, 0.01809] \\
...
\subsection{AC Power-Flow Validation on Six Distribution Networks}\label{ac-power-flow-validation-on-six-distribution-networks}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{AC validation summary over six networks (108 cases per method).}\label{tab:7}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\caption{Original SimBench-family AC slice. (a) AC-feasible rate over 72 network-by-scenario cases and over stress scenarios only; the dashed line is the no-plan reference. (b) Mean maximum line loading. The cross-family extension is reported in Table 7 and Figure 9.}\label{fig:6}
\end{figure*}
...
\subsection{Quality--Cost and Resilience-Composition Diagnostics}\label{qualitycost-and-resilience-composition-diagnostics}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.94\textwidth]{figures/fig_quality_cost_tradeoff.png}
\caption{Mean hypervolume versus mean runtime over the eight archive labels and 30 seed indices. Marker area represents mean feasible-front size. The proposed method is highlighted for identification, not to imply statistical dominance over the neighboring ablations.}\label{fig:7}
\end{figure*}
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Selected pooled quality--cost readouts from the archived run records.}\label{tab:8}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\subsection{Cross-Family AC Transportability}\label{cross-family-ac-transportability}
...
\caption{AC-feasible rates and mean minimum bus voltage by network family for the no-plan reference, SHIELD-MOEA, and NSGA-II with post-hoc final-population repair. The dashed voltage line marks the 0.95 pu criterion. Each family uses the same three planning-label compositions and six operating scenarios. SHIELD-MOEA improves over no planning in every family but does not outperform this comparator.}\label{fig:9}
\end{figure*}
