    ...
    *,
    ...
    metadata = {
        ...
        "formula": (
            ...
            "+ graph_signal_weight * graph_signal only for propagation_or_response and mitigation"
        ),
        "graph_active_roles": sorted(GRAPH_ACTIVE_ROLES),
        "graph_gate": "role_selective",
...
def main() -> None:
    ...
    metadata = {
        ...
        "method_iteration": {
            "harness_decision": "post-freeze role-selective graph gate revision candidate",
            "protocol": cv_protocol["name"],
            "legacy_condition": "c2ges_full_fixed_legacy",
            "revised_condition": "c2ges_full",
            "revision": "role_selective_graph_gate",
            "graph_active_roles": sorted(GRAPH_ACTIVE_ROLES),
            "cv_protocol_json": str(cv_protocol_path),
            "heldout_predictions_jsonl": str(heldout_predictions_path),
        },
