#!/usr/bin/env python3
"""Freeze and reproduce the MA-SQLGrid R2 offline coordination study.
...
METHODS = ["first_candidate", "validation_only_no_cf", "full_coordination_complete_invariance"]
...
def selection_view_from_gold(path: Path, test_ids: Sequence[str]) -> list[dict[str, str]]:
    records = {row["question_id"]: row for row in load_jsonl(path)}
    view = [{"question_id": qid, "question": records[qid]["question"]} for qid in test_ids]
...
def freeze(config_path: Path, freeze_dir: Path) -> None:
    ...
    splits = json.loads(resolved(config, "splits").read_text(encoding="utf-8"))
    test_ids = list(splits["test"])
    if len(test_ids) != config["question_count"]:
        raise AssertionError("registered question count mismatch")
    selection_view = selection_view_from_gold(resolved(config, "questions_with_gold"), test_ids)
    selection_path = freeze_dir / "selection_inputs.jsonl"
    write_jsonl(selection_path, selection_view)

    input_keys = ["questions_with_gold", "splits", "schema_sql", "qwen_predictions", "granite_predictions", "state_manifest"]
    ...
    state_dir = resolved(config, "state_directory")
    for state in [config["reference_state"], *config["reference_free_states"]]:
        path = (state_dir / f"{state}.sqlite").resolve(strict=True)
...
def run(config_path: Path, freeze_dir: Path, run_dir: Path) -> None:
    ...
    executors = {
        state: SQLiteReadOnlyExecutor(
            resolved(config, "state_directory") / f"{state}.sqlite",
    ...
    for qrow in selection_rows:
        ...
        first = Decision(pools[qid][0].candidate_id, pools[qid][0].sql, "selected", "registered first frozen candidate slot; validation and invariance ignored", ())
        validation_only = adjudicator.decide(pools[qid], validations, counterfactuals)
    ...
    gold_records = {row["question_id"]: row for row in load_jsonl(resolved(config, "questions_with_gold"))}
    gold_executor = SQLiteReadOnlyExecutor(
        resolved(config, "state_directory") / f"{config['reference_state']}.sqlite",
