...
## R2 implementation and independent-audit outcome

| Consolidated IDs | R2 change location | Verification outcome |
|---|---|---|
| R1-C01, C02, C07 | `original_title_rebuild/prospective_from_freeze_offline_study_v3/`; `INDEPENDENT_RELEASE_AUDIT_V3.md`; R2 Methods/Results/Discussion | Implemented and relabeled as deterministic no-generation descriptive re-execution over previously evaluated items. Release-v3 freeze SHA-256 `8b34bc370451173197ad07b460908537836409d6eb49f303caf46d13d53889e6`; mechanical integrity and dual reproduction pass. The independent scientific evidence-class gate fails because the frozen pre-run test read same-item v2 gold-derived outcomes. Fixed order 80/180, validation-only 100/180, and full metamorphic 101/180 remain descriptive. Q039 is only a trace example, not a general counterfactual or multi-agent gain. A genuinely prior-outcome-independent evaluation requires a new untouched resource and new authorization. |
| R1-C02, C03, C08 | `sqlite_readonly_executor.py`, `ma_sqlgrid_agents.py`, and `tests/` | 21 tests pass. URI `mode=ro`, `query_only`, authorizer, extension denial, time/opcode/row/table/column controls, retained traces, and fail-closed complete CF coverage are exercised. Validation-only receives an empty CF mapping. |
| R1-C02 independent re-audit | v1 `V1_INCIDENT_DIAGNOSTIC.md`; v2 audit; v3 `metamorphic_witnesses_v3/` | V1's three same-hash insertion files are retained and excluded. V2 numerical behavior was reproduced but its release provenance gate failed because builder/tests/chronology were not fully bound, so it remains diagnostic. V3 preserves the rules and data while binding the exact builder and tests before rerunning. Its three query-blind operators have distinct hashes: irrelevant relation/rows, harmless indexes/storage rebuild, and nullable schema extension. |
| R1-C05, C08, C09 | R2 Tables `tab:resources`, `tab:chronology`, and `tab:bird` | BIRD is labelled non-grid; all eight cells, correct/500, cluster intervals, calls/item, final omissions, unequal-call interpretation, incident exclusion, and evidence chronology are explicit. |
| R1-C04, C06 | R2 resource table and Limitations | Not falsely closed. GridDB remains development-visible synthetic evidence; RTS-GMLC/SimBench remain 0 human-reviewed and 0 sealed. Qualified expert review is a manual/future blocker. |
| R1-C10, C13--C16 | R2 back matter | Data Availability now states that a manuscript-bound archive exists and the public repository must be synchronized/tagged before submission. Email, exact funder identity/role wording, restricted-release check, and author confirmation remain manual gates. |
| R1-C12, C17 | `figures/fig_ma_sqlgrid_implemented_coordination_r2.{svg,png}` and R2 captions/text | Code-native R2 diagram exposes executor, complete-coverage, adjudication, and gold boundaries. No generated scientific image is used. Title retained; claim strength is bounded to measured mechanisms. |

The first offline implementation (v1) is not silently repaired or deleted. Its
same-hash witness defect is an immutable diagnostic incident. All manuscript
offline-selection release numbers and hashes are sourced only from v3; v1 and v2 are retained diagnostic predecessors.

| ID | Consolidated reviewer comment | Severity | Route | R2 decision and change | Verification / acceptance test | R2 status |
|---|---|---:|---|---|---|---|
| R1-C01 | The title method has no frozen system-level outcome; inherited GridDB/BIRD runs and the diagnostic replay are not five-role generation experiments. | Blocking | New untouched resource + authorization required | Retain v3 only as deterministic descriptive re-execution. For a stronger test, use an evaluation resource whose outcomes have not appeared in development, tests, reports, or prior runs; freeze against development-only fixtures and execute it once under a new authorization. | Prior-outcome independence documented; 180 or more sealed boards; direct raw-gold access only after sealing; independent reproduction; all rows/failures retained. | Blocking gate remains open |
...
| R1-C07 | Role value, fixed weights, tie rules, and coverage thresholds are untested. | Major | Partly feasible; confirmatory test needs untouched resource | Report v3's complete retained sensitivity grid as descriptive only, including rescue/harm, abstention, and reversed-order behavior. Do not assert prior outcome independence. | All sensitivity cells retained; manuscript states same-item outcome exposure and makes no confirmatory rule-value claim. | Descriptive repair complete; scientific gate open |
...
| R1-C09 | Evidence chronology is blurred among frozen execution, post-review reanalysis, retrospective diagnostic, and inherited evidence. | Major | Feasible | Add a study chronology and evidence-lineage table recording data visibility, rule/code binding, prior outcome access, evidence class, and what each study can establish. | Search audit finds no unsupported `preregistered`, `confirmatory`, or prior-outcome-independence wording. | Implemented in R2 |
