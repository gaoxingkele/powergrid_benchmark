"""R3 database-enforced, resource-bounded SQLite execution for MA-SQLGrid.
...
terminates work that exceeds the registered opcode, time, result-row, result-byte,
cell-byte, or output-column budget.  Callers can additionally select an explicit
SQLite function allowlist.  ``allowed_functions=None`` preserves the historical
default policy (all functions except the retained denylist).
...
def _encoded_value_size(value: Any) -> int:
    """Return the UTF-8 size of the value representation exposed to callers."""
            ...
            if len(columns) > self.max_output_columns:
                ...
                for row in cursor:
                    ...
                    if len(rows) >= self.max_rows:
                        ...
                        break
                    cell_sizes = [_encoded_value_size(value) for value in row]
                    if any(size > self.max_cell_bytes for size in cell_sizes):
                    ...
                    if result_bytes + added_bytes > self.max_result_bytes:
                        failure_kind = "result_byte_limit"
                        error = f"result exceeds registered byte limit {self.max_result_bytes}"
                        break
