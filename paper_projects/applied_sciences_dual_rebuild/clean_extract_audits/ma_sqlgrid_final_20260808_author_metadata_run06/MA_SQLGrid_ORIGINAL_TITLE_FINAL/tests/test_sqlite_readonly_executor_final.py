...
from sqlite_readonly_executor_final import (
    SQLiteReadOnlyExecutor,
    _budgeted_value_size,
    _raw_value_size,
)
...
class SQLiteReadOnlyExecutorTests(unittest.TestCase):
    ...
    def test_blob_cell_limits_charge_original_payload_before_hashing(self):
        for blob_bytes in (1_000_000, 5_000_000, 50_000_000):
            with self.subTest(blob_bytes=blob_bytes):
                result = self.executor(
                    max_cell_bytes=1_024,
                    max_result_bytes=1_024,
                    allowed_functions=["zeroblob"],
                )(f"SELECT zeroblob({blob_bytes})")
                self.assertFalse(result["executable"])
                self.assertEqual(result["failure_kind"], "cell_byte_limit")
                self.assertEqual(result["rows"], ())
                self.assertEqual(result["trace"]["row_count"], 0)
                self.assertEqual(result["trace"]["largest_cell_bytes"], blob_bytes)

    def test_blob_total_budget_counts_raw_payload_plus_structure(self):
        result = self.executor(
            max_cell_bytes=2_000_000,
            max_result_bytes=1_024,
            allowed_functions=["zeroblob"],
        )("SELECT zeroblob(1000000)")
        self.assertFalse(result["executable"])
        self.assertEqual(result["failure_kind"], "result_byte_limit")
        self.assertGreater(result["trace"]["result_bytes_accounted"], 1_000_000)
        self.assertEqual(result["rows"], ())

    def test_blob_budget_boundary_and_memoryview_accounting(self):
        payload = memoryview(b"x" * 1_024)
        self.assertEqual(_raw_value_size(payload), 1_024)
        self.assertGreater(_budgeted_value_size(payload), 1_024)
        accepted = self.executor(
            max_cell_bytes=1_024,
            max_result_bytes=4_096,
            allowed_functions=["zeroblob"],
        )("SELECT zeroblob(1024)")
        self.assertTrue(accepted["executable"])
        rejected = self.executor(
            max_cell_bytes=1_023,
            max_result_bytes=4_096,
            allowed_functions=["zeroblob"],
        )("SELECT zeroblob(1024)")
        self.assertEqual(rejected["failure_kind"], "cell_byte_limit")

    def test_blob_failures_are_retained_in_trace(self):
        executor = self.executor(
            max_cell_bytes=1_024,
            max_result_bytes=2_048,
            allowed_functions=["zeroblob"],
        )
        result = executor("SELECT zeroblob(5000000)")
        ledger = [json.loads(line) for line in self.trace_path.read_text(encoding="utf-8").splitlines()]
        self.assertEqual(len(ledger), 1)
        self.assertEqual(ledger[0]["failure_kind"], "cell_byte_limit")
        self.assertEqual(ledger[0]["largest_cell_bytes"], 5_000_000)
        self.assertEqual(ledger[0]["sql_sha256"], result["trace"]["sql_sha256"])
