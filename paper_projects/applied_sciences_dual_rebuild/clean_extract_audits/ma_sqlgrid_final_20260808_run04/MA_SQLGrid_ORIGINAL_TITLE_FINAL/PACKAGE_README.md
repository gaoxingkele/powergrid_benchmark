...
manual condition for each class. `CLEAN_EXTRACTION_AUDIT.json` records a fresh
extraction, hash, compile, test, and prohibited-content check. The final PDF is
...
Before submission, the authors must provide Yang Yong's email, confirm the
funding agency and sponsor role for grant 521300250006, approve authors/order/
affiliations/declarations, confirm exact GenAI product/model/version metadata,
resolve every third-party permission, synchronize and tag the public repository,
and decide whether to complete the untouched end-to-end and qualified expert
studies required for the retained title's stronger scientific interpretation.
