...
TARGET = ROOT / "packages" / "clean_unpack_validation_01"
...
def main() -> None:
    ...
    if digest(rebuilt_pdf) != original_pdf_hash:
        failures.append({"rebuilt_pdf_hash_mismatch": {"expected": original_pdf_hash, "observed": digest(rebuilt_pdf)}})
    ...
    receipt = {
        ...
        "clean_target": TARGET.relative_to(ROOT).as_posix(),
        ...
        "pdf_hash_reproduced": not any("rebuilt_pdf_hash_mismatch" in item for item in failures if isinstance(item, dict)),
        "final_log_counts": log_counts,
        "forbidden_files_after_reproduction": forbidden,
        "failures": failures,
    }
    OUT.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({key: receipt[key] for key in ("status", "initial_exact_set_verified_files", "all_four_figure_hashes_reproduced", "citation_audit_hash_reproduced", "pdf_hash_reproduced", "final_log_counts", "failures")}, indent=2))
