...
\Author{Zhang Linyao, Zheng Jieyun, Zhang Zhanghuang, Ni Shiyuan, Wu Guilian}
\AuthorNames{Zhang Linyao, Zheng Jieyun, Zhang Zhanghuang, Ni Shiyuan, Wu Guilian}
...
\abstract{Distribution utilities allocate limited budgets among reinforcement, distributed energy resources, storage, and automation. This optimizer study represents those choices with a SimBench-derived portfolio proxy rather than action-aligned expansion decisions. CARS-MODE integrates binary multi-objective differential evolution, adaptive parameters and mutation strategies, budget repair, and crowding preservation. A 2940-run evaluation covers six distinct configurations and an independent base-configuration seed replication. With equal configuration weight, CARS-MODE's sampled-bound/clipped hypervolume is 0.04240, 6.06\% above NSGA-II with returned-population repair. This ordering is not stable across diagnostics: under unclipped analytic envelopes and reference 1.05, the two methods score 0.00043464 and 0.00043530, respectively, while common-reference IGD+ ranks CARS-MODE fifth. FixedDE remains nominally ahead across the evaluated summaries, leaving the combined adaptation bundle unresolved. Repair and diversity retain substantial effects under the archived metric. An illustrative composition-level AC check also shows that proxy rank does not determine electrical feasibility. The results establish a reproducible constrained-search and metric-sensitivity workflow for screening proxy fronts before action-aligned physical evaluation.}
...
Evolutionary multi-objective algorithms are widely used in this literature, but two practical frictions persist. The first is algorithmic: differential evolution (DE) is sensitive to its control parameters and mutation strategy \cite{ref1}, and the planning problem is binary, budget-constrained, and rugged. A single fixed DE configuration can therefore stall. Self-adaptive DE variants (jDE \cite{ref2}, SaDE \cite{ref3}) addressed this limitation on continuous benchmarks two decades ago, whereas many energy-planning studies still use either fixed-parameter DE or an off-the-shelf NSGA-II. The second friction is evidential: studies based on private networks or unchecked objective proxies are difficult to reproduce and easy to over-interpret. Comparative optimizer research emphasizes consistent evaluation protocols \cite{ref4}. In this study, repeated-run statistical comparisons are therefore paired with an explicit power-flow check of the surrogate planning objectives.
...
The evaluation addresses two related questions. First, does the complete constrained-search framework remain competitive across planning configurations and direct multi-objective DE controls? Second, does its apparent ordering persist when sampled normalization is replaced by analytic envelopes, a tighter reference point, common-reference IGD+, and a physical-screening layer? SimBench supplies the public benchmark \cite{ref5}, and the stochastic comparisons use 30 seeded runs with multiplicity-controlled inference. The physical layer maps selected portfolio compositions onto SimBench MV networks for an illustrative pandapower AC check \cite{ref7}.

The planning objectives are engineering indices computed from SimBench subnet statistics. They enable repeated optimizer comparisons but remain portfolio proxies. The AC stage evaluates mapped action compositions rather than the optimizer's actions at their original nodes, so it diagnoses proxy--physics disagreement without constituting a seed-replicated nodal expansion study.
...
\tightlist
\item
  \textbf{An integrated constrained multi-objective DE framework.} CARS-MODE combines jDE-style parameter control, a success-driven two-strategy pool, deterministic budget repair, and crowding preservation. Individual repair and diversity controls and a joint fixed-parameter/fixed-strategy control separate framework-level performance from component attribution.
\item
  \textbf{A reproducible configuration and diagnostic evaluation.} Six SimBench-derived planning configurations, an independent base seed replication, 30 seeds per stochastic method and seed block, direct multi-objective DE controls, analytic normalization, common-reference IGD+, and an illustrative AC layer test whether apparent optimizer gains persist across evaluation choices.
\item
  \textbf{An empirical account of metric-sensitive ordering and bounded mechanism effects.} CARS-MODE is favored by the archived sampled-bound/clipped indicator, but analytic hypervolume and common-reference IGD+ alter the leading order. Repair and diversity remain consequential under the archived indicator, whereas the combined adaptation bundle is unresolved and proxy rank does not determine the illustrative electrical ranking.
\end{enumerate}
...
Active distribution network planning has moved from single-objective conductor sizing to portfolio decisions that co-optimize reinforcement, DER siting, storage, and flexibility. Recent surveys map this shift: Prenc \cite{ref8} catalogues the optimization principles applied in planning and operation of active distribution networks, and Salda\~{n}a-Gonz\'{a}lez et al.~\cite{ref9} organize the elements of modern planning models, including the entry of generative-AI scenario tools. On the modeling side, robustness to uncertainty dominates: Liu et al.~\cite{ref10} plan expansions under distributional uncertainty via Wasserstein-distance ambiguity sets and dual relaxation, and Wang et al.~\cite{ref11} coordinate source--network--storage expansion against WGAN-GP-generated scenarios. Reliability-driven formulations add restoration and islanding to reinforcement choices \cite{ref12}, while Ferreira et al.~\cite{ref13} extend the planning boundary upward to the transmission--distribution interface, and Chen et al.~\cite{ref14} specialize it to hybrid AC/DC campus networks for data centers. Closest to our task, He et al.~\cite{ref15} co-optimize distribution networks with storage and electric vehicles using an improved NSGA-II, and Alrashidi et al.~\cite{ref16} integrate DG, capacitor banks, and EV charging stations in radial feeders through a classification-based global optimization scheme.
...
The broader planning literature in energy venues provides many metaheuristic variants. Qi et al.~\cite{ref17} apply enhanced beluga whale optimization to vulnerability-driven storage planning; Demirbas et al.~\cite{ref18} develop an enhanced coati algorithm for static and dynamic transmission expansion; and Cadena-Albuja et al.~\cite{ref19} compare DE with other optimizers for energy-limited economic dispatch. Two limitations visible in this comparison set motivate our protocol. First, hybrid or ``improved'' algorithms are not always accompanied by mechanism controls, making the source of added value difficult to identify. We therefore isolate repair and diversity and test the parameter-and-strategy controller against a combined fixed control. Second, independent runs, rank-based tests, and standard indicators such as hypervolume \cite{ref20} are applied unevenly across the cited applications, while benchmarking audits document the associated reproducibility risks \cite{ref4,ref21,ref22}. We use a standard indicator, 30 seeded runs, and Holm-corrected non-parametric tests so that single-digit percentage differences can be distinguished from seed variation.
...
DE \cite{ref1} owes much of its practical success to parameter adaptation and constraint handling. jDE \cite{ref2} encodes \(F\) and \(CR\) in each individual and resamples them with a small probability per generation. SaDE \cite{ref3} additionally learns mutation-strategy probabilities from recent successes; Das and Suganthan \cite{ref23} review this lineage. Success-history adaptation (SHADE) \cite{ref24} and composite strategy--parameter schemes \cite{ref25} extend the same principle, while Ahmad et al.~\cite{ref26} survey recent developments. Multi-objective DE variants place these mechanisms under Pareto selection, commonly using non-dominated sorting and crowding \cite{ref27} or decomposition \cite{ref28}. Constraint-handling alternatives include penalties, feasibility-first domination, and repair \cite{ref29,ref30}. Repair is especially attractive for knapsack-like budgets because it returns evaluations to the fundable region.
...
Storage-excluded pool (DER-focused) & der\_siting\_sizing & 1.00 & 1.0 & 54 & storage candidates excluded before all methods run; reinforcement and automation remain & 0.025 & 0.56 \\
DER-excluded pool (storage-focused) & storage\_allocation & 1.00 & 1.0 & 54 & DER candidates excluded before all methods run; reinforcement and automation remain & 0.025 & 0.56 \\
...
The two control arrays \((F_i,CR_i)\) are initialized at \((0.5,0.9)\) and updated together: in every generation, the joint gate redraws \(F_i\sim U(0.1,0.9)\) and \(CR_i\sim U(0,1)\) with probability \(\tau=0.1\) \cite{ref2}. A source-level detail limits the usual ``per-individual'' interpretation: environmental selection reorders the genomes and phenotypes, but the implementation does not apply the survivor indices to the \(F/CR\) arrays. The controls therefore persist by population slot, not as heritable fields attached to the selected genome. The sensitivity study in Section 6.4 sweeps \(\tau\) and finds a flat response. This controller and the strategy pool share the FixedDE switch, so that control estimates neither feature separately.

\subsection{Success-Driven Two-Strategy Pool}\label{success-driven-two-strategy-pool}

Mutation chooses between DE/rand/1 (a uniformly drawn base vector) and DE/best/1 (a base vector drawn from the current feasible-first non-dominated front), with probabilities proportional to recent success masses \cite{ref3}. A trial is successful if it constraint-dominates its parent through lower violation or, at equal violation, Pareto improvement. Success masses decay by 0.95 per generation and have a floor of 0.2, preventing either strategy from being excluded. The pool combines exploratory rand/1 with the more intensive best/1 and lets observed successes determine their sampling probabilities.
...
Table 4 lists the fourteen methods: CARS-MODE, nine baselines, and four ablations. NSGA-II, NSGA-II with returned-population repair, MOEA/D, GDE3, and NSDE are pymoo implementations \cite{ref6}. GDE3 and NSDE operate in \([0,1]^n\) and decode with \(g_j\geq0.5\); Standard DE and CARS-MODE use the strict \(g_j>0.5\) rule. GA, binary PSO \cite{ref31}, and Standard DE optimize the same equally weighted normalized scalarization with a violation penalty. Weighted Sum is a deterministic greedy point rule. All final plans are evaluated by the method-independent proxy objectives, but their search-time constraint handling differs as disclosed below.
...
CARS-MODE is favored in all six configurations by the archived sampled-bound/clipped hypervolume, but this ordering weakens or reverses under analytic hypervolume and common-reference IGD+. Table 5 makes those configuration-specific changes primary. Every effect column is oriented so that a positive value favors CARS-MODE. The base row pools the primary and replicate 30-run seed blocks for a 60-run descriptive estimate; the two blocks remain separate in the upstream inference table, and cross-configuration summaries give one equal weight to each configuration.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Archived run-index-0 composition diagnostic. Stress-only excludes the base operating case (60 cases). Fractions summarize dependent fixed-case rows; they are not estimates of an optimizer's physical-feasibility probability.}\label{tab:7}
...
\caption{AC-feasible case fraction (all operating cases, upper bar; stress-only, lower bar) per method over 72 dependent pandapower cases on four SimBench MV networks. The dashed line is the No-Plan case fraction (0.50).}\label{fig:4}
\end{figure*}
...
\textbf{Metric choice is part of the empirical result.} CARS-MODE attains higher sampled-bound/clipped hypervolume than the tested external baselines in all six configurations, including NSGA-II with returned-population repair, GDE3, and NSDE. Analytic feasible envelopes and common-reference IGD+ do not preserve that ordering: NSGA-II+Repair is 0.15\% higher at analytic reference 1.05 in the equal-configuration summary, and common-reference IGD+ ranks CARS-MODE fifth. The apparent advantage therefore depends on how the proxy front is normalized and summarized.

\textbf{Framework performance and component attribution lead to different conclusions.} Removing diversity or repair reduces the effective set of feasible trade-off solutions under the archived indicator, whereas jointly fixing the DE parameters and restricting mutation to rand/1 leaves the front summary almost unchanged. FixedDE is nominally higher on all three equal-configuration diagnostics, and the joint control cannot determine whether parameter adaptation and strategy selection have opposing effects. The complete constrained-search pipeline can therefore remain competitive without evidence that its adaptive bundle is the source of that performance.

\textbf{The physical layer diagnoses translation rather than proving superiority.} The archived AC mapping shows that selected compositions can improve some fixed-case diagnostics and worsen others. It also changes the apparent method ordering. Because the panel uses one run-index-0 compromise per method and selected seed block, it cannot quantify optimizer-level physical-feasibility uncertainty. The disagreement nevertheless identifies where proxy optimization must hand off to action-aligned electrical evaluation.

\textbf{Practical use follows from that diagnostic sequence.} A proxy front can screen candidate compositions and expose configuration, normalization, and metric sensitivity before more expensive nodal analysis. It should not be treated as a physical planning certificate. The main contribution is a reproducible workflow for testing whether a favorable optimizer comparison survives alternative indicators and electrical screening.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Limitations}\label{limitations}

Five limitation groups bound the interpretation of the results.

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{The planning task and electrical check operate at different resolutions.} The optimizer selects subnet-level action compositions, whereas the illustrative AC layer maps those compositions to buses using fixed stress heuristics. It does not optimize nodal siting or sizing. The 72 dependent binary cases per method, drawn from one run-index-0 compromise in each of three selected seed blocks, support qualitative transfer patterns rather than inferential method ranking. The high-DER No-Plan pattern is therefore a mapping-rule artifact (Section 6.3), not evidence against planning.
\item
  \textbf{Proxy objectives and deterministic operating cases limit physical interpretation.} The five objectives are analytic indices of SimBench subnet statistics rather than power-flow quantities, and their relation to AC feasibility is imperfect. Each proxy configuration uses one fixed operating point, while the AC layer uses six fixed operating or stress cases. Accordingly, ``planning quality'' denotes proxy hypervolume unless stated otherwise; robustness to sampled load, DER, and outage conditions remains to be evaluated.
\item
  \textbf{The optimizer order is metric-sensitive.} Sampled lower bounds clip 2281 below-zero coordinates even though the 1.10 reference dominates every unclipped point. Analytic envelopes and common-reference IGD+ change the leading external-control order; the empirical IGD+ reference also contains the compared methods and is therefore complementary rather than independent. These diagnostics support a conditional framework comparison, not normalization-invariant optimizer superiority.
\item
  \textbf{External validity is restricted to one proxy benchmark family.} All planning configurations and the internal replication derive from one SimBench family, and candidate costs are synthetic units rather than calibrated investment values. A second family with nodal candidates---for example, IEEE 33/69-bus systems---and calibration against published utility costs are required before drawing engineering-economic or cross-network conclusions.
\item
  \textbf{Comparator and software evidence is configuration-specific.} MOEA/D's failure concerns the implemented penalty-based pymoo configuration, not decomposition methods generally. GDE3 and NSDE provide matched Pareto-based DE controls, but success-history variants remain absent. The exact rerun used the preserved source with pymoo 0.6.2; because the archived configuration did not pin the original version or all Boolean-operator defaults, the comparisons apply to the disclosed implementations rather than entire algorithm families.
\end{enumerate}
...
This study evaluates CARS-MODE as an integrated constrained multi-objective search framework on a SimBench-derived portfolio proxy. With equal configuration weight, its sampled-bound/clipped hypervolume is 0.04240014, 6.06\% above NSGA-II with returned-population repair, and the direct GDE3 and NSDE comparisons also favor CARS-MODE under that archived indicator. The main finding is conditional, however: analytic normalization and common-reference IGD+ change the leading order, with NSGA-II+Repair slightly higher at analytic reference 1.05 and CARS-MODE ranked fifth by common-reference IGD+.

Component evidence separates the complete framework from its adaptive bundle. Repair and diversity remain consequential under the archived indicator, whereas FixedDE is nominally ahead across the equal-configuration diagnostics and the combined parameter-and-strategy effect remains unresolved. The illustrative pandapower mapping further shows that proxy rank does not determine electrical feasibility.

CARS-MODE should therefore be used as a reproducible candidate-generation and diagnostic workflow rather than as evidence of normalization-invariant or physical superiority. Nodal siting, multi-seed AC mapping, monetary calibration, an independent benchmark family, and separate adaptation controls are the next requirements for stronger planning claims.
