...
The study uses a two-level evaluation. The planning objectives are engineering indices computed from SimBench subnet statistics---fast enough to support a rigorous statistical protocol, but proxies nonetheless. At the second level, the run-index-0 compromise-plan composition of each method present in the archived panel is mapped onto four SimBench MV networks and checked with pandapower AC load flow {[}7{]} across six fixed operating cases. GDE3, NSDE, and NSGA-II+Repair are absent from this panel. The resulting disagreement between proxy ranking and electrical feasibility is retained as an illustrative composition finding, without seed-level or hierarchical uncertainty.
...
\item
  \textbf{An audited public mixed-voltage portfolio-proxy protocol.} Seven deterministic planning experiments over at most 72 candidate actions derived from SimBench subnet statistics; 30 seeds per stochastic method and experiment; exact preservation of every returned front in a 2940-run rerun; and method-independent sampled and analytic normalization definitions (Sections 2 and 5). The benchmark does not provide nodal, monetarily calibrated expansion actions.
\item
  \textbf{A reference-sensitivity result that bounds optimizer claims.} The exact rerun reproduces the archived sampled-bound/clipped result (CARS-MODE 0.04218; 6.22\% above NSGA-II+Repair) but identifies 2281 clipped coordinates. Under unclipped analytic bounds and reference 1.05, NSGA-II+Repair is 0.03\% higher and CARS-MODE ranks fourth; common-reference IGD+ ranks it fifth. The evidence therefore rejects normalization-invariant superiority (Sections 6.1, 6.6, and 6.7).
\item
  \textbf{A bounded component and physical diagnostic.} The repair and diversity controls support their declared joint contrasts on the archived metric, whereas the combined parameter-and-strategy adaptation bundle remains unresolved. The archived pandapower panel supplies only a composition-level diagnostic; direct controls absent from that panel receive no electrical claim, and all-seed exported compromises were not AC-evaluated (Sections 6.2--6.3).
\end{enumerate}
...
\subsection{Deterministic Planning Experiments}\label{deterministic-planning-experiments}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{The seven deterministic planning experiments and descriptive targets.}\label{tab:3}
...
\midrule
base\_distribution\_planning & 1.00 & 1.0 & 72 & none & 0.018 & 0.48 \\
der\_siting\_sizing & 1.00 & 1.0 & 54 & storage candidates excluded before all methods run & 0.025 & 0.56 \\
storage\_allocation & 1.00 & 1.0 & 54 & DER candidates excluded before all methods run & 0.025 & 0.56 \\
load\_growth\_expansion & 1.00 & 1.3 & 72 & none & 0.025 & 0.48 \\
pareto\_quality & 1.00 & 1.0 & 72 & none (independent optimizer seed stream) & 0.018 & 0.48 \\
constraint\_repair & 0.82 & 1.0 & 72 & none (tight budget) & 0.018 & 0.48 \\
runtime\_scalability & 1.20 & 1.0 & 72 & none (loose budget) & 0.018 & 0.48 \\
\bottomrule
\end{tabularx}
\end{table*}

Two design notes. First, \texttt{pareto\_quality} intentionally replicates the base configuration under an independent per-method seed stream (seeds are derived by hashing the experiment and method identifiers), so it functions as an internal replication check; its results track the base experiment closely (Section 6.1). Second, the two budget variants (0.82x, 1.20x) probe the repair mechanism and the loose-budget regime respectively; the ``runtime scalability'' name is retained from an earlier benchmark version but used only for its budget role, and we do not make runtime-scaling claims from it.

Terminology is fixed as follows. A \textbf{planning experiment} is one deterministic proxy-problem configuration from Table 3 and contains exactly one objective-evaluation operating point \((\lambda_e,1,0)\) for load, DER, and outage multipliers. A \textbf{seeded run} is one stochastic optimizer repetition on that fixed experiment; the seed does not create a load or outage scenario. An \textbf{AC operating case} in Section 5.4 is one fixed combination of validation network and load/DER/\(N-1\) setting. Those AC cases are deterministic stress tests, not random scenarios or independent optimizer replications. The shared configuration class serializes inherited ranges for load \((0.95,1.25)\), DER \((0.7,1.3)\), and outage \((0,0.25)\), and the shared module defines a 16-scenario constant, but the P3 branch bypasses all four settings and returns the single point above; they generate no P3 scenario samples. \texttt{use\_expected\_loss} is false in every P3 experiment, so the P3 loss objective is \(L_e\), not an outage-weighted expected-loss variant. The values \(t_H\) and \(t_D\) are implementation-recorded descriptive targets used only to export shortfalls. The available evidence does not supply a utility-calibration rationale for 0.018/0.025, 0.48/0.56, or the 55\% renewable-gap constant; they are disclosed benchmark design choices and do not affect feasibility, hypervolume, or compromise selection.
...
Each stochastic method runs \textbf{30 independently derived seeds} per planning experiment. For zero-based run index \(h\), the implementation computes the SHA-1 digest of \texttt{p3\textbar{}experiment\textbar{}method} and sets
...
We compare CARS-MODE with each stochastic opponent using two-sided \textbf{Mann--Whitney U tests} per experiment (\(n = 30\) per group) and apply \textbf{Holm correction} over the twelve eligible opponents within each experiment {[}32{]} at \(\alpha = 0.05\). Rank-biserial correlation and 5000-resample bootstrap intervals quantify effect size and mean-difference uncertainty. Weighted Sum is reported as an \(n=1\) point comparison without a seed-level p-value.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Pooled leaderboard (210 runs per stochastic method). Weighted Sum is deterministic; its repeated archive rows provide rectangular provenance and do not constitute seeded inference.}\label{tab:5}
...
\textbf{CARS-MODE} & \textbf{proposed} & \textbf{0.04218} & \textbf{0.00381} & \textbf{38.6} & \textbf{0.119} \\
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{CARS-MODE vs. NSGA-II per experiment. The interval is a 5000-resample bootstrap CI for the mean HV difference; $r_{rb}$ is rank-biserial correlation. Holm correction covers twelve stochastic opponents per experiment.}\label{tab:6}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
Experiment & CARS-MODE & NSGA-II & $\Delta HV$ (95\% CI) & $r_{rb}$ & Holm p \\
\midrule
base\_distribution\_planning & 0.04108 & 0.03813 & 0.00295 [0.00205, 0.00385] & 0.789 & 6.39e-7 \\
constraint\_repair (0.82x budget) & 0.03925 & 0.03573 & 0.00352 [0.00272, 0.00437] & 0.920 & 3.90e-9 \\
der\_siting\_sizing & 0.04496 & 0.04308 & 0.00188 [0.00109, 0.00265] & 0.600 & 3.38e-4 \\
load\_growth\_expansion & 0.03700 & 0.03495 & 0.00204 [0.00120, 0.00300] & 0.691 & 1.33e-5 \\
pareto\_quality (same benchmark family) & 0.04062 & 0.03812 & 0.00250 [0.00183, 0.00318] & 0.802 & 4.92e-7 \\
runtime\_scalability (1.20x budget) & 0.04873 & 0.04481 & 0.00392 [0.00321, 0.00463] & 0.962 & 8.07e-10 \\
storage\_allocation & 0.04362 & 0.04282 & 0.00080 [0.00024, 0.00136] & 0.398 & 0.0416 \\
\bottomrule
\end{tabularx}
\end{table*}

The margin is significant in all seven planning experiments, largest under the 0.82x budget (+9.84\%) and smallest on the storage-only pool (+1.87\%). The pareto\_quality seeded repeat within the same benchmark family reproduces a positive margin (+6.55\% versus +7.75\% in the base setting); it is not an independent-dataset replication. CARS-MODE uses about 1.5x NSGA-II's runtime (0.119 s versus 0.077 s per run) and returns slightly smaller feasible fronts (38.6 versus 40.0 plans on average).
...
\caption{Mean hypervolume (+/- std) of CARS-MODE and the four ablations, pooled over all experiments and seeds. FixedDE is nominally higher and unresolved at the proxy level; under the fixed composition mapping, its descriptive AC-feasible rate is lower (Figure 4).}\label{fig:3}
\end{figure*}

Two components carry the proxy-level result. Removing budget repair costs 6.84\% of pooled hypervolume. It is significant in 6/7 experiments under the primary within-experiment opponent families and in 7/7 under the supplementary repair-specific Holm family across experiments. Removing crowding diversity costs 33.56\% and is significant in 7/7 under both families, while front sizes fall from 38.6 to 8.1 and run-to-run variation increases. The FixedDE contrast is unresolved in 7/7 experiments under both families. The two family definitions and the complete effect table are retained in \texttt{real\_simbench\_planning\_inference\_v2.csv} and the cross-experiment audit.

\textbf{The combined adaptation bundle does not improve the proxy indicator.} Ablation-FixedDE fixes \(F = 0.5\) and \(CR = 0.9\) and replaces the two-strategy pool with rand/1. It attains a pooled mean of 0.04243, \textbf{0.60\% above the full method}. The difference is not Holm-significant in any of the seven experiments (adjusted p = 0.22--0.76, all nominally favoring FixedDE). Because this control changes parameter adaptation and strategy selection together, it does not identify their individual effects. Ablation-NoDER is similarly close (+0.12\% pooled): the full method loses significantly in the tight-budget and DER-siting experiments and wins significantly in the loose-budget experiment. NoDER changes the candidate pool rather than an algorithmic component and is not used for component attribution. The FixedDE result does not support retaining the adaptive bundle for proxy accuracy; Section 6.3 treats the AC observations only as exploratory evidence for a possible secondary role.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{AC validation summary. Stress-only excludes the base operating case (60 cases).}\label{tab:7}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X@{}}
\toprule
...
\footnotesize
\setlength{\tabcolsep}{3pt}
\caption{Exploratory parameter sensitivity on the base experiment (10 seeds per point; nominal, multiplicity-unadjusted two-sided Mann--Whitney p-values versus the matched NSGA-II reference). The "$N_p$ = 40" and "$\tau$ = 0.1" rows are independent reruns of the same default configuration on different seed streams.}\label{tab:8}
...
\caption{Quality--cost diagnostic over the archived 2940-run records. Coordinates are pooled means across seven experiments; marker area represents mean feasible-front size. Pooled points are descriptive because the formal comparisons remain experiment-specific and Holm adjusted.}\label{fig:7}
\end{figure*}
...
\caption{Median and 95th-percentile maximum line loading over the 72 pandapower validation cases per method. The dashed line marks the 100\% thermal criterion used in the AC-feasibility definition. Colors identify CARS-MODE, the no-plan reference, and the remaining methods; the plotted aggregates are available in \texttt{derived\_tables/p3\_ac\_margin\_diagnostics.csv}.}\label{fig:8}
\end{figure*}
...
\caption{Mean hypervolume over 30 seeds for CARS-MODE, GDE3, and NSDE in each planning experiment. All fourteen CARS-MODE-versus-control tests are significant after Holm correction within experiment. These comparisons strengthen the framework-level result but do not rescue a component-level adaptation claim, because the combined FixedDE control remains statistically inseparable from the full method.}\label{fig:9}
\end{figure*}
...
\textbf{What the two-level evaluation supports.} CARS-MODE attains higher proxy hypervolume than every external baseline tested, including NSGA-II with returned-population repair, GDE3, and NSDE. The FixedDE ablation is nominally higher and statistically unresolved, so the evidence does not attribute the gain to adaptation. The AC layer shows that selected plans can improve several network diagnostics while also demonstrating that proxy rank does not determine electrical rank. The 6.22\% proxy advantage over the strongest implemented baseline followed by a mid-pack AC rank is the discrepancy that a proxy-only study would miss.

\textbf{Why the adaptive controller remains exploratory.} FixedDE is nominally 0.60\% higher at the proxy level, and the contrast remains unresolved in all seven planning experiments, so the primary evidence does not show a gain from jointly adapting parameters and strategies. The current control cannot determine whether one adaptive subcomponent helps while the other hurts. The AC inspection suggests a possible secondary role for the bundle: FixedDE selects one additional storage action in the base experiment (8 versus 7), and the mapped plan coincides with greater over-voltage in the rural network and less reinforcement in urban growth cases. This pattern is insufficient for a causal claim but motivates future electrically integrated search. More generally, proxy and external validation can assign different apparent value to a mechanism group; the corresponding contrast for budget repair (-6.84\% proxy, AC-neutral in aggregate) reinforces the need to report both levels.

\textbf{Why the complete framework can outperform controls without a resolved adaptation-bundle effect.} Removing diversity or repair reduces the effective set of feasible trade-off solutions, whereas jointly fixing the DE parameters and restricting mutation to rand/1 leaves that set almost unchanged. The direct GDE3 and NSDE comparisons show that the complete constrained-search pipeline is competitive relative to other multi-objective DE implementations, but the FixedDE result prevents attribution of that margin to either adaptive subcomponent. The framework-level and component-level conclusions are therefore distinct: the implemented combination performs well on the proxy benchmark, while repair and diversity---not adaptive control---are the mechanisms resolved by the ablations.

\textbf{Practical reading for planners.} All runtimes are sub-second at the tested scale. Front-returning methods provide a trade-off set and dominate the scalarized controls on proxy hypervolume, but the AC composition check does not preserve that ordering; Standard DE has the highest mapped AC-feasible rate. CARS-MODE has its largest proxy margin under the tight budget (9.84\%), yet its portfolios are not electrically dominant. The proxy front should therefore serve as a candidate-generation stage whose survivors undergo power-flow checks.
...
\tightlist
\item
  \textbf{Composition-level, not nodal, planning.} The optimizer selects subnet-level actions, and the AC validation maps plan \emph{compositions} onto concrete networks with fixed rules that choose buses by stress heuristics. Node-level siting and sizing---where method differentiation at the electrical layer would have to be demonstrated---is not performed, and the AC stage's per-method sample (72 dependent binary cases from one run-index-0 compromise in each of three planning experiments) supports qualitative patterns, not significance tests. Consequently, the high-DER case's No-Plan advantage is a mapping-rule artifact (Section 6.3) rather than a general electrical finding.
\item
...
\item
  \textbf{Single benchmark family.} All seven planning experiments derive from one SimBench network family. Standard distribution test systems (IEEE 33/69-bus) with nodal candidates are the natural second family and remain future work.
\item
  \textbf{Deterministic operating conditions.} Each planning experiment uses one fixed proxy operating point, and the AC layer uses six fixed operating/stress cases. Only optimizer initialization and variation are stochastic. A planning stage with sampled load/DER/outage scenarios would strengthen the robustness reading.
\item
...
\item
  \textbf{The direct DE comparison is still bounded.} GDE3 and NSDE now provide Pareto-based multi-objective DE controls under the same population and generation budget, but JADE-, SHADE-, and L-SHADE-derived multi-objective implementations remain absent. The evidence supports CARS-MODE against the nine implemented baselines, not the entire adaptive-DE family.
\end{enumerate}
