...
# CARS-MODE: Constraint-Aware Repair and Strategy-Pool Multi-Objective Differential Evolution for Distribution Network Expansion Planning
...
1. **A constraint-aware multi-objective DE framework with testable mechanism groups.** CARS-MODE integrates jDE parameter control, a success-driven two-strategy pool, deterministic budget repair, and crowding diversity. Repair and diversity are removed individually; a combined control replaces adaptive parameters and the strategy pool with fixed $F/CR$ and rand/1. The design therefore separates the framework-level result from repair and diversity effects while testing parameter-and-strategy adaptation as a bundle (Section 4).
