...
The study uses a two-level evaluation. The planning objectives are engineering indices computed from SimBench subnet statistics—fast enough to support a rigorous statistical protocol, but proxies nonetheless. At the second level, the run-index-0 compromise-plan composition of every method is mapped onto four real SimBench MV networks and checked with pandapower AC load flow [7] across six fixed operating cases (base, peak load, load growth, extreme growth, high DER infeed, and an $N-1$ contingency). The resulting disagreement between proxy ranking and electrical feasibility is treated as a descriptive finding.
...
2. **A reproducible public mixed-voltage portfolio-proxy benchmark with a method-independent evaluation protocol.** Seven deterministic planning experiments (candidate-pool, load-factor, and budget variants) over at most 72 candidate actions derived from SimBench subnet statistics; standard hypervolume under fixed seeded normalization bounds; 30 seeds per stochastic method and experiment (Sections 2 and 5). The benchmark does not provide nodal, monetarily calibrated expansion actions.
3. **A statistically grounded proxy-front result against direct controls.** CARS-MODE attains pooled mean hypervolume 0.04218, 6.22% above NSGA-II with returned-population repair (0.03971), and records 55 significant wins among 56 stochastic-baseline comparisons. Seven comparisons with the deterministic Weighted Sum rule are descriptive and favor CARS-MODE in mean HV. Its margins over GDE3 and NSDE are 8.57% and 8.52%, with significant separation in every planning experiment; these are hypervolume results on the proxy, not evidence of superior expansion actions (Sections 6.1 and 6.6).
...
Only the hypervolume input is clipped to $[0,1]^5$; raw objectives, scalar-search scores, and compromise scores are not. Hypervolume is the exact volume dominated by the clipped feasible non-dominated points and bounded by $r$, and an empty feasible front scores zero. The metric contains no method-aware term.
...
**Table 7.** AC validation summary. Stress-only excludes the base operating case (60 cases).
...
GDE3 and NSDE close the most consequential gap in the original baseline set because both retain differential-evolution variation and Pareto-based environmental selection. Their pooled hypervolumes are 0.03885 and 0.03887, respectively, compared with 0.04218 for CARS-MODE. The corresponding relative margins are 8.57% over GDE3 and 8.52% over NSDE. CARS-MODE has a higher mean and a Holm-significant difference against both direct controls in every one of the seven planning experiments (14 of 14 comparisons). Figure 9 reports the experiment-level means without collapsing the heterogeneous experiment effects into a single rank.

![Figure 9. Direct multi-objective DE controls across the seven planning experiments.](figures/fig_direct_de_controls.png)

**Figure 9.** Mean hypervolume over 30 seeds for CARS-MODE, GDE3, and NSDE in each planning experiment. All fourteen CARS-MODE-versus-control tests are significant after Holm correction within experiment. These comparisons strengthen the framework-level result but do not rescue a component-level adaptation claim, because the combined FixedDE control remains statistically inseparable from the full method.

---

## 7. Discussion

**What the two-level evaluation supports.** CARS-MODE attains higher proxy hypervolume than every external baseline tested, including NSGA-II with returned-population repair, GDE3, and NSDE. The FixedDE ablation is nominally higher and statistically unresolved, so the evidence does not attribute the gain to adaptation. The AC layer shows that selected plans can improve several network diagnostics while also demonstrating that proxy rank does not determine electrical rank. The 6.22% proxy advantage over the strongest implemented baseline followed by a mid-pack AC rank is the discrepancy that a proxy-only study would miss.

**Why the adaptive controller remains exploratory.** FixedDE is nominally 0.60% higher at the proxy level, and the contrast remains unresolved in all seven planning experiments, so the primary evidence does not show a gain from jointly adapting parameters and strategies. The current control cannot determine whether one adaptive subcomponent helps while the other hurts. The AC inspection suggests a possible secondary role for the bundle: FixedDE selects one additional storage action in the base experiment (8 versus 7), and the mapped plan coincides with greater over-voltage in the rural network and less reinforcement in urban growth cases. This pattern is insufficient for a causal claim but motivates future electrically integrated search. More generally, proxy and external validation can assign different apparent value to a mechanism group; the corresponding contrast for budget repair (−6.84% proxy, AC-neutral in aggregate) reinforces the need to report both levels.

**Why the complete framework can outperform controls without a resolved adaptation-bundle effect.** Removing diversity or repair reduces the effective set of feasible trade-off solutions, whereas jointly fixing the DE parameters and restricting mutation to rand/1 leaves that set almost unchanged. The direct GDE3 and NSDE comparisons show that the complete constrained-search pipeline is competitive relative to other multi-objective DE implementations, but the FixedDE result prevents attribution of that margin to either adaptive subcomponent. The framework-level and component-level conclusions are therefore distinct: the implemented combination performs well on the proxy benchmark, while repair and diversity—not adaptive control—are the mechanisms resolved by the ablations.
...
4. **Single benchmark family.** All seven planning experiments derive from one SimBench network family. Standard distribution test systems (IEEE 33/69-bus) with nodal candidates are the natural second family and remain future work.
5. **Deterministic operating conditions.** Each planning experiment uses one fixed proxy operating point, and the AC layer uses six fixed operating/stress cases. Only optimizer initialization and variation are stochastic. A planning stage with sampled load/DER/outage scenarios would strengthen the robustness reading.
6. **Baseline configuration caveats.** MOEA/D's failure here is a failure of the specific penalty-based pymoo configuration on this problem; a tuned decomposition method might be competitive, and the comparison should not be quoted against MOEA/D generally. The available run configuration also does not pin the pymoo version, so library-default Boolean crossover/mutation probabilities for NSGA-II/MOEA/D cannot be reconstructed as certified archival constants.
...
This study evaluates a constraint-aware, strategy-pool multi-objective DE on a SimBench-derived mixed-voltage portfolio proxy for distribution planning. It does not evaluate action-aligned, nodal expansion decisions. On this proxy benchmark, CARS-MODE exceeds NSGA-II with returned-population repair by 6.22% pooled hypervolume across seven deterministic planning experiments and 30 seeds per stochastic method. It has a higher mean in all 56 stochastic-baseline comparisons; 55 are Holm-significant. Seven additional deterministic Weighted Sum comparisons are descriptive and favor CARS-MODE. Direct comparisons with GDE3 and NSDE yield margins of 8.57% and 8.52%, with significant wins in all fourteen experiment-level tests.
...
CARS-MODE is therefore supported as a consistently higher-HV optimizer than the implemented external controls on the seven tested planning experiments, not as a superior action-aligned expansion planner. Its parameter-and-strategy controller is an implemented but empirically unresolved bundle, whereas repair and diversity carry the measured proxy gains. Separate parameter-only and strategy-only controls remain necessary for finer attribution. The disagreement between a 6.22% returned-population-repair baseline proxy advantage and the mid-pack AC rank is the central validity finding: proxy fronts should generate candidates for electrical checking rather than substitute for it. Nodal siting, multi-seed AC mapping, monetary calibration, a second benchmark family, and further multi-objective DE controls remain necessary extensions.
