...
2. **A reproducible public mixed-voltage portfolio-proxy benchmark with an audited evaluation protocol.** Seven deterministic planning experiments over at most 72 SimBench-derived candidate actions; 30 seeds per stochastic method and experiment; all 2940 final fronts preserved in the validation archive; and an exact eight-decimal rerun of the original hypervolume column (Sections 2 and 5). The benchmark does not provide nodal, monetarily calibrated expansion actions.
3. **A reference-sensitivity result that bounds the original optimizer ranking.** Under the archived sampled bounds and clipping, CARS-MODE scores 0.04218, 6.22% above NSGA-II with returned-population repair, and exceeds GDE3 and NSDE by 8.57% and 8.52%. Under unclipped analytic envelopes and reference point 1.05, however, NSGA-II+Repair is nominally 0.03% higher and CARS-MODE ranks fourth; common-reference IGD+ ranks it fifth. The evidence therefore rejects a normalization-invariant superiority claim (Sections 6.1, 6.6, and 6.7).
4. **A bounded component and physical-diagnostic analysis.** The legacy metric and analytic robustness checks retain large pooled deficits when repair or diversity is removed, whereas the combined parameter-and-strategy adaptation bundle remains unresolved. The archived pandapower panel identifies proxy--physics disagreement and matched changes relative to the same No-Plan cases, but it evaluates only three run-index-0 compositions per method and supplies neither optimizer-seed uncertainty nor electrical results for the later direct controls (Sections 6.2–6.3).
...
The P3 objective mapping is also deterministic. Let $P_\Sigma=\sum_sP_s$, $D_\Sigma=\sum_sD_s$, and let $\lambda_e$ be the load factor of planning experiment $e$. The baseline indices and hosting denominator are
...
### 3.3. Deterministic Planning Experiments

Seven experiments exercise the pool along three axes—candidate-pool composition, load factor, and budget tightness—with an identical reporting protocol throughout (Table 3). Both the proxy operating point and the later AC stress cases are deterministic; stochasticity comes only from the optimizer seed for methods that use random variation.

**Table 3.** The seven deterministic planning experiments and descriptive targets.

| Experiment | Budget factor | Load factor $\lambda_e$ | Search dimension $n$ | Pool restriction | $t_H$ | $t_D$ |
|---|---:|---:|---:|---|---:|---:|
| base_distribution_planning | 1.00 | 1.0 | 72 | none | 0.018 | 0.48 |
| der_siting_sizing | 1.00 | 1.0 | 54 | storage candidates excluded before all methods run | 0.025 | 0.56 |
| storage_allocation | 1.00 | 1.0 | 54 | DER candidates excluded before all methods run | 0.025 | 0.56 |
| load_growth_expansion | 1.00 | 1.3 | 72 | none | 0.025 | 0.48 |
| pareto_quality | 1.00 | 1.0 | 72 | none (independent optimizer seed stream) | 0.018 | 0.48 |
| constraint_repair | 0.82 | 1.0 | 72 | none (tight budget) | 0.018 | 0.48 |
| runtime_scalability | 1.20 | 1.0 | 72 | none (loose budget) | 0.018 | 0.48 |
...
The deterministic Weighted Sum rule sorts by the **un-divided** benefit $b_j$ defined in Section 4 (not $q_j$), then scans that order and adds an action only if its cost fits the remaining budget. Its five fixed benefit scales are therefore $(0.12,0.02,5,2,2)$, including resilience and excluding DER support. Equal benefits have no authored secondary criterion; the implementation uses the order returned by NumPy's descending `argsort`. This point rule is feasible by construction and has one effective output per planning experiment.

### 5.2. Protocol

Each stochastic method runs **30 independently derived seeds** per planning experiment. For zero-based run index $h$, the implementation computes the SHA-1 digest of `p3|experiment|method` and sets

$$
\operatorname{seed}(h)=200000+7919h+\big(\operatorname{int}(\text{first six hex digits},16)\bmod4096\big).
$$

Thus method cells do not share a random stream. The archive has $14\times7\times30=2940$ rows: 2730 seeded stochastic runs and 210 repeated invocations of Weighted Sum. Because Weighted Sum is deterministic, those 210 provenance rows reduce to seven effective outputs, one per experiment, and are not treated as independent observations. Seed-level tests therefore cover the twelve stochastic opponents of CARS-MODE; the seven Weighted Sum gaps are descriptive.
...
Table 5 reports the exactly reproduced sampled-bound/clipped HV results over all 7 experiments x 30 seeds; Figure 2 shows per-experiment distributions for the original seven-method comparison set, while Section 6.6 isolates the added direct controls and Section 6.7 reports the reference audit.

**Table 5.** Archived sampled-bound/clipped HV leaderboard (210 runs per stochastic method). Weighted Sum is deterministic; its repeated archive rows provide rectangular provenance and do not constitute seeded inference. Pooled ranks are descriptive.
...
Under the archived sampled-bound/clipped definition, CARS-MODE attains a pooled mean hypervolume of 0.04218 (std 0.00381), 6.22% above NSGA-II+Repair (0.03971, constrained NSGA-II with post-search returned-population repair) and 6.34% above plain NSGA-II (0.03966). Repair alone changes NSGA-II by 0.11% on this metric. Across eight stochastic baselines and seven planning experiments, CARS-MODE has a higher mean in all 56 cells; 55 differences remain significant after within-experiment Holm correction. The only unresolved stochastic-baseline comparison is NSGA-II+Repair in storage_allocation. The deterministic Weighted Sum rule is lower in all seven descriptive comparisons and receives no seed-level p-value. These are legacy-metric statements, not normalization-invariant superiority claims: the analytic-bound and common-reference results in Section 6.7 change the external-control ordering. Ablation-FixedDE also remains nominally ahead of the full method. MOEA/D's penalty-based configuration collapses to the empty plan on this problem; we report that implementation-specific failure without generalizing it to decomposition methods.

![Figure 2. Hypervolume distributions (30 seeds) for the seven main methods across the seven planning experiments.](./figures/fig_hv_boxplot.png)

**Figure 2.** Hypervolume distributions (30 seeds per box) across the seven deterministic planning experiments for CARS-MODE and the original six baselines. The returned-population-repair and direct DE controls are reported numerically in Table 5 and Figure 9.
...
For sampled-bound/clipped HV, the margin is significant in all seven planning experiments, largest under the 0.82x budget (+9.84%) and smallest on the storage-only pool (+1.87%). The pareto_quality seeded repeat within the same benchmark family reproduces a positive legacy-metric margin (+6.55% versus +7.75% in the base setting); it is not an independent-dataset replication. CARS-MODE uses about 1.5x NSGA-II's runtime (0.119 s versus 0.077 s per run) and returns slightly smaller feasible fronts (38.6 versus 40.0 plans on average).
...
**Table 7.** Archived run-index-0 composition diagnostic. Stress-only excludes the base operating case (60 cases). Rows are dependent fixed-case evaluations, not optimizer-seed replications.
...
**Figure 4.** AC-feasible rate (all operating cases, upper bar; stress-only, lower bar) per method over 72 pandapower cases on four SimBench MV networks. The dashed line is the No-Plan reference (0.50).
...
Third, the AC inspection changes the interpretation of the joint-controller control. **Ablation-FixedDE, the nominal proxy winner (+0.60%, n.s.), has an overall AC-feasible rate of 0.569 versus 0.611 for the full method** (stress-only 0.517 versus 0.567). Under stress-only operating cases, Ablation-NoDiversity is lower still (0.483). In the base experiment, FixedDE selects eight storage actions compared with seven for CARS-MODE, and the additional mapped injection coincides with greater rural-network over-voltage. Conversely, NoRepair loses 6.84% proxy hypervolume yet matches NSGA-II's AC rate of 0.667. These observations show that proxy and AC assessments capture different properties. Because the AC stage contains one run-index-0 compromise per method and selected experiment (72 dependent binary outcomes per method), it supports a qualitative hypothesis about the **joint adaptation bundle**, not a statistically powered component claim (Section 8).
...
The proxy indicator and AC-feasibility results differ partly because the algorithms return different compromise compositions. Figure 6 summarizes those compositions across the seven planning experiments. CARS-MODE allocates more actions to storage than the three comparison methods shown, whereas Standard DE allocates more to reinforcement. This visualization supports the composition-based explanation in Section 6.3, but it does not establish nodal electrical causality: placement is imposed later by a common deterministic mapping.

![Figure 6. Mean compromise-plan composition across seven SimBench-derived planning experiments.](./figures/fig_portfolio_composition.png)

**Figure 6.** Mean number of reinforcement, storage, DER, and automation actions in the selected run-index-0 compromise plans across seven planning experiments. Bars are descriptive aggregates from the archived composition table. They identify portfolio differences before the common composition-to-network mapping and should not be interpreted as nodal siting decisions or independent AC-feasibility tests.
...
GDE3 and NSDE close the most consequential gap in the original baseline set because both retain differential-evolution variation and Pareto-based environmental selection. Under sampled-bound/clipped HV, their pooled values are 0.03885 and 0.03887, respectively, compared with 0.04218 for CARS-MODE. The corresponding legacy-metric relative margins are 8.57% over GDE3 and 8.52% over NSDE. CARS-MODE has a higher mean and a Holm-significant difference against both direct controls in every one of the seven planning experiments (14 of 14 comparisons). Figure 9 reports the experiment-level means without collapsing the heterogeneous experiment effects into a single rank. With analytic-bound HV, however, only one of seven comparisons is Holm-significant in CARS-MODE's favor against each direct control; common-reference IGD+ favors CARS-MODE by mean in four of seven experiments against each, again with one significant favorable contrast. The direct-control conclusion is therefore metric-specific.

![Figure 9. Direct multi-objective DE controls across the seven planning experiments.](figures/fig_direct_de_controls.png)

**Figure 9.** Mean sampled-bound/clipped hypervolume over 30 seeds for CARS-MODE, GDE3, and NSDE in each planning experiment. All fourteen CARS-MODE-versus-control tests are significant after Holm correction within experiment for this legacy metric. The analytic-bound and common-reference diagnostics in Section 6.7 are less favorable, and the combined FixedDE control remains statistically inseparable from the full method.
...
**Table 9.** Pooled robustness diagnostics. HV is higher-is-better; IGD+ is lower-is-better. Pooled ranks combine heterogeneous experiments and are descriptive. "Favorable" and "significant favorable" count CARS-MODE versus NSGA-II+Repair across seven experiment-specific seed comparisons; Holm correction covers twelve stochastic opponents within each experiment and metric.

| Metric | CARS-MODE | NSGA-II+Repair | CARS pooled rank | Favorable experiments | Significant favorable | Significant unfavorable |
|---|---:|---:|---:|---:|---:|---:|
| Sampled bounds, clipped HV, $r=1.10$ | 0.04217835 | 0.03970671 | 3 | 7/7 | 6/7 | 0/7 |
| Analytic bounds, HV, $r=1.10$ | 0.00211607 | 0.00213251 | 4 | 3/7 | 1/7 | 1/7 |
| Analytic bounds, HV, $r=1.05$ | 0.00043362 | 0.00043374 | 4 | 3/7 | 1/7 | 1/7 |
| Common-reference IGD+ | 0.02231102 | 0.02133904 | 5 | 1/7 | 1/7 | 0/7 |

The alternative 1.05 reference dominates every analytic-normalized point by at least 0.05. At that reference, NSGA-II+Repair is 0.03% higher than CARS-MODE in pooled mean, and CARS-MODE ranks fourth behind the problem-variant NoDER, FixedDE, and NSGA-II+Repair. Common-reference IGD+ ranks CARS-MODE fifth, behind NoDER, NSGA-II+Repair, plain NSGA-II, and FixedDE. NoDER's first-place rank is not a component result because it changes the feasible decision pool. The empirical IGD+ reference contains the non-dominated union of all tested methods and seeds within each experiment and is therefore complementary, not independent. Collectively, these checks reject a normalization-invariant external-control advantage while retaining the narrower finding that CARS-MODE is competitive within the implemented constrained-search set.

---

## 7. Discussion

**What the two-level evaluation supports.** CARS-MODE attains higher sampled-bound/clipped hypervolume than every external baseline tested, including NSGA-II with returned-population repair, GDE3, and NSDE. That ordering is not invariant: under analytic feasible envelopes, NSGA-II+Repair is 0.03% higher at reference 1.05, and common-reference IGD+ ranks CARS-MODE fifth. The FixedDE ablation is nominally higher under every reported normalization and statistically unresolved in the pairwise analytic comparisons, so the evidence does not attribute a gain to adaptation. The archived AC layer shows that one selected composition can improve several network diagnostics while also demonstrating that proxy rank does not determine electrical rank. It remains illustrative because neither multi-seed AC evaluation nor hierarchical optimizer uncertainty is available.

**Why the adaptive controller remains exploratory.** FixedDE is nominally 0.60% higher on the archived proxy metric and also higher on both analytic-bound HV variants; none of the seven analytic-bound pairwise contrasts establishes a significant CARS-MODE advantage. The current joint control cannot determine whether one adaptive subcomponent helps while the other hurts. The AC inspection notes that FixedDE selects one additional storage action in the base experiment (8 versus 7), and the mapped plan coincides with greater over-voltage in the rural network and less reinforcement in urban growth cases. This pattern is insufficient for a causal claim. More generally, proxy and external diagnostics can assign different apparent value to a mechanism group; the corresponding contrast for budget repair (6.84% on legacy HV, AC-neutral in aggregate) reinforces the need to report both levels.
...
**Practical reading for planners.** All runtimes are sub-second at the tested scale. Front-returning methods provide trade-off sets and rank above the scalarized controls on each reported proxy diagnostic, but the illustrative AC composition check does not preserve that ordering; Standard DE has the highest mapped AC-feasible rate. CARS-MODE has its largest sampled-bound/clipped margin under the tight budget (9.84%), yet its portfolios are not electrically dominant. A proxy front should therefore serve as a candidate-generation stage whose survivors undergo power-flow checks.
...
1. **Composition-level, not nodal, planning.** The optimizer selects subnet-level actions, and the AC validation maps plan *compositions* onto concrete networks with fixed rules that choose buses by stress heuristics. Node-level siting and sizing—where method differentiation at the electrical layer would have to be demonstrated—is not performed, and the AC stage's per-method sample (72 dependent binary cases from one run-index-0 compromise in each of three planning experiments) supports qualitative patterns, not significance tests. Consequently, the high-DER case's No-Plan advantage is a mapping-rule artifact (Section 6.3) rather than a general electrical finding.
...
5. **Single benchmark family.** All seven planning experiments derive from one SimBench network family. Standard distribution test systems (IEEE 33/69-bus) with nodal candidates are the natural second family and remain future work.
6. **Deterministic operating conditions.** Each planning experiment uses one fixed proxy operating point, and the AC layer uses six fixed operating/stress cases. Only optimizer initialization and variation are stochastic. A planning stage with sampled load/DER/outage scenarios would strengthen the robustness reading.
...
This study evaluates a constraint-aware, strategy-pool multi-objective DE on a SimBench-derived mixed-voltage portfolio proxy for distribution planning. It does not evaluate action-aligned, nodal expansion decisions. The exact 2940-run rerun reproduces the archived sampled-bound/clipped hypervolumes: CARS-MODE is 6.22% above NSGA-II with returned-population repair, and the direct GDE3 and NSDE contrasts favor it on that legacy metric. The reference audit finds 2281 below-zero clipped coordinates among 68,248 returned front points, despite strict reference dominance before clipping.

Under unclipped analytic envelopes and reference 1.05, CARS-MODE scores 0.00043362 versus 0.00043374 for NSGA-II+Repair and ranks fourth overall; common-reference IGD+ ranks it fifth. The fixed-parameter, single-strategy control remains ahead, so the combined parameter-and-strategy adaptation bundle is unresolved. The repair and diversity ablations retain their scope as joint contrasts on the archived metric; NoDER remains a different problem rather than a component test.

The archived pandapower layer is retained as an illustrative composition diagnostic: one run-index-0 compromise per method and selected experiment produces dependent fixed-case rows, not hierarchical optimizer uncertainty. CARS-MODE maps to 0.611 overall AC feasibility versus 0.500 for No-Plan and 0.667 for NSGA-II, but this is not electrical superiority evidence. The supported contribution is therefore a reproducible constrained-search and diagnostic workflow with a demonstrated normalization limitation, not consistent optimizer or action-aligned planning superiority. Nodal siting, multi-seed AC mapping, monetary calibration, a second benchmark family, separate adaptation controls, and further multi-objective DE controls remain necessary extensions.
