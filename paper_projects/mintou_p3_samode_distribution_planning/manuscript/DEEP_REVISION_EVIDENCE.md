...
| Self-Adaption Multi-objective Differential Evolution Algorithm | Section 4 and the read-only planning implementation define binary multi-objective DE, a joint adaptive controller, deterministic budget repair, and crowding selection; FixedDE, NoRepair, and NoDiversity are the evaluated controls. The prospective Stage-3 module specifies independent parameter and strategy gates but has no experiment results. | FixedDE disables parameter and strategy adaptation jointly. The evaluated evidence neither identifies either adaptive subcomponent nor supports the causal wording that self-adaptation improves performance. The code split alone is not effect evidence. |
...
| Prospective Stage-3 contract | Four adaptation arms, independent labelled RNG streams, heritable F/CR state, a complete action-registry validator, inclusive voltage/thermal rules, and separate evaluation counters are implemented and unit-tested. No optimizer or AC run is added. |
...
The present Stage-3 action/method contract also runs no optimizer or AC experiment. It adds `scripts/p3_s03_method_contract.py`, its unit tests, a machine-readable contract, and `manuscript/reconstruction_v2/METHOD_IMPLEMENTATION_CONTRACT.md`. The prospective code separates parameter and strategy gates, makes F/CR heritable through survivor selection, isolates their random streams, freezes the `>=0.5` prospective decoder, specifies deterministic repair, and rejects incomplete action registries. Its backward-compatible mode preserves the historical one-switch configuration meaning, not bitwise numerical equivalence after the inheritance and random-stream corrections. No observed effect is assigned to either mechanism.
