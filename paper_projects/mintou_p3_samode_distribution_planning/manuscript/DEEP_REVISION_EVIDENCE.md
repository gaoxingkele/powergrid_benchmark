...
The archived primary estimand is the within-experiment distribution of sampled-bound, clipped hypervolume for each stochastic method. The optimizer seed is the analysis unit (`n = 30` per stochastic method and experiment). Two-sided Mann--Whitney U tests compare CARS-MODE with twelve stochastic opponents, with Holm correction within each planning experiment. Pooled means across seven heterogeneous experiments are descriptive.

This stage adds three robustness estimands evaluated on every preserved rerun front: unclipped analytic-bound hypervolume at reference 1.10, the same hypervolume at alternative reference 1.05, and common-reference IGD+ against the empirical non-dominated union within each experiment. The same seeded-run analysis unit and within-experiment Holm family are retained separately for each diagnostic. The three metric definitions are robustness families; no multiplicity claim is made across them.

Weighted Sum has one effective deterministic output per experiment. Its 30 repeated archive rows per experiment preserve rectangular provenance and are not inferential replicates.

The AC layer addresses a different question. It contains one run-index-0 compromise composition per archived method and each of three planning experiments, deterministically mapped to four MV networks and six fixed cases. Its 72 rows per method share three compositions and are not optimizer replications. This stage retains AC as an illustrative composition diagnostic and does not compute seed-level or hierarchical optimizer uncertainty.

## Comparison Budget and Data Visibility

| Item | Evidence contract |
|---|---|
| Main optimizer archive | 14 methods x 7 experiments x 30 rows = 2940; 2730 seeded stochastic runs and 210 deterministic Weighted Sum provenance rows. |
...
| Common-reference metric | IGD+ against the empirical non-dominated union of all methods and seeds within each experiment; complementary, not independent. |
| Inference | Seed-level Mann--Whitney U, Holm-corrected over twelve stochastic opponents within experiment and metric. |
...
The new immutable evidence directory is `evidence/runs/p3_s3_planning_validation_20260813/`. It contains the run manifest, exact-reproduction comparison, all returned front objectives and their index, all-seed deterministic compromise compositions, sampled and analytic bounds, reference/clipping audit, per-run robustness metrics, pooled descriptive leaderboards, experiment-level inference, per-experiment common reference fronts, the AC scope decision, and the matched No-Plan table. `ANALYSIS.md` and `VALIDATION_REPORT.md` record the Material Passport, warnings, reproducibility verdict, and 11-category statistical-fallacy scan.
...
- **External-control ordering is normalization-sensitive.** Archived sampled-bound/clipped HV gives CARS-MODE 0.04217835 versus 0.03970671 for NSGA-II+Repair. With analytic bounds and reference 1.05, the values are 0.00043362 versus 0.00043374, and CARS-MODE ranks fourth overall. Common-reference IGD+ ranks CARS-MODE fifth (0.02231102 versus 0.02133904; lower is better). A consistent-superiority claim is not supported.
...
- **Adaptation bundle unresolved.** FixedDE is higher than CARS-MODE under the archived metric and both analytic-reference variants. The joint control does not establish a benefit from parameter-and-strategy adaptation and cannot separate its two parts.
...
- **Direct-control evidence weakens under robustness metrics.** Sampled-bound/clipped HV favors CARS-MODE over GDE3 and NSDE in all experiments. Under analytic HV only one of seven contrasts against each is Holm-significant in the favorable direction; common-reference IGD+ favors CARS-MODE by mean in four of seven against each, with one significant favorable contrast.
...
This stage reran all 2940 P3 optimizer rows in a new evidence directory using the declared source-derived seeds. All archived sampled-bound hypervolumes match at eight decimals; the maximum absolute serialized difference is 0. The rerun preserves 68,248 returned front points and exports a deterministic normalized-sum compromise for every optimizer seed.

New deterministic evaluations of those fronts are:
...
- IGD+ against one empirical common reference front per planning experiment; and
- seed-level Mann--Whitney/Holm tables for each robustness metric.

No new AC power-flow case was run. The all-seed compromise compositions were exported for possible future evaluation but are not AC evidence. The only new physical table is a descriptive row-matched reanalysis of the existing AC common panel against No-Plan. No method, parameter, seed, bound, reference, or result was selected to improve CARS-MODE's apparent ranking; the unfavorable and null findings are retained.
