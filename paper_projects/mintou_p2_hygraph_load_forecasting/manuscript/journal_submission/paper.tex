...
Recurrent networks are an established deep-learning line in load forecasting. Building on the LSTM cell \cite{ref9}, Kong et al.~demonstrate short-term forecasting at the residential scale \cite{ref10}. DeepAR extends shared training to large panels of related series \cite{ref11}, an idea reflected in CSA-LoadNet's shared encoder. The cited energy-series comparison evaluates temporal convolutional networks against recurrent alternatives \cite{ref12}, while the Temporal Fusion Transformer \cite{ref13} and Informer \cite{ref14} extend attention-based forecasting.

The cited recent applied-load papers use hybrid models, including LSTM--CNN fusion \cite{ref15}, BiLSTM--Transformer combinations \cite{ref16}, decomposition pipelines \cite{ref17}, and LSTM--XGBoost ensembles \cite{ref18}. In this four-paper sample, component evidence is primarily reported through accuracy comparisons and ablations; it does not establish how common multi-seed, component-level, multiplicity-corrected inference is across the full field.

\subsection{Cross-Series and Graph-Structured Load Forecasting}\label{cross-series-and-graph-structured-load-forecasting}

A second thread exploits correlation among regional or nodal series. LSTNet models dependence through joint convolutional--recurrent channels \cite{ref19}. Graph formulations emerged in traffic forecasting \cite{ref20}, and MTGNN learns adjacency end to end \cite{ref21}. Power-load examples include LoadSeer \cite{ref4}, GCN--BiLSTM--Adaboost \cite{ref1}, spatio-temporal graph transformers \cite{ref2}, and GCN--Transformer integration \cite{ref3}. A parallel literature treats multiple series as a hierarchy requiring reconciliation \cite{ref22,ref23}. The Ausgrid source provides residential customer load and rooftop-PV data that can be aggregated into an evaluation hierarchy \cite{ref24}; the dataset paper itself does not establish that this constructed hierarchy is a standard testbed.

The four cited graph-based load studies use different graph constructions and weighting mechanisms \cite{ref1,ref2,ref3,ref4}. Their reported comparisons do not, as a group, supply the exact matched question studied here: selected weighting versus informative equal averaging under one seeded protocol. A learnable-adjacency appliance study includes fixed-graph controls \cite{ref31}, and a smart-meter STGNN benchmark reports gains at residential but not aggregate level \cite{ref32}; these nearby designs prevent any claim that such controls or null findings are absent from the field. The bounded motivation is therefore to test the specific weighting and aggregation choices under this paper's frozen protocol, not to claim the first such test.

\subsection{Simple Baselines and Honest Evaluation}\label{simple-baselines-and-honest-evaluation}

The third thread concerns evaluation discipline. Sound out-of-sample practice was codified early \cite{ref25}. Cross-validation for time-series predictors requires assumptions and split designs appropriate to temporal dependence; ordinary random-fold practice is not automatically valid \cite{ref26}. Common metrics also have known failure modes; MAPE becomes unstable near zero loads \cite{ref27}, directly motivating the SimBench and Ausgrid choices. Diebold--Mariano testing \cite{ref28} made statistical comparison a standard forecasting question. The M competitions \cite{ref6,ref7}, DLinear \cite{ref5}, and later best-practice guides \cite{ref8} further emphasize strong simple baselines.

The cited evaluation papers primarily compare complete forecasting methods, while the present study applies the same caution to selected forecaster components. Recent benchmark work also emphasizes broader method coverage and consistent pipelines \cite{ref33}, so this targeted map is not evidence of methodological novelty. Section 6.3 applies multiplicity-corrected tests to the aggregation weight forms in the frozen comparison family.
...
The primary metric is chosen per dataset to avoid known metric pathologies \cite{ref27}. For a fixed dataset--horizon test set of \(N\) targets, the implementation computes
...
Forecasting comparisons are sensitive to temporal leakage \cite{ref8,ref26}. The sample origins are not shuffled across the train/test boundary. Per-series z-normalization means and sample standard deviations (\texttt{torch.std} with correction 1, clamped below at \(10^{-6}\)) are computed from raw values at indices \([0,b)\); this pre-test segment includes the later validation portion and is therefore a \textbf{training-segment}, not fit-subset, transformation. The final 15\% of strided training origins by time is used to choose the lowest-validation-loss checkpoint. All test origins are evaluated without stride.
...
For each series \(i\) in the pool, the past 168 processed positions of per-series z-normalized load are passed through a shared multilayer perceptron \(168 \to 96 \to 48\), yielding a temporal encoding \(h_i \in \mathbb{R}^{48}\). Sharing one encoder across all series follows the cross-learning principle established by panel forecasters such as DeepAR \cite{ref11}: every series' history contributes gradient signal to the same temporal features. The evaluated pool sizes are 6, 8, and 17 series.
...
Table 2 lists the implemented capacities. Counts are exact source-level arithmetic over instantiated tensors, including the embedding, scale, and value-map tensors that remain instantiated in TemporalOnly even though its forward pass does not use aggregation. The five external baselines span MLP, LSTM \cite{ref9,ref10}, TCN \cite{ref12}, DLinear \cite{ref5}, and a lightweight two-layer patch transformer. All models consume the same 168-position normalized histories, split boundaries, and full test origins; they do not have identical parameter counts or epoch budgets.
...
The ten-seed fixed split favors CSA-LoadNet over the smaller-head TemporalOnly arm and the selected MLP. In the matched rolling-origin analysis, CSA-LoadNet's mean MAPE is only 0.69\% lower than target-self context when the target-self mean is the denominator, and the Holm-adjusted p is 0.984. No statistically distinguishable difference is detected among learned Poincar\'{e}, uniform, and Euclidean cross-series weighting. The fixed-scale arm is nominally best, including lower WAPE at every origin, but its primary MAPE p becomes 0.0625 after correction. The architecture screen and the single favorable fixed split therefore cannot carry a confirmatory aggregation claim.
...
All datasets used in this study are public: the Open Power System Data 60 min package (https://open-power-system-data.org) \cite{ref29}, SimBench load profiles (https://simbench.de) \cite{ref30}, and Ausgrid solar-home electricity data (https://www.ausgrid.com.au) \cite{ref24}. The supplementary package contains the CSA-LoadNet implementation and configurations; 280 fixed-split non-hierarchical decision-set records; 440 exact-hierarchy model--reconciliation records; and the accepted rolling-origin evidence comprising 240 method--block--seed rows, 6,750 date-aggregated forecast-target audit rows, block-level inference, and a validation report. {[}AUTHOR INPUT REQUIRED: confirm the package-access wording and deposit a persistent public archive before adding a repository URL or DOI.{]} No persistent public archive is verified in the current evidence.
...
\bibitem{ref31} Li, D.; Xia, J.; Li, J.; Xiao, C.; Stankovic, V.; Stankovic, L.; Shi, Q. A Temporal--Spatial Graph Network With a Learnable Adjacency Matrix for Appliance-Level Electricity Consumption Prediction. \emph{IEEE Transactions on Artificial Intelligence} \textbf{2025}, \emph{6}(4), 989--1002. \url{https://doi.org/10.1109/TAI.2024.3507734}
\bibitem{ref32} Nguyen, Q.V.; Delgado Fernandez, J.; Potenciano Menci, S. Spatiotemporal Graph Neural Networks for Short-Term Load Forecasting: Does a Graph Inferred from Smart Meter Data Help? In \emph{Proceedings of IEEE PowerTech 2025}, Kiel, Germany, 29 June--3 July 2025. \url{https://doi.org/10.1109/PowerTech59965.2025.11180479}
\bibitem{ref33} Qiu, X.; Hu, J.; Zhou, L.; Wu, X.; Du, J.; Zhang, B.; Guo, C.; Zhou, A.; Jensen, C.S.; Sheng, Z.; Yang, B. TFB: Towards Comprehensive and Fair Benchmarking of Time Series Forecasting Methods. \emph{Proceedings of the VLDB Endowment} \textbf{2024}, \emph{17}(9), 2363--2377. \url{https://doi.org/10.14778/3665844.3665863}
\end{thebibliography}
