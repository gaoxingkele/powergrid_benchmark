...
| P4-C06 | `PROTOCOL_FROZEN_NO_RESULT` | Stage 4 freezes the Ausgrid accounting hierarchy and the per-split, training-prefix-only OPSD functional-graph rule, including provenance fields and controls. | Raw-source/licence verification and executed leakage audits remain required; neither graph is physical-topology evidence. |
| P4-C07 | `IMPLEMENTED_AND_PROTOCOL_FROZEN_NO_RESULT` | The Stage-3 genuine HGCN/Euclidean GCN implementation contract and Stage-4 matched experiment protocol exist. | No pilot or formal forecast has run, so no HGCN benefit, null, stability, or efficiency result may be reported. |
...
The prospective Stage-4 family is separate from that accepted legacy family. It freezes two disjoint development/formal schedules, five paired formal seeds, and fourteen WAPE contrasts across Ausgrid and OPSD as one Holm family. Its ten-arm matrix contains persistence, DLinear, CSA-LoadNet, matched Euclidean GCN and fixed/learnable-curvature HGCN cells, and identity/random graph controls. The graph-only cells inherit the corresponding real-graph setting and are not separately tuned. No Stage-4 outcome is visible, and no legacy result enters its family.
...
The Stage-3 P4 graph/model implementation contract is executable under `experiments/p2_s4_electronics_hgcn_load_v1/`. It freezes source-bound node and target semantics, explicit hierarchy provenance, training-prefix-only OPSD correlation construction, a matched Euclidean GCN, and a Poincare-ball HGCN with explicit curvature, origin maps, adjacency aggregation, and numerical safeguards. Its verification script uses synthetic tensors only. This is an implementation invariant check, not a pilot, rerun, or forecasting experiment.

Stage 4 now freezes `config.json`, `data_manifest.json`, `environment.json`, `RUNBOOK.md`, and `planned_vs_executed.json` in that namespace. The exact Ausgrid node/parent map is copied from the existing manuscript record rather than inferred; its historical full-record leaf-selection limitation remains disclosed. The protocol separates three earlier development blocks and their seeds from eight later formal blocks and five paired seeds, fixes lead 24, train-prefix normalization, sequence-phase covariates, graph provenance, tuning grids and budgets, the direct graph/geometry ablations, outcomes, failure handling, and the fourteen-comparison Holm family. Its ledger is `NOT_EXECUTED / NO_RESULTS`. Therefore it creates no accuracy, calibration, efficiency, stability, p-value, or method-performance evidence, and the manuscript's no-GCN/HGCN-result boundary remains in force.
...
- **Stage-4 data and execution:** raw Ausgrid files and their hashes are absent; the inherited OPSD hash is recorded but its file is not materialized here; source-specific licence evidence is absent for both. Processed hashes, anchor-to-position maps, the Stage-4 runner, a verified runtime pilot, and formal resource acceptance are also missing. These gates block pilot/tuning/formal execution and cannot be filled by inference.
- **Bibliography and novelty:** all 30 inherited references plus three newly cited nearby/evaluation sources now have an item-level existence, metadata, and sentence-support audit, and a targeted HGCN/STGNN/load-forecasting search is recorded under `manuscript/reconstruction_v2/literature/`. The search is not systematic or exhaustive and therefore does not establish novelty. Similarity screening and external domain-expert review remain `UNVERIFIED`.
- **Current PDF:** an isolated three-pass `pdflatex` build of the canonical journal source succeeded on 2026-09-03, producing a 25-page A4 PDF with the locked title and author metadata and no undefined citations or references. `latexmk` remains unavailable because the local MiKTeX installation cannot find Perl; this is a wrapper dependency, not a source-compilation failure. The checked-in PDFs still predate the accepted source and remain stale, so they are not evidence of current manuscript content and must be replaced and rendered at the final artifact gate.
