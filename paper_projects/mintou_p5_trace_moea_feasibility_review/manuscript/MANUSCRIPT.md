<!-- MDPI Energies submission manuscript. Paper ID: mintou_p5 (TRACE-MOEA).
     Section: Electrical Power and Energy Systems. Quantitative result tables and
...
3. **A reproducible, comparison-bounded evaluation.** Seven review scenarios, 30 seeded runs per stochastic method, component and combined-configuration controls, a matched three-budget scan, a one-output comparison, and prespecified bound/reference and formulation/preference sensitivity checks separate global front quality from compromise selection. The complete method improves pooled proxy hypervolume over NSGA-II and the implemented R-NSGA-II control under the reported metric, whereas the direct preference-ablation effect is unresolved and deterministic-rule ordering is normalization-dependent. NERC and MTEP16 diagnostics remain descriptive external-consistency checks rather than expert-validated review performance.
...
Related work is organized around three unresolved problems: connecting investment screening to physical and economic validation, evaluating preference guidance under hard constraints, and distinguishing run summaries from decision provenance.

### 2.1. Unresolved Problem: From Network Planning to Auditable Project-Queue Selection

Transmission expansion planning treats network topology or asset capacity as decision variables and has been surveyed from both modeling [1] and practice perspectives [2]; a stochastic engineering-economic formulation shows how market and regulatory uncertainty can enter transmission-planning decisions [10]. The present work addresses a different, narrower sub-problem: selecting from a pre-specified list of candidate projects under a hard budget.
...
Recent power-system planning studies add application-specific validation through coordinated transmission-storage planning [29], interruption-cost estimation for reliability investment [30], grid-side storage investment-return analysis [31], and hybrid exact-heuristic treatment of non-analytical investment costs [32]. Loss- and N-1-aware transmission planning further demonstrates that physical constraints can change selected expansion plans [34]. These studies optimize system expansion or asset configuration rather than the proxy project-queue selection problem studied here. Hybrid grid-investment models couple risk and benefit dimensions using MCDA fronts [11], while a recent IEEE-33-bus study uses an improved NSGA-II for investment-cost, reliability, and network-loss objectives but leaves scenario constraints and N-1 security as future work [35]. The unresolved gap is therefore not the absence of power-system optimization; it is whether a large pre-specified project queue can be searched under a hard budget while preserving inspectable records and then survive independent economic and electrical validation. The current study addresses only the proxy-search and run-summary parts of that gap.

### 2.2. Unresolved Problem: Preference Guidance Under Hard Constraints

General-purpose dominance, decomposition, and reference-point families [8,9,12] seek broad Pareto-front coverage, which motivated preference injection into evolutionary multi-objective optimization. Representative approaches include reference-point guidance [13], achievement-scalarizing function methods [14], preference-modified dominance relations [15], and taxonomies of a priori, interactive, and a posteriori preference articulation [16,17]. Constrained-MOEA research separately documents multiple constraint-handling families and warns that feasibility handling and objective search must be evaluated together [28].
...
### 2.3. Unresolved Problem: Run Summaries Are Not Decision Provenance
...
Provenance research supplies taxonomies of why, how, and where records are generated [25], while PROV formalizes entities, activities, agents, and their derivation relations [26]. End-to-end algorithmic-audit work emphasizes artifact collection throughout the system life cycle [27]. These literatures set a higher bar than a count of optimizer events. TRACE-MOEA generates repair-drop and preference-best-response payloads during each run and quarantines their aggregate reporting fields from the evaluation objective. The current release preserves counts and pool-local set-overlap coverage rather than a PROV-compliant, ordered, or replayable project-level record.

### 2.4. Differentiation from the Companion Study

The companion project `mintou_p6_bilonsga_project_review` (BiLo-NSGA; internal companion study, not used as external literature evidence) uses the same public candidate generator and common benchmark, evaluation, and public-record backtest infrastructure but poses a different mechanism question. It modifies variation through insertion, substitution, and dependency-aware local moves under a hard budget. TRACE-MOEA studies selection-stage adaptive preference elitism, deterministic repair, and run-level event co-occurrence summaries in a five-objective review formulation. The projects share infrastructure and source records; their configurations, executions, run outputs, selected portfolios, statistical comparisons, and claims remain paper-specific.
...
A stage-local asset review did not find a source-specific licence record, candidate-to-cost-record mapping, or currency/base-year transformation that would support candidate-level economic calibration. It also found no mapping from the archetype-level portfolio bits to bus, branch, generator, topology, dispatch, or control actions required for an AC/OPF case. Cost calibration and electrical post-validation are therefore NO-GO for the present claim set. The existing NERC and MTEP16 records remain usable only for the descriptive external-consistency checks in Section 6.6; citation of official records does not imply permission to redistribute source PDFs or portal extracts. The complete decision record is `reconstruction_v2/METHOD_DATA_IMPLEMENTATION_CONTRACT.md`.
...
The candidate-generation code of Section 3.2 is a shared artifact used by this paper and by the internal companion project `mintou_p6_bilonsga_project_review` (BiLo-NSGA). The projects also share source corpora, common execution and evaluation utilities, and public-record backtest infrastructure. The five-objective p5 formulation, p5 scenario and method configurations, executions, run outputs, selected portfolios, statistical comparisons, and claims are paper-specific. The Data Availability statement records this boundary.
...
Within each run, the implementation appends two event-record types to an in-memory list: `repair_drop` contains (`gen`, `event`, `item`), and `preference_elite` contains (`gen`, `event`, `pref`, `items`). The `item` and `items` values are zero-based positions in the current experiment pool. Although the candidate object has a stable `cid` field, that field is not written to these records or the released run table. Consequently, the current evidence cannot link an event position across differently filtered pools or establish lineage.
...
The primary pooled proxy result is a mean feasible-front hypervolume of 0.17425 for TRACE-MOEA (standard deviation 0.00635), 0.89% above NSGA-II at 0.17270. This pooled difference is a descriptive scenario-pooled summary, not a pooled hypothesis test. Table 4 reports the full leaderboard across all seven scenarios. Against the four stochastic baselines, TRACE-MOEA has 27 positive scenario-level mean differences in 28 comparisons; 24 are significant after Holm correction across all 12 stochastic opponents within the corresponding scenario, and none is a significant loss. The 21 gaps against three deterministic ranking rules are descriptive unique-output comparisons and favor TRACE-MOEA only under the reported clipped full-front metric. R-NSGA-II provides the direct implemented preference-family control and is retained despite its lower pooled hypervolume.
...
![Figure 4. Run-level event-record diagnostics for the full method and event-bearing ablations.](figures/fig_event_record_diagnostics.png)
...
![Figure 7. Search effort, event-record summaries, and hypervolume for TRACE-MOEA and controls.](figures/fig_search_event_efficiency.png)
...
Ten limitations constrain the scope of the claims in this paper.
...
4. **Run-level event records are implemented, not validated as lineage or review support.** Mean position co-occurrence is 98.6%, and quarantine prevents event statistics from entering objectives, selection, or the reported metric. Payloads use pool-local integer positions and the released table retains only count and set overlap; stable IDs, order, replacement/eviction flags, and population snapshots are absent. The release therefore provides neither project-level lineage nor replay evidence. Whether fuller records improve board decisions, review speed, stakeholder contestability, or regulatory compliance requires a human-subjects study that has not been performed.
...
8. **Library-default baseline state is incompletely recorded.** R-NSGA-II is passed the disclosed raw reference point, `epsilon=0.01`, population 40, and `n_gen=40`, but the pymoo version, Boolean-operator probabilities, internal normalization mode, and per-generation ideal/nadir values were not serialized in the original configuration. The stage runtime could not rerun the recorded comparator fronts under a compatible library ABI, so only preserved comparator rows enter the matched-output analysis and no alternative library version is substituted. Exact survival-state replay cannot be certified.
...
10. **Objective-hiding controls are combined perturbations.** `NoReliabilityFeatures`, `NoRenewableFeatures`, and `NoScheduleRisk` remove one search objective but the preserved implementation truncates the first scenario-derived preference vector rather than remapping it to the surviving coordinates. Their full-front hypervolume contrasts therefore estimate the joint effect of objective hiding and that weight-mapping change. No isolated reliability-, renewable-, or schedule-risk contribution is identified by those runs.
...
On the five-objective, 120-project proxy benchmark, TRACE-MOEA attains a descriptive pooled mean hypervolume of 0.17425, 0.89% above NSGA-II (0.17270). Across four stochastic baselines and seven scenarios, it has 27 positive mean differences in 28 comparisons, 24 wins after Holm correction across 12 stochastic opponents within the corresponding scenario, and no significant loss. Twenty-one deterministic-rule full-front gaps are descriptive and favor TRACE-MOEA under the reported clipped metric; the separate matched one-output comparison is also descriptive and shows competing trade-offs. In the 270-run matched-budget scan, TRACE-MOEA significantly exceeds R-NSGA-II and NSGA-II at all three budgets within each budget's two-comparator family while retaining the lowest descriptive preference distance. Its 210 main run rows average 1126 implemented event records and 98.6% final-front candidate-position co-occurrence; these summaries do not establish lineage, replay, or human-review value.
...
Adaptive preference elitism changes reported pooled hypervolume by 0.17%, but its direct effect is unresolved after Holm correction across seven scenarios (minimum adjusted $p=0.0722$). TRACE-MOEA remains slightly above `NoPreferenceRanking` under the tested empirical, expanded, analytic, and alternative-reference schemes, while formulation and preference sensitivity includes adverse and near-null cells. The deterministic-rule order changes with normalization, and 9.79% of rerun front points are clipped by the reported empirical bounds. Its measured record-level effect is the addition of preference-best-response records and a change in event--front position co-occurrence; actual replacement is not counted. The `NoScheduleRisk` combined-control contrast is likewise unresolved and cannot isolate a schedule-risk contribution; the external checks remain descriptive. The evidence therefore supports a budget-constrained proxy-search framework with implemented run-level event records and co-occurrence summaries, not normalization-invariant decision superiority. Stable-ID lineage and replay evidence, expert labels, electrical checks, and human evaluation remain necessary before claims about chronology, review effectiveness, or deployment.
...
27. Raji, I.D.; Smart, A.; White, R.N.; Mitchell, M.; Gebru, T.; Hutchinson, B.; Smith-Loud, J.; Theron, D.; Barnes, P. Closing the AI accountability gap: defining an end-to-end framework for internal algorithmic auditing. In *Proceedings of the 2020 Conference on Fairness, Accountability, and Transparency*, Barcelona, Spain, 27–30 January 2020; pp. 33–44. https://doi.org/10.1145/3351095.3372873

28. Liang, J.; Ban, X.; Yu, K.; Qu, B.; Qiao, K.; Yue, C.; Chen, K.; Tan, K.C. A Survey on Evolutionary Constrained Multiobjective Optimization. *IEEE Transactions on Evolutionary Computation* **2023**, *27*(2), 201--221. https://doi.org/10.1109/TEVC.2022.3155533
...
34. Huanca, D.H.; Falcao, D.M.; Bento, M.E.C. Transmission Expansion Planning Considering Storage, Flexible AC Transmission System, Losses, and Contingencies to Integrate Wind Power. *Energies* **2024**, *17*(7), 1777. https://doi.org/10.3390/en17071777

35. He, R.; Hao, J.; Zhou, H.; Chen, F. Multi-Objective Collaborative Optimization of Distribution Networks with Energy Storage and Electric Vehicles Using an Improved NSGA-II Algorithm. *Energies* **2025**, *18*(19), 5232. https://doi.org/10.3390/en18195232
