...
def main() -> None:
    ...
    for cond in CONDITIONS:
        ...
        result[cond] = {
            ...
            "prompt_token_estimate_mean": round(statistics.mean(est_by_cond[cond]), 1),
        ...
        print(f"{cond:<42} {r['n']:>4} {r['token_input_median']:>7} {r['token_input_mean']:>8} {r['token_output_mean']:>9} "
              f"{r['latency_ms_mean']:>9} {r['latency_ms_median']:>8} {r['prompt_token_estimate_mean']:>7} {dict(sorted(retries.items()))!s:>8}")
    ...
    est_reduction = 1 - c4["prompt_token_estimate_mean"] / c2["prompt_token_estimate_mean"]
    result["derived"] = {
    ...
    out = HERE / "efficiency_stats.json"
