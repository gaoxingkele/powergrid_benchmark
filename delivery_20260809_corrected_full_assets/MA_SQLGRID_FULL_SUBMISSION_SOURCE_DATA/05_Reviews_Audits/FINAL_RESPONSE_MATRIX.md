...
| D5 | Rights, expert governance, and submission metadata are open | Added file-level `RIGHTS_INVENTORY.csv`; restricted/uncertain source assets are excluded from the local package; manual gates remain explicit | MANUAL | Corresponding email, exact funder identity/role, all-author approval, permissions, ethics decision if experts are added, and repository release |
...
| J5 | GenAI disclosure and mandatory declarations incomplete | Added a Methods disclosure covering prose, experiment/audit suggestions, code/LaTeX, provenance, build/QA, exclusions, validation, and author responsibility; expanded Acknowledgments without inventing an exact backend ID | CLOSED-FINAL for recorded use; MANUAL for missing metadata | Authors must confirm exact Codex product/model/version from account records, email, funder wording/role, rights, and final declarations |
...
power-grid semantic validation remain unresolved scientific gates. The email,
funding-agency wording/role, permissions, author approvals, AI model/version
confirmation, and public repository tag remain manual pre-submission gates.
