...
本节收录老师资料中对后续论文选题和数据准备确实有帮助、但不适合全部塞进正文主线的资源。它们包括更细粒度的数据集、专题 benchmark、Mendeley/Kaggle 条目、设备诊断与配电网真实数据等。建议在具体选题确定后，再按方向深入筛选。

### 11.1 运行优化、OPF 与电网图建模扩展资源

1. **OPFData**
   - 用于 AC-OPF 大规模样本学习与拓扑扰动泛化。
   - 入口：http://arxiv.org/pdf/2406.07234.pdf

2. **PFΔ / PF∆ benchmark**
   - 用于学习型潮流计算的统一 benchmark 评测。
   - 入口：https://www.climatechange.ai/papers/iclr2025/67

3. **Scalable bilevel optimization for OPF datasets**
   - 更偏“如何构造高代表性 OPF 数据集”的方法参考。
   - 入口：http://arxiv.org/pdf/2304.10912.pdf

4. **Alberta Power Network case**
   - 适合图建模、网络拓扑恢复、图学习与可视化分析。
   - 入口：https://arxiv.org/html/2504.07870v1

### 11.2 风光预测与可再生能源扩展资源

1. **SOLETE**
   - 丹麦风光共址实测数据，15 个月，含气象、风机与光伏功率，适合站点级短期预测。
   - 数据论文：https://linkinghub.elsevier.com/retrieve/pii/S2352340922002578
   - 代码仓库：https://github.com/DVPombo/SOLETE

2. **Synthetic Photovoltaic and Wind Power Forecasting Data (Germany)**
   - 德国合成风光多站点预测数据，适合迁移学习和多站点预测。
   - 数据入口：https://daks.uni-kassel.de/entities/dataset/57ea0681-d8b2-4e76-b31d-578178961f87
   - 论文入口：https://arxiv.org/pdf/2204.00411.pdf

3. **Chinese State Grid Renewable Energy Generation Forecasting Competition Dataset**
   - 中国国家电网风光竞赛数据，适合国内风光预测论文。
   - 论文入口：https://www.nature.com/articles/s41597-022-01696-6

4. **NREL / PERFORM regional datasets**
   - 适合区域尺度、多时间尺度风光和负荷预测。
   - 入口：https://docs.nrel.gov/docs/fy24osti/83828.pdf
   - 补充入口：https://docs.nrel.gov/docs/fy23osti/79498.pdf

5. **S2S4E Europe**
   - 28 国国家级日尺度风光与需求预测，更适合中长期预测与气候服务，而非站点级短期预测。
   - 入口：https://essd.copernicus.org/articles/13/2259/2021/

6. **France national wind-solar benchmark dataset**
   - 法国全国风光发电 benchmark，适合国家级预测比较。
   - 入口：https://arxiv.org/abs/2504.16100

7. **中国逐小时气候+风光发电数据（31省）**
   - 适合中国本土区域风光、气象驱动预测。
   - 论文入口：https://www.nature.com/articles/s41597-025-04479-x

8. **ERA5 单层小时数据**
   - 适合作为中国或全球风光/负荷预测的气象侧输入。
   - 入口：https://cds.climate.copernicus.eu/datasets/reanalysis-era5-single-levels

9. **风光小型公开数据**
   - Wind and solar power generation dataset（Mendeley）：
     https://data.mendeley.com/datasets/gxc6j5btrx
   ...
   - Solar Irradiance and Weather Forecasting Dataset（Kaggle）：
     https://www.kaggle.com/datasets/zoya77/solar-irradiance-and-weather-forecasting-dataset

### 11.3 故障保护、PMU 与电能质量扩展资源

1. **Dataset for Testing Transmission Line Protections: Energy Transition Oriented**
   - 波形级保护仿真数据，适合故障检测、分类、定位。
   - 入口：https://data.mendeley.com/datasets/9s58y9dkdt/1

2. **Power System Faults Dataset**
   - 结构化故障日志型数据，适合故障类型分类和风险管理。
   - 入口：https://www.kaggle.com/datasets/ziya07/power-system-faults-dataset/versions/1

3. **Synthetic PMU Data (TAMU)**
   - 适合 PMU 事件检测、振荡分析与扰动识别。
   - 入口：https://electricgrids.engr.tamu.edu/electric-grid-test-cases/synthetic-pmu-data/

4. **Distribution PMU Data Set**
   - 适合配电网 PMU/μPMU 监测与事件分析。
   - 入口：https://bigdata.seas.gwu.edu/data-set-4-distribution-pmu-data-set/

5. **Power Quality Disturbance Classification baseline repo**
   - 提供 PQD 数据生成、S-transform 和 CNN 分类完整 pipeline。
   - 入口：https://github.com/Vishal-Prakash-1/Classification-Of-Power-Quality-Disturbances-Using-Deep-Learning

6. **IEEE 1159 风格 PQD 生成器**
   - 适合构造标准电能质量扰动数据。
   - 参考入口：
     https://cris.tau.ac.il/en/publications/open-source-generator-for-power-quality-disturbances-based-on-ieee

### 11.4 电力设备、DGA 与专题监测扩展资源

1. **Mendeley DGA 数据集**
   - 真实 DGA 油气分析与故障标签。
   - 入口：https://data.mendeley.com/datasets/98f4z3f8tx

2. **317 条真实变压器故障数据集**
   - 适合规则法、SVM 与传统 ML 对比。
   - 入口：https://ieeexplore.ieee.org/document/10271490/

3. **TLR-ADASYN 相关 DGA 数据**
   - 更适合研究不平衡数据处理。
   - 入口：https://www.nature.com/articles/s41598-023-43729-z

4. **CWRU 轴承数据集**
   - 虽然不是电力系统专属，但对断路器、旋转设备诊断、迁移学习仍有参考价值。
   - 入口：https://engineering.case.edu/bearingdatacenter

5. **NASA ADAPT**
   - 适合电力系统级故障诊断与断路器相关研究参考。
   - 入口：https://data.nasa.gov/

### 11.5 配电网、智能电表与 EV 扩展资源

1. **IEEE 123 节点配网**
   - 不平衡三相配网 benchmark，适合 OpenDSS 配网研究。
   - GitHub 镜像：
     https://github.com/tshort/OpenDSS/blob/master/Distrib/IEEETestCases/123Bus/IEEE123Master.dss

2. **SMART-DS (NREL)**
   - 合成配电网，含 DER/EV，适合配网控制与数据驱动研究。
   - 入口：https://opennetzero.org/nrel-smart-ds

3. **欧洲真实 LV 配网（Mendeley）**
   - 10290 节点 + 20 天智能电表。
   - 入口：https://data.mendeley.com/datasets/685vgp64sm/1

4. **欧洲三套真实配网（Mendeley）**
   - 工业/农村/城市三类配网，含 OpenDSS 模型与智能电表曲线。
   - 入口：https://data.mendeley.com/datasets/gspyzvvrhm/1

5. **UrbanEV / ST-EVCDP**
   - 中国城市 EV 充电数据与建模代码，适合 EV charging 与时空预测。
   - 入口：https://github.com/IntelligentSystemsLab/ST-EVCDP

### 11.6 代码库与开源基线扩展

1. **ml4ps**
   - 电网图学习与机器学习框架。
   - 入口：https://github.com/bdonon/ml4ps

2. **power_system_operation**
   - 电网运行优化与可再生场景构造。
   - 入口：https://github.com/xuwkk/power_system_operation

3. **Open-source-power-dataset / PSML**
   - 多尺度电网时序数据与基线代码。
   - 入口：https://github.com/tamu-engineering-research/Open-source-power-dataset

4. **SOLETE**
   - 风光数据配套预测代码。
   - 入口：https://github.com/DVPombo/SOLETE

### 11.7 使用这些额外资料时的原则

1. 正文优先使用高复用、社区认可度高、口径清晰的主干资源。
2. 本节资源更适合在具体选题确定后，按方向做第二轮深入筛选。
3. 若某资源获取成本高、标签空间模糊或数据规模偏小，不建议直接作为主实验唯一数据源。
4. Mendeley、Kaggle、论文附录型数据应特别注意版本、字段说明和可持续访问性。
