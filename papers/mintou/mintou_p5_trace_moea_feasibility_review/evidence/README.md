...
- `evidence/tables/real_project_review_leaderboard.csv`: method-level aggregate table for TRACE-MOEA, baselines, and ablations.
...
- `evidence/runs/*_v1_weak.*`, `*_v2_weak.*`, `*_v3_near_miss.*`: preserved negative and near-miss revisions.

Current value signal: promising proxy-level public signal, with TRACE-MOEA exceeding the strongest baseline by `1.23%` and strongest ablation by `3.71%`.
