...
_id: `relax_discrete_search_to_continuous` · journal=`peerj-computer-science` · primary_count=1_

**Plain alias**. _Relax discrete search to continuous_

**Operational signature**. discrete search → continuous relaxation → optimize

**When to apply (in `peerj-computer-science`)**. When local accepted papers in this venue execute this move (see examples). Prefer composing with a journal-house pattern below.
...
## Local examples
- Relational Graph Convolutional Networks: A Closer Look (`peerj-computer-science__2022__relational…`) — bottleneck: forecasting accuracy / uncertainty under nonstationarity
